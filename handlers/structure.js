...
function aggregateFixedDaily(raw,days){
  ...
  for(let i=0;i<used.length;i+=days){
    const g=used.slice(i,i+days);if(g.length!==days)continue;
    out.push([Number(g[0][0]),String(g[0][1]),String(Math.max(...g.map(k=>Number(k[2])))),String(Math.min(...g.map(k=>Number(k[3])))),String(g.at(-1)[4]),String(g.reduce((s,k)=>s+Number(k[5]||0),0)),Number(g.at(-1)[6])]);
  }
  return{rows:out,partialTailBars:raw.length-full,sourceBars:raw.length};
}
function syntheticStructureFromDaily(symbol,interval,agg){
  const I=spot._internals||{},rows=agg.rows||[],days=interval==='28d'?28:14;
