...
function defaultService(getStore){
  if(!singleton){
    ...
    singleton=createSignalPerformanceService({store:createBlobStore({getStore}),resolver:createBinanceResolver({})});
  }
  return singleton;
}
module.exports=async function handler(req,res,ctx={}){
  ...
  try{const service=ctx.service||defaultService(ctx.getStore);const evaluation=await service.evaluateDue();const data=await service.getPerformance({classKey,horizon,symbol,limit});return res.status(200).json({status:'ok',evaluation,...data})}
