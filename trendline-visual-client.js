(()=>{
  if(window.__pulseTrendlineVisualV6)return;window.__pulseTrendlineVisualV6=true;
  ...
  function removeSeries(){
    if(hostChart){for(const s of [supSeries,resSeries]){if(s)try{hostChart.removeSeries(s)}catch{}}}
    supSeries=resSeries=null;
  }
  ...
  function normalizedTouches(L,full){
    const raw=Array.isArray(L?.touchPoints)&&L.touchPoints.length?L.touchPoints:(L?.touchBars||[]).map((barIndex,i)=>({barIndex,price:L?.touchPrices?.[i]}));
    return raw.map(p=>{
      const barIndex=Number(p?.barIndex),c=full[barIndex],time=secOf(p?.time??candleTime(c));
      const price=Number.isFinite(+p?.price)?+p.price:(Number.isFinite(barIndex)?(+L.intercept)+(+L.slope)*barIndex:NaN);
      return{barIndex,time,price};
    }).filter(p=>Number.isFinite(p.barIndex)&&p.time!=null&&Number.isFinite(p.price)).sort((a,b)=>a.barIndex-b.barIndex);
  }
  function lineData(L,full){
    if(!L||L.state==='broken')return[];
    ...
    const visibleEnd=visibleStart+(F?.b?.length||full.length)-1;
    // Only use actual swing touches that are inside the candles currently shown on screen.
    // If fewer than two touches are visible, hide the line instead of showing a detached fragment.
    ...
    if(b.time<=a.time||b.barIndex-a.barIndex<3)return[];
    return[{time:a.time,value:a.price},{time:b.time,value:b.price}];
  }
  function renderNative(){
    try{
      const oldSvg=$('#trendSvg');if(oldSvg)oldSvg.innerHTML='';
      if(!F||!DATA||!$('#tTrend')?.checked||!ensureSeries()){
        if(supSeries)supSeries.setData([]);if(resSeries)resSeries.setData([]);return;
      }
    ...
    }catch(e){console.warn('trendline visual v6',e)}
  }
  function install(){
    if(typeof renderOverlay!=='function'||typeof $!=='function')return false;
    if(!renderOverlay.__pulseTrendlineVisualV6){
      ...
      renderOverlay.__pulseTrendlineVisualV6=true;
    }
