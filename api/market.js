...
async function getJson(path,params={}){
  ...
  for(const base of BASES){
    try{
      const r=await fetch(base+path+(qs?'?'+qs:''),{headers:{'User-Agent':'PulseRadar-Pro/3.6'}});
...
function liteDiagnostics(rows){
  ...
  const bins=[{label:'0-19',min:0,max:19},{label:'20-39',min:20,max:39},{label:'40-59',min:40,max:59},{label:'60-79',min:60,max:79},{label:'80-89',min:80,max:89},{label:'90-99',min:90,max:99},{label:'100',min:100,max:100}].map(b=>{const count=scores.filter(x=>x>=b.min&&x<=b.max).length;return{...b,count,rate:count/total*100}});
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    res.status(200).json({ok:true,version:'3.6',updatedAt:new Date().toISOString(),total:rows.length,eligible:rows.filter(r=>r.pass).length,liteDiagnostics:liteDiagnostics(rows),results:rows});
  }catch(e){res.status(502).json({ok:false,error:e?.message||'Data fetch failed'});}
};
