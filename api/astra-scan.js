...
const {createAstraAutoScanner,CONFIG,MANUS_CONFIG,PERPLEXITY_CONFIG,METHODS,VERSION,methodOf}=require('../lib/coin-scan/astra-auto-scanner.js');
let singleton=null;
...
module.exports=async function handler(req,res,ctx={}){
  ...
  try{
    ...
    if(stage==='oi'){
      ...
      return res.status(200).json(await scanner.oi(symbols,{method,asOf:n(q.asOf)}));
    }
    if(stage==='deep'){
      const symbols=symbolsOf(q);if(!symbols.length)return res.status(400).json({status:'error',version:VERSION,method,error:'symbols required'});
      return res.status(200).json(await scanner.deep(symbols,{method,market:marketOf(q),asOf:n(q.asOf)}));
    }
    return res.status(400).json({status:'error',version:VERSION,method,error:'unknown stage',allowed:['universe','oi','deep'],methods:Object.values(METHODS),config:{astra:CONFIG,manus:{...CONFIG,...MANUS_CONFIG},perplexity:{...CONFIG,...PERPLEXITY_CONFIG}}});
