...
async function derivatives(symbol){const now=Date.now(),dayAgo=now-24*60*60*1000;const settled=await Promise.allSettled([
  ...
  getFutures('/fapi/v1/aggTrades',{symbol,limit:'1000'}),
  getFutures('/fapi/v1/ticker/24hr',{symbol})
]);
const [pR,oR,hR,oldR,tR,kR,aR,qR]=settled;if(pR.status!=='fulfilled'||qR.status!=='fulfilled')return{ok:true,mode:'deriv',available:false,symbol,reason:'USD-M 선물 데이터 없음'};
const p=pR.value||{},o=oR.status==='fulfilled'?oR.value:{},h=hR.status==='fulfilled'&&Array.isArray(hR.value)?hR.value:[],oldH=oldR.status==='fulfilled'&&Array.isArray(oldR.value)?oldR.value:[],t=tR.status==='fulfilled'&&Array.isArray(tR.value)?tR.value:[],ks=kR.status==='fulfilled'&&Array.isArray(kR.value)?kR.value:[],ag=aR.status==='fulfilled'&&Array.isArray(aR.value)?aR.value:[],q=qR.value||{};
...
if(ratio==null&&ag.length){let buy=0,sell=0;for(const x of ag){const qv=num(x.q,0);if(x.m) sell+=qv; else buy+=qv}if(sell>0){ratio=buy/sell;takerSource='recent aggregate trades'}}
