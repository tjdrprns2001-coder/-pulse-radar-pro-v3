...
const params={emaFast:3,emaMid:5,emaSlow:8,emaSeedBars:8,atrPeriod:3,maxDistanceToEma60Atr:20};
const r=D.analyze({candles:c,analysisAsOf:asOf,params});
const scan=D.scanCandidates({candles:c,analysisAsOf:asOf,params});
assert.equal(r.mode,'SHADOW_ONLY');assert.equal(r.rankingContribution,0);assert(['CANDIDATE','NOT_CONFIRMED'].includes(r.status));
for(const x of scan){const prefix=c.slice(0,x.index+1),z=D.analyze({candles:prefix,analysisAsOf:prefix.at(-1).closeTime+1,params});assert.equal(z.status,'CANDIDATE','one-pass candidate must match prefix analyzer')}
