...
const r=D.analyze({candles:c,analysisAsOf:asOf,params:{emaFast:3,emaMid:5,emaSlow:8,holdBars:2}});
assert.equal(r.mode,'SHADOW_ONLY');assert.equal(r.rankingContribution,0);assert(['CANDIDATE','NOT_CONFIRMED'].includes(r.status));
assert.throws(()=>D.analyze({candles:[...c.slice(0,-1),{...c.at(-1),partial:true}],analysisAsOf:asOf,params:{emaFast:3,emaMid:5,emaSlow:8}}),/CLOSED_ONLY/i);
