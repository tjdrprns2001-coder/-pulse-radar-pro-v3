...
(async()=>{
 ...
 const refreshStore=createMemoryBowl224Store();
 await refreshStore.putEvent({eventId:'partial',symbol:'AAAUSDT',cohort:'TEST',group:'B',source:'bowl224-formal',signalCloseTs:0,entryPrice:100});
 await refreshStore.putOutcome('partial',{eventId:'partial',horizons:{d7:{status:'unavailable'}}});
 let evalFetch=0;
 const evalRunner=createBowl224Runner({provider:{async getKlinesRange(){evalFetch++;const future=Array.from({length:168},(_,i)=>[(i+1)*H,100,101,99,100,1,(i+2)*H-1,100,1,1,1,0]);return{rows:future,coverage:{complete:true}}}},store:refreshStore});
 await evalRunner.evaluateBatch({runId:'eval',limit:20});assert(evalFetch>=1);assert.equal((await refreshStore.getOutcome('partial')).horizons.d7.status,'evaluated');
