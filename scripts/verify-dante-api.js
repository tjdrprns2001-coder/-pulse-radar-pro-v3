...
(async()=>{
  ...
  let r=await call({symbol:'btcusdt',start:'2021-01-01',end:'2025-01-01',frictionPct:'0.3',sensitivity:'1',compact:'1'},{runtime});
  assert.equal(r.code,200);assert.equal(r.payload.status,'ok');assert.equal(args.symbol,'BTCUSDT');assert.equal(args.frictionPct,.3);assert.equal(args.sensitivity,true);assert.equal(args.compact,true);
