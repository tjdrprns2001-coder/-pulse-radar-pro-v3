...
function input(over={}){
 ...
 const intelligence={updatedAt:D-500,cex:{exchangeCount:3,spotCount:2,derivativesCount:2,maxPriceDispersionPct:.2,spotFuturesBasisPct:.05,sources:[
...
assert.equal(r.gates.find(x=>x.id==='execution').pass,null,'execution gate stays explicitly unevaluated without depth/spread ledger');
...
(async()=>{
 const store=createMemoryStore();assert(await store.putMarketValidationSnapshot(r.snapshotId,r.snapshot));assert.equal(await store.putMarketValidationSnapshot(r.snapshotId,r.snapshot),false);const saved=await store.getMarketValidationSnapshot(r.snapshotId);assert.equal(saved.snapshotId,r.snapshotId);assert.equal((await store.listMarketValidationSnapshots()).length,1);
