...
assert.equal(replay.length,4);assert.equal(replay[0].endIndex,2);assert.equal(replay[0].nextEligibleExecutionIndex,3);assert.equal(replay[0].executionPolicy,'NEXT_ELIGIBLE_BAR');assert.equal(replay.at(-1).nextEligibleExecutionIndex,null);assert.equal(replay.at(-1).analysisAsOf,rows.at(-1).closeTime);
