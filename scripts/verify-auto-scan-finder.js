...
must(html.includes('id="autoRefreshSelect"'),'auto refresh selector missing');
must(js.includes("AUTO_REFRESH_KEY")&&js.includes("autoRefreshMinutes:0"),'manual-by-default refresh state missing');
must(js.includes("function armAutoRefresh()")&&js.includes("state.autoRefreshMinutes*60000"),'opt-in auto refresh scheduler missing');
must(!js.includes("setInterval(()=>{if(document.visibilityState==='visible')refresh(false)},60000)"),'legacy 60s autoscan cadence must be removed');
