...
function outcomeKo(v){return v==='REACHED'?'DOL 도달':v==='INVALIDATED_BEFORE_REACH'?'무효화 선행':v==='AMBIGUOUS'?'동일봉 충돌':v==='만료'?'시간 만료':v==='NO_SIGNAL'?'신호 없음':v==='PENDING_REFERENCE'?'기준봉 대기':'추적 중'}
function outcomeTone(v){return v==='REACHED'?'confirm':v==='INVALIDATED_BEFORE_REACH'?'risk':v==='AMBIGUOUS'?'wait':v==='만료'||v==='NO_SIGNAL'?'na':'info'}
...
function renderJournal(){
  ...
  const c=journalState.current,o=c.outcome||{},events=(c.events||[]).map(x=>x.eventType).join(' → ')||'이벤트 없음',path=o.path||{};
  $('journalCurrent').innerHTML='<div class="journalHero '+outcomeTone(o.status)+'"><div><span>현재 스냅샷</span><b>'+escapeHtml(shortId(c.id))+' · '+escapeHtml(outcomeKo(o.status))+'</b></div><div><span>기준가 / 최대유리폭·최대불리폭</span><b>'+(finite(o.referencePrice)?escapeHtml(price(o.referencePrice))+' · ':'')+escapeHtml(fmt(path.mfePct,2))+'% / -'+escapeHtml(fmt(path.maePct,2))+'%</b></div><div><span>독립 이벤트 · 순서 '+escapeHtml(shortId(c.sequenceId))+'</span><b>'+escapeHtml(events)+'</b></div></div>';
  ...
  $('journalHorizons').innerHTML=['h4','h12','h24','h48'].map(k=>{const h=o.horizons?.[k]||{},done=h.status==='FINALIZED';return '<div class="horizonCell '+(done?'done':'pending')+'"><span>'+k.toUpperCase()+'</span><b>'+(done?escapeHtml(h.result||'만료'):'대기')+'</b><small>MFE '+escapeHtml(fmt(h.mfePct,2))+'% · MAE -'+escapeHtml(fmt(h.maePct,2))+'%</small></div>'}).join('');
  const tt=c.trendline?.telemetry;if(tt){$('journalStats').innerHTML+='<span>추세선 동일봉 후보 '+(tt.sameBarWouldConfirm?'✓':'-')+'</span><span>무효 대기구간 '+escapeHtml(String(tt.deadZoneBars??0))+'봉</span><span>확인 +'+escapeHtml(String(tt.confirmationBarsAfterTouch??'-'))+'봉</span>'}
  $('journalRecent').innerHTML=(journalState.recent||[]).slice(0,4).map(x=>'<div class="journalRow"><div><b>'+escapeHtml(shortId(x.id))+'</b><span>'+escapeHtml(x.stage||'N/A')+' · '+escapeHtml((x.events||[]).map(e=>e.eventType).slice(-3).join(' → ')||'이벤트 없음')+'</span></div><div class="journalOutcome '+outcomeTone(x.outcome?.status)+'">'+escapeHtml(outcomeKo(x.outcome?.status))+'<small>MFE '+escapeHtml(fmt(x.outcome?.path?.mfePct,2))+'% · MAE -'+escapeHtml(fmt(x.outcome?.path?.maePct,2))+'%</small></div></div>').join('');
}
function gateKo(v){return({INSUFFICIENT:'표본 부족',OBSERVING:'관찰 중',REVIEWABLE_INITIAL:'초기 검토점 동결',VALIDATING:'검증 표본 수집',INSUFFICIENT_VALIDATION:'검증 TF 부족',REVIEWABLE_VALIDATED:'검토 가능 · 재현됨',REVIEWABLE_UNSTABLE:'검토 가능 · 불안정',PARAMS_CHANGED:'게이트 파라미터 변경'}[v]||v||'대기')}
...
function renderGate(){
  ...
  if(!gateState){badge.textContent='대기';setTone(badge,'tone-na');$('gateProgress').innerHTML='<div class="gateCell"><span>유효 재시험 표본</span><b>0 / 200</b></div>';$('gateTf').innerHTML='';$('gateMeta').textContent='24시간 결과가 확정된 추세선 재시험 이벤트만 준비도 표본으로 집계';return}
  ...
  $('gateProgress').innerHTML='<div class="gateCell"><span>유효 재시험 표본</span><b>'+escapeHtml(String(p.eligibleN||0))+' / 200</b></div><div class="gateCell"><span>관찰 경과</span><b>'+escapeHtml(days)+'일 / 14일</b></div><div class="gateCell"><span>압축 보관 표본</span><b>'+escapeHtml(String(p.archiveN||0))+' · 대기 '+escapeHtml(String(p.pendingArchived||0))+'</b></div><div class="gateCell"><span>검증</span><b>'+(val?escapeHtml(String(val.eligibleN))+' / 50':escapeHtml(String(r?.validationProgress?.eligibleN||0))+' / 50')+'</b></div>';
  ...
  $('gateTf').innerHTML='<span>탐색 시간봉 '+escapeHtml(dTf.join(', ')||'-')+'</span><span>검증 TF '+escapeHtml(vTf.join(', ')||'-')+'</span><span>회차 공통 시간봉 '+escapeHtml(roundTf.join(', ')||'-')+'</span>';
  const issues=r?.decision?.stability?.issues||[];$('gateMeta').textContent=(r?.roundId?('회차 '+shortId(r.roundId)+' · '):'')+'게이트 '+shortId(gateState.gateParamsHash)+(issues.length?' · '+issues.join(', '):'')+' · 검증 완료여도 자동 활성화하지 않음';
}
function updateGate(){
  if(!GATE||!gateStore||!journalStore)return;
  try{gateState=GATE.evaluateGate({journalDb:journalStore.export(),store:gateStore});renderGate()}catch(e){gateState=null;const badge=$('gateState');if(badge){badge.textContent='게이트 오류';setTone(badge,'tone-risk')}}
}
