...
function chartTechnical(technical){if(!technical)return null;const ma=technical.ma||{},picked={};for(const p of[112,224,448]){if(Number.isFinite(Number(ma[p])))picked[p]=Number(ma[p]);if(Array.isArray(ma['series'+p]))picked['series'+p]=ma['series'+p]}return{...technical,ma:picked}}
function compose({canvas,panels={},renderer,documentRef=(typeof document!=='undefined'?document:null),symbol='BTCUSDT'}={}){
  ...
  ORDER.forEach((tf,idx)=>{
    ...
    renderer.draw(off,{symbol,timeframe:tf,candles:p.candles,analysis:p.analysis,smc:p.smc,liquidity:p.liquidity,ict:p.ict,technical:chartTechnical(p.technical),show:{trend:true,structure:true,pd:true,smc:true,liquidity:true,profile:false,ma:true,dante:false,labels:true}});
