...
function analyze({candles=[],analysisAsOf,params={}}={}){
  ...
  if(n<Math.max(P.lookbackBars,P.volumeLookback)+2)return C.freeze({version:VERSION,mode:'SHADOW_ONLY',status:'DATA_INSUFFICIENT',state:'NO_SETUP',rankingContribution:0});
  ...
  return C.freeze({version:VERSION,mode:'SHADOW_ONLY',status:candidate?'CANDIDATE':'NOT_CONFIRMED',state:candidate?'V_REVERSAL_CANDIDATE':'NO_SETUP',dropPct:drop,recoveryFraction:recovery,barsSinceLow,rvol:rv,evidenceFactIds:facts,params:P,paramsHash:C.paramsHash(P),rankingContribution:0});
}
return{VERSION,DEFAULTS,median,analyze};
});