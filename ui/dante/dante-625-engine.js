...
function analyze({candles=[],analysisAsOf,params={},market='KR_EQUITY'}={}){
  ...
  if(!P.allowedMarkets.includes(market))return C.freeze({version:VERSION,sourceClass:SOURCE_CLASS,mode:'SHADOW_ONLY',status:'NOT_APPLICABLE',state:'MARKET_SESSION_UNSUPPORTED',market,rankingContribution:0});
  if(candles.length<3)return C.freeze({version:VERSION,sourceClass:SOURCE_CLASS,mode:'SHADOW_ONLY',status:'DATA_INSUFFICIENT',state:'NO_SETUP',market,rankingContribution:0});
  ...
  return C.freeze({version:VERSION,sourceClass:SOURCE_CLASS,mode:'SHADOW_ONLY',status:ok?'CANDIDATE':'NOT_CONFIRMED',state:ok?'SETUP':'NO_SETUP',market,priorReturnPct:priorRet,evidenceFactIds:facts,params:P,paramsHash:C.paramsHash(P),rankingContribution:0});
}
return{VERSION,SOURCE_CLASS,DEFAULTS,analyze};
});