...
function analyze({candles=[],analysisAsOf,params={}}={}){
  C.assertClosedCandles(candles,analysisAsOf);const P=C.normalizeParams(params),n=candles.length;if(n<P.emaSeed.seedBars+2)return C.freeze({version:VERSION,status:'DATA_INSUFFICIENT',state:'NO_STRIKE',evidenceFactIds:[],paramsHash:C.paramsHash(P)});
  const close=candles.map(x=>Number(x.close)),seed=P.emaSeed.seedBars,e112=C.seededEma(close,P.ema.fast,seed),e224=C.seededEma(close,P.ema.pivot,seed),e448=C.seededEma(close,P.ema.long,seed),i=n-1,prev=i-1,p=close[i],pp=close[prev],facts=[];
