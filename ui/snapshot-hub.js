...
function updateHistory(){const u=new URL(location.href);u.searchParams.set('mode',mode);u.searchParams.set('symbol',symbol());if(u.href!==location.href)history.replaceState(null,'',u)}
...
function relayChildMessage(e){const d=e.data||{};if(d.type==='pulse-symbol-sync'){const s=clean(d.symbol),prev=symbol();$('hubSymbol').value=s;updateHistory();if(s!==prev)syncOuter(s);return}if(d.type==='pulse-nav'){try{parent.postMessage(d,'*')}catch{}}}
