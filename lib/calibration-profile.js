...
const CALIBRATION_VERSION='calib-2026-09-14-b';
...
const PARAM_META={
  confidence:{step:1,min:60,max:90},touchTol:{step:.03,min:.30,max:.80},minSpan:{step:2,min:6,max:24},
  maxBreach:{step:.02,min:.06,max:.30},parallel:{step:.01,min:.06,max:.24},
  minShoulderSymmetry:{step:.05,min:.35,max:.75},minHeadProminenceAtr:{step:.10,min:.25,max:1.2},
  minSimilarity:{step:.05,min:.35,max:.85},minDepthAtr:{step:.10,min:.45,max:1.5}
};
...
module.exports={ENGINE_VERSION,CALIBRATION_VERSION,MIN_SAMPLES_PER_FREE_PARAMETER,MIN_VALID_SAMPLES,SPLIT_RATIOS,RELIABILITY_BANDS,GLOBAL,TF_DELTA,RULES,TUNABLES,PARAM_META,FAMILY,tfConfig,tunablesFor,requiredSamplesFor};