...
function runWalkForward({symbol='UNKNOWN',candles=[],analysisStartTs=null,analysisEndTs=null,riceParams={},d256Params={},frictionPct=.2,horizons=DEFAULT_HORIZONS,strategies=['RICE_BOWL','DANTE_256']}={}){
  ...
  for(let i=Math.max(2,minSeed);i<candles.length-1;i++){
    ...
    if(strategies.includes('RICE_BOWL')){
      const r=Rice.analyze({symbol,candles:prefix,analysisAsOf:signalTs,riceParams,params:riceParams,previous:previousRice});
    ...
    if(strategies.includes('DANTE_256')){
      const d=D256.analyze({candles:prefix,analysisAsOf:signalTs,params:d256Params});
