...
const {runSensitivity,compactSensitivity}=require('./sensitivity.js');
...
function createDanteBacktestRuntime({fetchImpl=globalThis.fetch,now=()=>Date.now()}={}){
  ...
  async function run({
    ...
    const sensitivityOutput=sensitivityRows?(compact===true?compactSensitivity(sensitivityRows):sensitivityRows):null;
    const walkForwardOutput=compact===true?{version:wf.version,replayMode:wf.replayMode,symbol:wf.symbol,frictionPct:wf.frictionPct,horizons:wf.horizons,analysisStartTs:wf.analysisStartTs,analysisEndTs:wf.analysisEndTs,eventCount:wf.eventCount,diagnostics:wf.diagnostics}:wf;
    return{
