...
function standard(exchange,{symbol,baseAsset,quoteAsset='USDT',lastPrice,priceChangePercent,quoteVolume24h,openInterestUsd=null,fundingRate=null}={}){
  ...
  return{exchange,symbol:String(symbol||'').toUpperCase(),baseAsset:base,quoteAsset:String(quoteAsset||'USDT').toUpperCase(),lastPrice:price,priceChangePercent:change,quoteVolume24h:vol??0,openInterestUsd:finite(openInterestUsd),fundingRate:finite(fundingRate),contractType:'PERPETUAL'};
}
...
async function loadBinance(){
  const [info,tickers]=await Promise.all([getJson(BASES.binance+'/fapi/v1/exchangeInfo'),getJson(BASES.binance+'/fapi/v1/ticker/24hr')]);
  ...
  for(const x of info?.symbols||[]){
    ...
    const row=standard('binance',{symbol:x.symbol,baseAsset:x.baseAsset,quoteAsset:x.quoteAsset,lastPrice:t.lastPrice,priceChangePercent:t.priceChangePercent,quoteVolume24h:t.quoteVolume});
    if(row)out.push(row);
  }
  return out;
}
...
async function loadOkx(){
  ...
  for(const x of info?.data||[]){
    ...
    const row=standard('okx',{symbol:id,baseAsset:base,quoteAsset:'USDT',lastPrice:last,priceChangePercent:pctFromOpen(last,q.open24h),quoteVolume24h:last!=null&&baseVol!=null?last*baseVol:0});
    if(row)out.push(row);
  }
  return out;
}
...
async function loadKucoin(){
  ...
  for(const x of d?.data||[]){
    ...
    const row=standard('kucoin',{symbol:x.symbol,baseAsset:x.baseCurrency,quoteAsset:x.quoteCurrency,lastPrice:last,priceChangePercent:pctMaybeDecimal(x.priceChgPct),quoteVolume24h:x.turnoverOf24h,openInterestUsd:finite(x.openInterest)!=null?finite(x.openInterest)*last:null,fundingRate:x.fundingFeeRate});
    if(row)out.push(row);
  }
  return out;
}
...
module.exports={EXCHANGE_ORDER,cleanBase,userBase,standard,mergeMarkets,rankRows,collect,market,catalog,find,resetCache,LOADERS};
