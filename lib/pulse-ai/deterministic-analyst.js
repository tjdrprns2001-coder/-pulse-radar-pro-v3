...
function deterministicAnswer({question='',selectedSymbol=null,selectedDetail=null,scan,eventCatalysts=[],researchStatus=null}={}){
  ...
  if(symbol&&(mentioned||/이거|종목|상태|어때|왜|근거|눌림|리테스트|진입|점화/.test(q))){const base=focusSymbol(scan,symbol),row=base?.found?base:(focusFromDetail(selectedDetail,symbol)||base);return{intent:'symbol',symbol,answer:symbolAnswer(row)}};
