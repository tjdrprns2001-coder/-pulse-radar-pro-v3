...
function topCandidates(scan,limit=8){
  ...
  const clean=items.filter(x=>!isExtended(x)&&String(x?.scanClass?.key||'')!=='DISTRIBUTION-RISK');
...
function sectorRows(scan,limit=6){
  ...
  const all=[...groups.entries()].map(([sector,rows])=>{
    ...
    return{sector:sector==='기타'?'미분류':sector,count:rows.length,candidates:candidateRows.length,avgChange24h:avg==null?null:Number(avg.toFixed(2)),leaders,unclassified:sector==='기타'};
  }).filter(x=>x.candidates>0);
  const named=all.filter(x=>!x.unclassified).sort((a,b)=>b.candidates-a.candidates||b.count-a.count);
  const other=all.filter(x=>x.unclassified).sort((a,b)=>b.candidates-a.candidates||b.count-a.count);
  return (named.length?named.concat(other):all).slice(0,limit);
}
function marketPulse(scan){
  ...
  const ratio=up/den,median=n(b.median),majors=b.majors||{},majorVals=[n(majors.BTC),n(majors.ETH),n(majors.SOL)].filter(x=>x!=null);
  const majorAvg=majorVals.length?majorVals.reduce((s,x)=>s+x,0)/majorVals.length:null;
  const allMajorsDown=majorVals.length>=2&&majorVals.every(x=>x<0),allMajorsUp=majorVals.length>=2&&majorVals.every(x=>x>0);
  let regime='MIXED';
  if(ratio>=.62&&(median==null||median>=.4))regime=(allMajorsDown&&majorAvg!=null&&majorAvg<=-.25)?'ALT_BREADTH':'RISK_ON';
  else if(ratio<=.38&&(median==null||median<=-.4))regime=(allMajorsUp&&majorAvg!=null&&majorAvg>=.25)?'MAJOR_DIVERGENCE':'RISK_OFF';
  return{
    regime,breadthRatio:Number(ratio.toFixed(3)),up,down,flat,total,medianChange24h:median,majorAvg:majorAvg==null?null:Number(majorAvg.toFixed(3)),
...
function dataQuality(scan,now=Date.now()){
  const h=scan?.dataHealth||{},coverage=scan?.marketCoverage||{},total=Math.max(1,n(scan?.scanCount,n(h.live,0)+n(h.delayed,0)+n(h.blocked,0)+n(h.errors,0))),live=n(h.live,0);
  const liveRatio=live/total,ageMs=n(scan?.updatedAt)!=null?Math.max(0,now-n(scan.updatedAt)):null;
  const futures=n(coverage.futures,0)+n(coverage.both,0),spot=n(coverage.spot,0)+n(coverage.both,0);
  let state='LIVE';
  if(scan?.status!=='ok'||n(h.errors,0)>0||n(h.blocked,0)>0)state='DEGRADED';
  else if(futures===0&&spot>0)state='DEGRADED';
  else if(n(h.delayed,0)>0||liveRatio<.98)state='PARTIAL';
  else if(ageMs!=null&&ageMs>180000)state='STALE';
  return{state,live,total,liveRatio:Number(liveRatio.toFixed(4)),futures,spot,delayed:n(h.delayed,0),blocked:n(h.blocked,0),errors:n(h.errors,0),partial:Boolean(scan?.partial),ageMs,sourceWarning:scan?.sourceWarning||null};
}
...
function summaryText(scan,researchStatus=null){
  ...
  const regime=m.regime==='RISK_ON'?'상승 확산':m.regime==='ALT_BREADTH'?'알트 확산 · 메이저 약세':m.regime==='RISK_OFF'?'위험 축소':m.regime==='MAJOR_DIVERGENCE'?'메이저 강세 · 시장 폭 약세':'혼조';
...
function deterministicAnswer({question='',selectedSymbol=null,selectedDetail=null,scan,eventCatalysts=[],researchStatus=null}={}){
  ...
  if(/RESEARCH\s*AI|리서치\s*AI|학습\s*상태|학습\s*(진행|결과|데이터)|모델\s*(상태|학습|활성)|SHADOW/i.test(q)&&researchStatus){
  ...
  if(symbol&&(mentioned||/이거|종목|상태|어때|왜|근거|눌림|리테스트|진입|점화/.test(q))){
    const base=focusSymbol(scan,symbol),row=base?.found?base:(focusFromDetail(selectedDetail,symbol)||base);
    return{intent:'symbol',symbol,answer:symbolAnswer(row)};
  }
