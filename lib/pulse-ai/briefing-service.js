...
function createBriefingService({scanService,gateway,marketIntelService=null,now=()=>Date.now(),cacheMs=30000}={}){
  ...
  async function addMarketIntel(context){if(!marketIntelService?.getOverview)return context;try{const m=await marketIntelService.getOverview();context.marketIntel={health:m.health||{},global:m.global||null,trending:(m.trending||[]).slice(0,10),categories:(m.categories||[]).slice(0,10),warnings:m.warnings||[]}}catch(e){context.marketIntel={health:{},global:null,trending:[],categories:[],warnings:[`시장 보강 데이터 오류: ${e.message||e}`]}}return context}
