...
function fallback(scan,detected,{warning=null,context=null,eventCatalysts=[],searched=false,researchStatus=null,aiConfigured=false,now=Date.now()}={}){
  ...
  const dataWarnings=[];
  const q=Analyst.dataQuality(scan,now);if(q.state!=='LIVE')dataWarnings.push(q.state==='DEGRADED'?'선물 우선 데이터 커버리지가 불완전합니다.':'스캐너 일부 데이터가 지연되거나 오래되었습니다.');
...
function createBriefingService({
  scanService,gateway,marketIntelService=null,researchAI=null,detailResolver=null,eventResolver=null,now=()=>Date.now(),cacheMs=30000,eventRefreshMs=300000
}={}){
  ...
  const detailCache=new Map(),eventCache=new Map();
  ...
  async function getSelectedDetail(scan,selectedSymbol){
    const symbol=String(selectedSymbol||'').trim().toUpperCase();
    if(!symbol||Analyst.focusSymbol(scan,symbol)?.found||typeof detailResolver!=='function')return null;
    const cached=detailCache.get(symbol);
    if(cached&&now()-cached.at<60000)return cached.value;
    try{
      const value=await detailResolver(symbol);
      if(value)detailCache.set(symbol,{at:now(),value});
      return value||null;
    }catch{return null}
  }
  function intelEventType(row={}){
    const t=String(row.titleKo||row.title||'').toLowerCase(),cat=String(row.category||'');
    if(/unlock|vesting|언락|베스팅/.test(t+cat))return'token_unlock';
    if(/delist|상장폐지/.test(t+cat))return'delisting';
    if(/futures|perpetual|선물/.test(t+cat))return'futures_listing';
    if(/list|listing|상장/.test(t+cat))return'listing';
    if(/mainnet|메인넷/.test(t+cat))return'mainnet';
    if(/upgrade|hard fork|업그레이드|하드포크/.test(t+cat))return'upgrade';
    if(/governance|proposal|vote|거버넌스|투표/.test(t+cat))return'governance';
    if(/airdrop|에어드롭/.test(t+cat))return'airdrop';
    if(/migration|migrate|마이그레이션/.test(t+cat))return'migration';
    return'official_news';
  }
  async function getLocalEvents(selectedSymbol){
    const symbol=String(selectedSymbol||'').trim().toUpperCase();
    if(!symbol||typeof eventResolver!=='function')return[];
    const cached=eventCache.get(symbol);
    if(cached&&now()-cached.at<300000)return cached.value;
    try{
      const intel=await eventResolver(symbol),base=symbol.replace(/USDT$/,'');
      const rows=(Array.isArray(intel?.events)?intel.events:Array.isArray(intel?.timeline)?intel.timeline:[]).slice(0,10);
      const value=rows.map((x,i)=>({
        id:`local-news-${base}-${i}-${String(x.link||x.title||'').slice(-24)}`,symbol:base,eventType:intelEventType(x),
        titleKo:String(x.titleKo||x.title||'이벤트·뉴스 확인'),summaryKo:[x.category,x.source].filter(Boolean).join(' · ')||'보조 뉴스',
        eventTime:x.eventDate||null,sourceTier:'C',sourceName:String(x.source||'뉴스 피드'),sourceUrl:x.link||null,confirmed:false
      }));
      eventCache.set(symbol,{at:now(),value});return value;
    }catch{return[]}
  }
  function decorate(base,scan,detected,researchStatus,selectedSymbol=null,selectedDetail=null){
    const snap=Analyst.snapshot(scan,researchStatus,selectedSymbol,now());
    const selectedFocus=snap.focus?.found?snap.focus:(Analyst.focusFromDetail(selectedDetail,selectedSymbol)||snap.focus);
    return{
      ...base,version:VERSION,generatedAt:base?.generatedAt||now(),updatedAt:scan?.updatedAt??base?.updatedAt??now(),
      scanSource:scan?.pulseAiScanSource||'local',
      scanUpdatedAt:scan?.updatedAt??null,marketPulse:snap.market,dataQuality:snap.dataQuality,
      candidates:snap.candidates,extended:snap.extended,sectors:snap.sectors,selectedFocus,
  ...
  async function scanAndContext(selectedSymbol=null,maxSymbols=16){
    ...
    const selectedDetail=await getSelectedDetail(scan,selectedSymbol);
    let context=buildContext(scan,detected,{maxSymbols,selectedSymbol,researchStatus});
    if(selectedDetail)context.selectedDetail=Analyst.detailContext(selectedDetail,selectedSymbol);
    context=addEventResearchContext(await addMarketIntel(context));
    previous=scan;
    return{scan,detected,researchStatus,context,selectedDetail};
  }
  async function getBrief(input={}){
    ...
    if(!force&&cachedBase&&cachedScan&&now()-cachedAt<cacheMs){const selectedDetail=await getSelectedDetail(cachedScan,selectedSymbol);return decorate(cachedBase,cachedScan,cachedDetected,cachedResearch,selectedSymbol,selectedDetail)}
    let scan,detected,researchStatus,context,selectedDetail;
    try{
      ({scan,detected,researchStatus,context,selectedDetail}=await scanAndContext(selectedSymbol,16));
    }catch{
      ...
      return decorate(out,broken,{events:[],sectorClusters:[]},null,selectedSymbol,null);
    }
    const eventRefreshDue=!lastEventResearchAt||now()-lastEventResearchAt>=eventRefreshMs;
    if(!gateway?.available){
      const localEvents=await getLocalEvents(selectedSymbol),eventRows=localEvents.length?localEvents:cachedCatalysts;
      cachedBase=fallback(scan,detected,{
        warning:'외부 생성형 AI가 비활성화되어 로컬 시장 분석을 표시합니다.',context,eventCatalysts:eventRows,
        searched:Boolean(localEvents.length||lastEventResearchAt>0),researchStatus,aiConfigured:false,now:now()
      });
      cachedScan=scan;cachedDetected=detected;cachedResearch=researchStatus;cachedAt=now();
      return decorate(cachedBase,scan,detected,researchStatus,selectedSymbol,selectedDetail);
    }
  ...
  async function chat(input={}){
    ...
    let scan,detected,researchStatus,context,selectedDetail;
    try{({scan,detected,researchStatus,context,selectedDetail}=await scanAndContext(selectedSymbol,20))}
    catch(e){e.statusCode=e.statusCode||503;throw e}
    if(!gateway?.available){
      const localEvents=await getLocalEvents(selectedSymbol),eventRows=localEvents.length?localEvents:cachedCatalysts;
      const det=Analyst.deterministicAnswer({question,selectedSymbol,selectedDetail,scan,eventCatalysts:eventRows,researchStatus});
      const out=fallback(scan,detected,{
        warning:'외부 생성형 AI가 비활성화되어 로컬 질의 분석을 사용했습니다.',context,eventCatalysts:eventRows,
        searched:Boolean(localEvents.length||lastEventResearchAt>0),researchStatus,aiConfigured:false,now:now()
      });
      return{...decorate(out,scan,detected,researchStatus,selectedSymbol,selectedDetail),answer:det.answer,answerMode:'deterministic',intent:det.intent};
    }
    try{
      ...
      return decorate(out,scan,detected,researchStatus,selectedSymbol,selectedDetail);
    }catch(e){
      const det=Analyst.deterministicAnswer({question,selectedSymbol,selectedDetail,scan,eventCatalysts:cachedCatalysts,researchStatus});
      const out=fallback(scan,detected,{
        warning:aiWarning(e),context,eventCatalysts:cachedCatalysts,searched:lastEventResearchAt>0,
        researchStatus,aiConfigured:true,now:now()
      });
