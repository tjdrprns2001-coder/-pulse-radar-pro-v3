...
function normalizeCatalyst(x={}){const tier=String(x.sourceTier||'C').toUpperCase();return{symbol:String(x.symbol||'').toUpperCase().replace(/[^A-Z0-9]/g,''),eventType:String(x.eventType||'other'),titleKo:String(x.titleKo||'이벤트 확인'),summaryKo:String(x.summaryKo||''),eventTime:x.eventTime?String(x.eventTime):null,sourceTier:['A','B','C'].includes(tier)?tier:'C',sourceName:String(x.sourceName||'출처 미상'),sourceUrl:safeUrl(x.sourceUrl),confirmed:Boolean(x.confirmed)}}
function normalizeBrief(raw={},meta={}){
  ...
  return{status:'ok',model:meta.model||raw.model||null,usedWeb:Boolean(meta.usedWeb),summary:String(raw.summary||''),highlights:arr(raw.highlights).slice(0,12),watch:arr(raw.watch).slice(0,12),eventCatalysts,dataWarnings:arr(raw.dataWarnings).slice(0,12),sources};
}
...
function createOpenAIGateway({fetchImpl=global.fetch,apiKey=process.env.GEMINI_API_KEY||'',baseUrl=process.env.GEMINI_BASE_URL||DEFAULT_BASE,model=process.env.GEMINI_MODEL||DEFAULT_MODEL,sleep=(ms)=>new Promise(r=>setTimeout(r,ms))}={}){
  ...
  const system='You are Pulse AI, a cautious crypto market research assistant. Use only supplied scanner context and cited web evidence. Never imply guaranteed returns or execute trades. All user-facing text must be natural Korean. Preserve proper nouns, ticker symbols, numbers, dates, and source URLs accurately. When web search is enabled, research material crypto catalysts for supplied symbols: official exchange notices, listings/delistings, futures changes, token unlocks, swaps/migrations, mainnet/upgrades/hardforks, governance deadlines, airdrops/snapshots, product launches, partnerships, regulatory/ETF/legal developments, and scheduled events. Prefer first-party project or exchange sources. Use sourceTier A only for first-party official evidence, B for reputable media/aggregators citing verifiable facts, C for unconfirmed community claims. Never invent an event or date. Search recent 72-hour news and upcoming 30-day scheduled events. Events alone are not trading signals. Return JSON only, without markdown fences, with summary, highlights, watch, eventCatalysts, dataWarnings, sources. eventCatalysts items must use: symbol, eventType (listing|delisting|futures_listing|futures_delisting|token_unlock|token_swap|migration|mainnet|upgrade|hardfork|governance|airdrop|snapshot|partnership|product_launch|conference|regulatory|etf|lawsuit|exchange_notice|official_news|other), titleKo, summaryKo, eventTime (ISO-8601 or null), sourceTier (A|B|C), sourceName, sourceUrl, confirmed.';
