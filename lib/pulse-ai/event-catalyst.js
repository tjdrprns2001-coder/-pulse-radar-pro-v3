...
const TIER_KO=Object.freeze({A:'공식 확정',B:'신뢰 매체',C:'보조 뉴스·미확인'});
...
function summarizeCatalysts(events=[],searched=true){
  ...
  if(!a.length)return searched?'현재 확인된 이벤트·뉴스 촉매가 없습니다.':'외부 이벤트 검색을 사용할 수 없어 기술 데이터만 표시합니다.';
  const trusted=a.filter(x=>x.sourceTier==='A'||x.sourceTier==='B');
  if(!trusted.length)return `공식·신뢰 이벤트는 확인되지 않았고, 보조 뉴스 ${a.length}건을 참고용으로 표시합니다.`;
  ...
  const aux=a.length-trusted.length;
  return `공식·신뢰 이벤트 ${trusted.length}건을 확인했습니다. 72시간 내 예정 ${soon.length}건, 기술 신호 동반 ${tech.length}건입니다.${aux>0?` 보조 뉴스 ${aux}건도 함께 표시합니다.`:''}${top.length?` 주요: ${top.join(', ')}.`:''}`;
}
