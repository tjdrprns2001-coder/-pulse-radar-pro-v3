...
function enrichCatalysts(events=[],scan=null,now=Date.now()){
  const rows=new Map((scan?.items||[]).map(x=>[baseSymbol(x.symbol),x]));
  return (Array.isArray(events)?events:[]).map((x,i)=>{
    ...
    const row=rows.get(symbol)||null,technicalStateKo=technicalState(row);
    const hoursUntil=eventTime==null?null:(eventTime-now)/3600000;
    ...
    return{
      ...
      sourceTier:tier,sourceTierKo:TIER_KO[tier],sourceName:String(x?.sourceName||'출처 미상'),
      sourceUrl:x?.sourceUrl||null,confirmed:Boolean(x?.confirmed&&tier!=='C'),technicalStateKo,catalystScore,
      scanCategory:row?.category||null
    };
  }).sort((a,b)=>b.catalystScore-a.catalystScore||((a.hoursUntil??1e9)-(b.hoursUntil??1e9)));
}
...
module.exports={TYPE_KO,TIER_KO,baseSymbol,enrichCatalysts,summarizeCatalysts};
