...
function clusterKey(rows){return rows.map(x=>x.sector+':'+x.symbols.slice().sort().join(',')).sort().join('|')}
function detectEvents(current,previous,config={}){
  ...
  const sectorChanged=Boolean(previous)&&clusterKey(sectorClusters)!==clusterKey(prevClusters);
...
module.exports={VERSION,isPre,clusterRows,detectEvents};
