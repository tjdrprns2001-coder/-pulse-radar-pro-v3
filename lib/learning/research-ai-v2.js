...
function statusOf(state){
  ...
  return{
    ...
    research:{archetypeCandidates:archetypes.length,bookRuleRows:stats.bookRules.length,derivedRuleCandidates:stats.derivedRules.length},
...
class ResearchAIv2{
  ...
  exportState(){const research={stats:ResearchStats.summary(this.state.observations),archetypes:ArchetypeLab.buildClusters(this.state.observations)};return JSON.parse(JSON.stringify({...this.state,research,exportedAt:Date.now(),featureNames:Evidence.FEATURE_NAMES}))}
}
let singleton;
