...
function ingestScannerItems(items=[],{source='scanner',marketState=null,asOf=null}={}){
  const ai=defaultResearchAIv2(),normalized=(Array.isArray(items)?items:[]).map(item=>normalizeScannerItem(item,{source,marketState,asOf}));
