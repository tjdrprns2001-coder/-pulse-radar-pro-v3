...
function scannerScore(item={}){
  return n(item.score,n(item.preIgnitionScore,n(item.candidateScore,n(item.tradeSignal?.confidence,0))));
}
...
function direction(v){
  const raw=v&&typeof v==='object'?(v.trend??v.direction??v.bias):v;
...
function normalizeItem(item={}){
  ...
  const qv=n(item.quoteVolume24h,n(item.futuresQuoteVolume24h,n(item.spotQuoteVolume24h,0)));
  const logVol=qv>0?Math.log10(Math.max(1,qv/1e6))/4:0;
  const oi=n(item.flow?.oi4hPct,n(item.oi4hChangePct,0));
  const taker=n(item.flow?.takerRatio,n(item.trueTakerRatio,n(item.takerRatio,1)));
  const funding=n(item.flow?.funding8hPct,n(item.fundingRate,0));
  const regularSetup=Boolean(item.bottomReady||item.pullback||item.breakout||item.samplePattern);
  const setupType=item.setup?.type||(item.breakout?'BREAKOUT_RETEST':item.pullback?'TREND_PULLBACK':item.bottomReady?'SWEEP_RECLAIM':'NONE');
  const rvol1=n(s1.rvol,n(item.v3Rvol?.main1h?.value,n(item.volumeAcceleration1h,n(item.volumeAcceleration,1))));
  const rvol15=n(s15.rvol,n(item.v3Rvol?.ignition15m?.value,n(item.volumeAcceleration15m,1)));
  const structDir=direction(item.structure);
  const dTrend=item.stats?.['1d']?.trend??item.tfState?.['1d']??(structDir>0?'UP':structDir<0?'DOWN':null);
  const h4Trend=item.stats?.['4h']?.trend??item.tfState?.['4h']??(structDir>0?'UP':structDir<0?'DOWN':null);
  const obv=s1.obvUp===true?1:s1.obvUp===false?-1:0;
  const out=[
    clamp(scannerScore(item)/100,0,1),
    clamp(n(item.change24h,n(item.priceChange24h,0))/8),
    clamp(logVol,0,1),
    clamp(oi/10),
    clamp((taker-1)/.8),
    clamp(funding/.05),
    item.setup?.valid||regularSetup?1:0,
    typeCode(setupType),
    direction(dTrend),
    direction(h4Trend),
    clamp((n(s1.rsi,50)-50)/30),
    clamp(Math.log2(Math.max(.25,rvol1))/4),
    clamp(1-n(s1.compression,4)/8,-1,1),
    obv,
    clamp((n(s15.rsi,50)-50)/30),
    clamp(Math.log2(Math.max(.25,rvol15))/4),
    regime(item.regime),
...
function bookEvidence(item={}){
  if(n(item.bookScore)!=null){
    const raw=item.bookEvidence||{};
    const reasons=[...(raw.book?.reasons||[]),...(raw.smartMoney?.reasons||[])].slice(0,8);
    return{score:Math.round(clamp(n(item.bookScore,0),0,100)),matched:reasons.map((why,i)=>({id:'SCANNER_BOOK_'+(i+1),label:'기존 책 합성 엔진',why})),registryVersion:Book.VERSION,source:'scanner-book-evidence'};
  }
  const matched=[];
...
function sampleSimilarity(item={}){
  if(n(item.sampleScore)!=null){
    return{score:Math.round(clamp(n(item.sampleScore,0),0,100)),top:item.samplePattern?[{symbol:String(item.sampleArchetype||item.samplePattern?.archetype||'LIVE_PATTERN'),similarity:clamp(n(item.sampleScore,0)/100,0,1)}]:[],source:'scanner-sample-engine'};
  }
...
function predictionFor(state,item){
  const sample=sampleSimilarity(item),book=bookEvidence(item),base=clamp(scannerScore(item)/100,0,1);
...
function maybeResolveOutcomes(state,item,now=Date.now()){
  const price=itemPrice(item);if(!(price>0))return 0;
  let changed=0;
...
function observeBatch(state,items=[],meta={}){
  ...
  for(const item of items||[]){
    ...
    state.observations.push({
      ...
      scannerState:item.state||item.scanClass?.key||item.dataState||null,scannerScore:scannerScore(item),regime:item.regime||null,
      setupType:item.setup?.type||item.sampleArchetype||item.v2Type||null,features:normalizeItem(item),
