...
function runStress({rows,transitions,config={},multipliers=[1,1.5,2]}={}){
  ...
  for(const m of multipliers){
    ...
    const trades=(replay.closedTrades||[]).map(t=>({
      ...
      entryPrice:t.entryPrice,exitPrice:t.exitPrice,netReturnPct:finite(t.returnFraction)*100,holdingBars:t.exitIndex-t.entryIndex+1,
...
function validateSetupResearch({rows=[],transitions=[],splits,config={},costMultipliers=[1,1.5,2],lockedConfigFingerprint=null}={}){
  ...
  return{
    ...
    lockedOosPolicy:{
      mutable:false,
      configFingerprint:lockedConfigFingerprint||null,
...
module.exports={VERSION,dist,scaleConfig,splitTransitions,signalsWithRisk,setupLatency,metricsFromReplay,runStress,assertLockedSplit,validateSetupResearch};
