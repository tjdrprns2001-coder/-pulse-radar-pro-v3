...
const RejectionAudit=require('./setup-feature-rejection-audit.js');
...
function runCase(input={}){
  ...
  if(!transitions){
    pit=SetupPit.replayPointInTime({
      ...
      defaultDataState:String(input.defaultDataState||'live'),includeSnapshots:input.featureAudit===true
    });
    transitions=pit.transitions;
  }
  const featureAudit=pit&&input.featureAudit===true?RejectionAudit.auditReplay({symbol,snapshots:pit.snapshots||[],splits:input.splits||{}}):null;
...
function runBatchValidation({cases=[],costKeys=['1x','1.5x','2x']}={}){
  ...
  const featureAudit={};
  for(const split of ['development','walkForward','lockedOos'])featureAudit[split]=RejectionAudit.mergeAudits(results.map(x=>x.featureAudit?.splits?.[split]).filter(Boolean));
  const symbolSummary=results.map(x=>({
    ...
    lockedOosStructureTriggers:x.validation.splits.lockedOos.structureCoverage?.summary?.triggerCount??0,
    lockedOosExecutionCoverageRate:x.validation.splits.lockedOos.structureCoverage?.summary?.executionCoverageRate??null,
    lockedOosTriggerConfirmConversion:x.validation.splits.lockedOos.structureCoverage?.summary?.confirmConversionRate??null
  }));
  return{
    version:VERSION,
