...
const {simulatePortfolio}=require('./portfolio-simulator.js');
const {buyHold,emaCrossBaseline,regimePerformance}=require('./baselines.js');
...
function runPresetBacktest({rows,presetId,params={},marketType='spot',costAssumptions={},exitPolicy={},startIndex=0,endIndex=null,costMultipliers=[1,1.5,2]}={}){
  ...
  const base=stress['1x'];
  const portfolio=base?simulatePortfolio(base.trades,{initialEquity:10000,maxOpenPositions:4,riskPct:.25,maxWeightPct:10}):null;
  const baselines={buyHold:buyHold(rows,startIndex,endIndex),ema20x60:emaCrossBaseline(rows,{fast:20,slow:60,startIndex,endIndex})};
  const regimes=base?regimePerformance(base.trades,rows):{};
  const robustness=base?robustnessReport({trades:base.trades,variants:[]}):null;
  return{presetId,params,signalCount:signals.length,signals,stress,portfolio,baselines,regimes,robustness,executionPolicy:'NEXT_ELIGIBLE_BAR',lookaheadSafe:true};
}
...
function walkForward({rows,presetId,paramGrid={},marketType='spot',costAssumptions={},exitPolicy={},trainBars=365,validationBars=120,stepBars=120,objectiveMetric='profitFactor'}={}){
  ...
  for(let trainStart=0;trainStart+trainBars+validationBars<=rows.length;trainStart+=stepBars){
    ...
    folds.push({fold:++fold,train:{startIndex:trainStart,endIndex:trainEnd},validation:{startIndex:valStart,endIndex:valEnd},selectedParams:selected.params,trainObjective:selected.score,trainReport:selected.report,trainVariants:ranked.map(x=>({params:x.params,score:x.score,report:x.report})),validationReport:validation.stress['1x'].report,costStress:Object.fromEntries(Object.entries(validation.stress).map(([k,v])=>[k,v.report])),validationTrades:validation.stress['1x'].trades,regimes:validation.regimes,baselines:validation.baselines,portfolio:validation.portfolio});
  }
  ...
  const variants=folds.flatMap(x=>x.trainVariants||[]),allValidationTrades=folds.flatMap(x=>x.validationTrades||[]);
  const robustness=robustnessReport({trades:allValidationTrades,variants,walkForwardResult:{folds},trialCount:paramsList.length});
  return{presetId,objectiveMetric,trainBars,validationBars,stepBars,folds,robustness,summary:{foldCount:folds.length,evaluatedFolds:valid.length,meanProfitFactor:avg('profitFactor'),meanNetReturnPct:avg('meanNetReturnPct'),meanMaxDrawdownPct:avg('maxDrawdownPct'),lookaheadSafe:true}};
}
