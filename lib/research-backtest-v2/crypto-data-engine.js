...
const DEFAULT_SPOT_BASES=['https://data-api.binance.vision','https://api.binance.com','https://api1.binance.com','https://api2.binance.com','https://api3.binance.com'];
...
function createCryptoResearchDataEngine({
  ...
  async function buildHistoricalContextTimeline(symbol,{startTime,endTime,stepMs=900000,warmupHours=30}={}){
    ...
    const safe=async(name,fn)=>{try{return{name,ok:true,value:await fn()}}catch(e){return{name,ok:false,error:String(e?.message||e),value:{rows:[],pages:0}}}};
    const parts=await Promise.all([
      safe('oi',()=>getHistoricalOiRange(s,{period:'1h',startTime:from,endTime:end})),
      safe('taker1h',()=>getHistoricalTakerRange(s,{period:'1h',startTime:from,endTime:end})),
      safe('taker15m',()=>getHistoricalTakerRange(s,{period:'15m',startTime:from,endTime:end})),
      safe('funding',()=>getHistoricalFundingRange(s,{startTime:from,endTime:end})),
      safe('spot',()=>getKlinesRange(s,'15m',{startTime:from,endTime:end}))
    ]);
    const by=Object.fromEntries(parts.map(x=>[x.name,x]));
    const oi=by.oi.value,t1=by.taker1h.value,t15=by.taker15m.value,funding=by.funding.value,spot=by.spot.value;
    const oiRows=oi.rows||[],t1Rows=t1.rows||[],t15Rows=t15.rows||[],fundRows=funding.rows||[],spotRows=spot.rows||[];
    const sourceErrors=Object.fromEntries(parts.filter(x=>!x.ok).map(x=>[x.name,x.error]));
    const timeline=[];
    for(let cutoff=start;cutoff<=end;cutoff+=Math.max(60000,Number(stepMs)||900000)){
      ...
      timeline.push({availableAt:cutoff,derivativesProfile:profile,spot15m:spotRows.filter(r=>(finite(Array.isArray(r)?r[6]:r?.closeTime)||Infinity)<=cutoff).slice(-64),execution:null,intelligence:null,eventRisk:null,data:{stale:false},historicalCoverage:{oi:oiRows.length>0,taker1h:t1Rows.length>0,taker15m:t15Rows.length>0,funding:fundRows.length>0,spotVolume:spotRows.length>0,execution:false}});
    }
    return{symbol:s,startTime:start,endTime:end,timeline,coverage:{oiRows:oiRows.length,taker1hRows:t1Rows.length,taker15mRows:t15Rows.length,fundingRows:fundRows.length,spot15mRows:spotRows.length,executionHistorical:false,fullDerivativesHistorical:Boolean(oiRows.length&&t1Rows.length&&t15Rows.length&&fundRows.length),partialHistorical:true,sourceErrors},pages:{oi:oi.pages||0,taker1h:t1.pages||0,taker15m:t15.pages||0,funding:funding.pages||0,spot:spot.pages||0}};
  }
