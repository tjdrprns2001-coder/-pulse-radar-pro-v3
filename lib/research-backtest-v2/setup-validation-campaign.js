...
function firstUsableContext(timeline=[]){
  for(const x of timeline||[]){
    ...
    if(oi!=null&&t15>=4&&t1>=2&&spot.length>=21)return finite(x.availableAt);
  }
  return null;
}
...
function contextCoverageSummary(context={}){
  const timeline=Array.isArray(context?.timeline)?context.timeline:[],first=firstUsableContext(timeline);
  return{
    firstUsableAt:first,
...
async function runCampaign({
  ...
  if(usable.length<Math.max(1,Number(minimumUsableSymbols)||4))throw new Error('insufficient symbols with causal historical derivatives coverage');
...
module.exports={VERSION,DEFAULT_SYMBOLS,firstUsableContext,splitWindow,buildLock,contextCoverageSummary,prepareCase,runCampaign};
