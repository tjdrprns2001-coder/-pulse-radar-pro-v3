...
const FRAME_LIMITS=Object.freeze({'1w':80,'1d':120,'4h':160,'1h':220,'15m':260});
...
function frameSnapshot(tf,rows,cutoff){
  const allClosed=Transition.confirmedCandles(rows,cutoff),limit=FRAME_LIMITS[tf]||220,candles=allClosed.slice(-limit);
...
function flattenReplayRecord(decision,outcome=null){const h6=outcome?.horizons?.h6||{},h24=outcome?.horizons?.h24||{};return deepFreeze({schema_version:SCHEMA_VERSION,event_id:decision?.event_id||null,symbol:decision?.symbol||null,replay_cutoff_ts:decision?.replay_cutoff_ts??null,selected_tf:decision?.selected_tf||null,stage_label:decision?.stage_label||null,stage_transition:decision?.stage_transition||null,is_stage_transition:Boolean(decision?.is_stage_transition),direction:decision?.direction||null,state_score:finite(decision?.final_state_score),data_coverage:finite(decision?.evidence_coverage),mtf_score:finite(decision?.mtf?.score),mtf_agreement:finite(decision?.mtf?.agreement),local_directional_score:finite(decision?.selected_tf_features?.local_directional_score),local_quality:finite(decision?.selected_tf_features?.local_quality),next_open:finite(outcome?.next_open_price),next_open_ts:finite(outcome?.next_open_ts),outcome_class:outcome?.outcome_class||'PENDING',hit_6h_8pct:outcome?.labels?.Hit_6H_8pct??null,hit_24h_12pct:outcome?.labels?.Hit_24H_12pct??null,h6_return_pct:finite(h6.returnPct),h6_mfe_pct:finite(h6.mfePct),h6_mae_pct:finite(h6.maePct),h6_rr:finite(h6.rr),h24_return_pct:finite(h24.returnPct),h24_mfe_pct:finite(h24.mfePct),h24_mae_pct:finite(h24.maePct),h24_rr:finite(h24.rr),future_candle_seen:Boolean(decision?.provenance?.future_candle_seen),context_not_after_cutoff:decision?.provenance?.context_not_after_cutoff??null})}
function batchSummary({decisions=[],outcomes=[]}={}){
  ...
  const surgeCount=evaluated.filter(x=>x.outcome.outcome_class==='SURGE').length,controlCount=evaluated.filter(x=>x.outcome.outcome_class==='CONTROL').length,classBalanceReady=surgeCount>=20&&controlCount>=20,performanceConclusionAllowed=evaluated.length>=50&&classBalanceReady;
  return deepFreeze({schemaVersion:SCHEMA_VERSION,decisionCount:rows.length,evaluatedCount:evaluated.length,surgeCount,controlCount,classBalanceReady,byStage,performanceConclusionAllowed,note:performanceConclusionAllowed?'최소 표본·양 클래스 수 기준 충족. 그래도 frozen train/validation 무결성 검토 후 해석.':'누수·상태전이 스모크/탐색 검증용. 성능 결론 금지.'});
}
module.exports={
  SCHEMA_VERSION,REPLAY_TFS,FRAME_LIMITS,STAGE_ORDINAL,rowOpenTs,rowCloseTs,maxClosedTs,frameSnapshot,
  validateHistoricalContext,buildReplayDecision,evaluateNextOpenOutcome,replaySequence,flattenReplayRecord,batchSummary
};
