  ...
  for(const cutoff of grid){
    ...
    if(ctx.execution){
      ...
      for(const key of ['bottom','breakout']){
        item.setupFeatures[key]={...(item.setupFeatures[key]||{}),executionPass:false,netRPass:false,hardReject:true,
          evidence:{...(item.setupFeatures[key]?.evidence||{}),pointInTimeContextMissing:'execution'}};
      }
    }
