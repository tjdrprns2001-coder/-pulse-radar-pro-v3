...
function alignCloseTs(ts,grid){
  const n=finite(ts),g=finite(grid);if(n==null||g==null||g<=0)throw new Error('invalid grid timestamp');
  return Math.ceil((n+1)/g)*g-1;
}
function createCollector({provider,store,buildFeatures,screen,now=()=>Date.now(),maxStepsPerRun=24}={}){
  ...
  async function run({runId,manifest,universe,startTs,endTs,validationEndTs}={}){
    ...
    const alignedStart=alignCloseTs(start,grid);
    ...
    const runMeta={runId:id,manifestVersion:manifest.manifestVersion,universeVersion:universe?.universeVersion,universeMode:universe?.universeMode,survivorshipSafe:Boolean(universe?.survivorshipSafe),startTs:alignedStart,requestedStartTs:start,endTs:end,evaluationGridMs:grid,updatedAt:now()};
    await store.putRun(id,runMeta);
    let ts=finite(prior?.nextTs)??alignedStart,processedSteps=0,matchedEvents=0;const errors=[];
    while(ts<=end&&processedSteps<cap){
      const symbols=eligibleSymbolsAt(universe,ts),stepErrors=[];
      for(const symbol of symbols){
        try{
          const frames=await provider.getHistoricalFrames(symbol,{simulatedTs:ts});
          const event=await buildFeatures({symbol,frames,signalCandleCloseTs:ts,manifest,universe,validationEndTs});
          if(screen(event,manifest)===true){if(await store.putEvent(event))matchedEvents++}
        }catch(e){const msg=symbol+':'+ts+':'+String(e?.message||e);stepErrors.push(msg);errors.push(msg)}
      }
      processedSteps++;
      if(stepErrors.length){
        await store.putCheckpoint(id,{runId:id,manifestVersion:manifest.manifestVersion,universeVersion:universe?.universeVersion,nextTs:ts,startTs:alignedStart,endTs:end,evaluationGridMs:grid,complete:false,failedAtTs:ts,updatedAt:now()});
        break;
      }
      ts+=grid;
      await store.putCheckpoint(id,{runId:id,manifestVersion:manifest.manifestVersion,universeVersion:universe?.universeVersion,nextTs:ts,startTs:alignedStart,endTs:end,evaluationGridMs:grid,complete:ts>end,updatedAt:now()});
    }
...
module.exports={alignCloseTs,createCollector};
