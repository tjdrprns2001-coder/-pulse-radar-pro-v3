...
function bottomReason(s={}){
  ...
  if(state==='FAILED_SETUP'||state==='EXPIRED')return'terminal_state_persistence';
...
function diagnose(summary={}){
  const c=summary.counts||{},n=Math.max(1,(summary.decisionCount||0)*2);
  const issues=[];
  ...
  if((c.terminal_state_persistence||0)>0)issues.push({kind:'wiring',reason:'terminal_state_persistence',count:c.terminal_state_persistence});
...
function auditSnapshots({symbol,snapshots=[],window={}}={}){
  ...
  const summary={symbol,window,decisionCount:rows.length,evaluatedSetups:rows.length*2,counts,examples,nearMiss};
...
function mergeAudits(items=[]){
  ...
  return{version:VERSION,decisionCount,evaluatedSetups:decisionCount*2,counts,symbols,diagnosis:Object.entries(diagnosisCounts).map(([key,count])=>{const i=key.indexOf(':');return{kind:key.slice(0,i),reason:key.slice(i+1),count}}).sort((a,b)=>b.count-a.count)};
}
