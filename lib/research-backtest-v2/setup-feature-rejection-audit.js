...
function bottomReason(s={}){
  ...
  if(state==='FAILED_SETUP'||state==='EXPIRED')return'terminal_state_wait';
...
function diagnose(summary={}){
  ...
  if((c.terminal_state_wait||0)>0)issues.push({kind:'state_lifecycle',reason:'terminal_state_wait',count:c.terminal_state_wait});
