...
function createBowl224Runtime({getStore,fetchImpl=globalThis.fetch,now=()=>Date.now()}={}){
 ...
 return{
  ...
  async stats(){const [events,annotations,outcomes]=await Promise.all([store.listEvents(),store.listAnnotations(),store.listOutcomes()]);return buildBowlStats({events,annotations,outcomes,hypothesisEvidence:[]})},
...
module.exports={createBowl224Runtime};