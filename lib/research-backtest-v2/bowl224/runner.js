...
function createBowl224Runner({provider,store,now=()=>Date.now()}={}){
 ...
 async function runBatch({runId,startTs,endTs,symbols=[],hypothesisRegistry=[]}={}){
  if(!runId)throw new Error('runId required');const selection=await discoverFormalSymbols({provider,symbols,hypothesisRegistry,asOfTs:startTs,maxSelected:10});
  const run={runId,orderedCandidates:selection.orderedCandidates,selectedSymbols:selection.selected,selectionShortfall:selection.shortfall,universeLCount:selection.universeLCount,universeNCount:selection.universeNCount,universeCheckedCount:selection.checkedCount,universeDiscoveryExhaustive:selection.universeDiscoveryExhaustive,startTs,endTs,updatedAt:now(),status:'running'};await store.putRun(runId,run);
...
module.exports={selectFormalSymbols,discoverFormalSymbols,collectStandalone1hSignals,createBowl224Runner,eventId};