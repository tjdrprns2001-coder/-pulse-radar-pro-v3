...
const {HYPOTHESIS_SYMBOLS}=require('./hypothesis-evidence.js');
...
async function processSymbol({symbol,startTs,endTs,provider,store}){
 ...
 const baselines=[],signalEvents=primaryEvents.slice();
 for(const e of signalEvents){
  const b=selectBaseline({symbol,signalEventId:e.eventId,signalTs:e.signalCloseTs,candidateTimestamps:hourlyCandidates.map(x=>x.ts),usedTimestamps:used,isValid:ts=>hourly.coverage?.complete===true&&byTs.has(ts)});
...
function createBowl224Runner({provider,store,now=()=>Date.now()}={}){
 ...
 async function runBatch({runId,startTs,endTs,symbols=[],hypothesisRegistry=HYPOTHESIS_SYMBOLS,maxSymbolsPerRun=1}={}){
