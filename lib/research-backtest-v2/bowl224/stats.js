...
function median(a){const x=a.map(finite).filter(v=>v!=null).sort((p,q)=>p-q);if(!x.length)return null;const m=Math.floor(x.length/2);return x.length%2?x[m]:(x[m-1]+x[m])/2}
function buildBowlStats({events=[],annotations=[],outcomes=[],hypothesisEvidence=[]}={}){
 ...
 const cohorts={};for(const cohort of ['3A','3B','3C','3B1H']){
  ...
  const evaluated=rows.filter(r=>r.outcome),labelEvaluated=evaluated.filter(r=>typeof r.outcome?.labels?.Hit_72H_10pct==='boolean');const clean=labelEvaluated.filter(r=>r.outcome.labels.Hit_72H_10pct===true&&finite(r.outcome?.hits?.Hit_72H_10pct?.mae_before_hit_pct)>=-3).length;
  cohorts[cohort]={sampleCount:rows.length,evaluatedCount:evaluated.length,labels:{
   ...
   },clean10pctMae3Share:labelEvaluated.length?clean/labelEvaluated.length:null,mae72Median:median(evaluated.map(r=>r.outcome?.horizons?.h72?.mae_pct)),mfe72Median:median(evaluated.map(r=>r.outcome?.horizons?.h72?.mfe_pct)),timeToHit72MedianMs:median(evaluated.map(r=>r.outcome?.hits?.Hit_72H_10pct?.time_to_hit_ms))};
 }
