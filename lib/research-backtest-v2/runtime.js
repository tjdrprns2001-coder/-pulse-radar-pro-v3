...
const SetupValidationReport=require('./setup-validation-report.js');
...
function createResearchRuntime({getStore,fetchImpl=globalThis.fetch,now=()=>Date.now()}={}){
  ...
  async function runSetupValidationReport(body={}){
    let source=null,sourceReportId=String(body.sourceReportId||'').trim()||null;
    if(body.batchResult&&typeof body.batchResult==='object')source=body.batchResult;
    else if(sourceReportId){
      const saved=await store.getReport(sourceReportId);
      if(!saved)throw new Error('source report not found');
      source=saved.result||saved;
    }
    if(!source)throw new Error('sourceReportId or batchResult required');
    const built=SetupValidationReport.buildReport(source,{generatedAt:now(),sourceReportId});
    const id=String(body.reportId||('setup-validation-report-'+Math.trunc(now())));
    const report={id,type:'setup-validation-report',createdAt:now(),engine:built.version,sourceReportId,json:built.json,markdown:built.markdown};
    await store.putReport(id,report);return report;
  }
  async function runSetupBatchValidation(body={}){
    const tf=String(body.signalTimeframe||'15m').toLowerCase(),startTime=finite(body.startTime),endTime=finite(body.endTime)??now();
    const symbols=(Array.isArray(body.symbols)?body.symbols:[]).map(x=>String(x||'').toUpperCase().replace(/[^A-Z0-9]/g,'')).filter(Boolean);
    let cases=Array.isArray(body.cases)?body.cases.slice():[];
    if(symbols.length){
      if(startTime==null)throw new Error('startTime required for symbol batch');
      const contextBySymbol=body.contextBySymbol&&typeof body.contextBySymbol==='object'?body.contextBySymbol:{};
      const splitTemplate=body.splits;
      for(const symbol of symbols){
        const hist=await provider.getHistoricalFramesRange(symbol,{
          startTime,endTime,
          intervals:Array.isArray(body.intervals)?body.intervals:['1w','3d','1d','12h','4h','1h','15m','5m'],
          warmupBars:Number(body.warmupBars)||240,
          maxBarsPerFrame:Number(body.maxBarsPerFrame)||250000
        });
        let historicalContext=null;
        if(Array.isArray(contextBySymbol[symbol]))historicalContext={timeline:contextBySymbol[symbol],coverage:{provided:true}};
        else if(body.buildHistoricalContext!==false&&typeof provider.buildHistoricalContextTimeline==='function'){
          historicalContext=await provider.buildHistoricalContextTimeline(symbol,{startTime,endTime,stepMs:Number(body.contextStepMs)||900000,warmupHours:Number(body.contextWarmupHours)||30});
        }
        cases.push({
          symbol,frames:hist.frames,rows:hist.frames[tf]||[],signalTimeframe:tf,
          startTs:startTime,endTs:endTime,
          contextTimeline:Array.isArray(historicalContext?.timeline)?historicalContext.timeline:[],
          contextCoverage:historicalContext?.coverage||null,
    ...
    if(!cases.length)throw new Error('cases or symbols required');
    const result=SetupBatch.runBatchValidation({cases,costKeys:Array.isArray(body.costKeys)?body.costKeys:['1x','1.5x','2x']});
    const id=String(body.reportId||('setup-batch-'+tf+'-'+Math.trunc(endTime)));
    const report={
      id,type:'setup-batch-validation',timeframe:tf,engine:result.version,createdAt:now(),
      dataEngine:provider.provenance(),result,lookaheadSafe:true,
      validationPolicy:'MULTI-SYMBOL PAGINATED PIT REPLAY -> DEVELOPMENT/WF/LOCKED OOS -> REGIME AGGREGATION'
    };
    await store.putReport(id,report);return report;
  }
  ...
  return{store,provider,collector,evaluator,paper,status,runCollection,evaluate,runDante,runCausalIct,runSetupValidationReport,runSetupBatchValidation,runSetupValidation,runSetupPointInTime,runSetupReplay,runWalkForward,reports,report,paperList,paperStats,paperOpen,paperMark,paperClose,stats,events,screenEvent,dantePresets:PRESETS};
}
