...
const {buildFeatureSnapshot}=require('./features.js');
...
function createResearchRuntime({getStore,fetchImpl=globalThis.fetch,now=()=>Date.now()}={}){
  ...
  const collector=createCollector({
    ...
    buildFeatures:async(args)=>{
      const scannerItem=await classifyHistorical({symbol:args.symbol,frames:args.frames,simulatedTs:args.signalCandleCloseTs});
      return buildFeatureSnapshot({...args,scannerItem});
    }
  });
...
module.exports={screenEvent,createResearchRuntime};
