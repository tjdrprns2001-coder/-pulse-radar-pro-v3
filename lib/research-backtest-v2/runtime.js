...
function createResearchRuntime({getStore,fetchImpl=globalThis.fetch,now=()=>Date.now()}={}){
  ...
  return{store,provider,collector,evaluator,paper,status,runCollection,evaluate,runDante,runWalkForward,reports,report,paperList,paperStats,paperOpen,paperMark,paperClose,stats,events,screenEvent,dantePresets:PRESETS};
}
