...
function createMemoryResearchStore(){
  const maps={events:new Map(),outcomes:new Map(),runs:new Map(),manifests:new Map(),universes:new Map(),stats:new Map(),checkpoints:new Map(),paperTrades:new Map(),reports:new Map()};
  ...
  return{
    ...
    async putPaperTrade(id,v){if(!id)throw new Error('paper trade id required');maps.paperTrades.set(String(id),clone(v));return true},
    async getPaperTrade(id){return clone(maps.paperTrades.get(String(id))??null)},
    async listPaperTrades(){return [...maps.paperTrades.values()].map(clone)},
    async putReport(id,v){if(!id)throw new Error('report id required');maps.reports.set(String(id),clone(v));return true},
    async getReport(id){return clone(maps.reports.get(String(id))??null)},
    async listReports(){return [...maps.reports.values()].map(clone)}
  };
}
function createBlobResearchStore({getStore}={}){
  ...
  return{
    ...
    async putPaperTrade(id,v){if(!id)throw new Error('paper trade id required');await write(key('paper-trades',id),v);return true},
    async getPaperTrade(id){return read(key('paper-trades',id))},
    async listPaperTrades(){return list('paper-trades')},
    async putReport(id,v){if(!id)throw new Error('report id required');await write(key('reports',id),v);return true},
    async getReport(id){return read(key('reports',id))},
    async listReports(){return list('reports')}
  };
}
