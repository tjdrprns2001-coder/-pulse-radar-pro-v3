const {evaluateTrendlineBreak,classifyTrendlinePattern}=require('./trendline-patterns.js');
...
function candidateLines({candles,swings,atrByIndex,side,asOf,recentPoints=18,minAnchorGap=8,minSpanBars=20,minTouches=3,touchToleranceAtr=.30,minTouchSpacing=3,maxBarsSinceTouch=60,allowBrokenCandidates=false}){
  ...
  for(let a=0;a<pts.length-1;a++)for(let b=a+1;b<pts.length;b++){
    ...
    const integ=lineIntegrity({candles,line:L,side,start:first,end:asOf,atrByIndex});if(integ.breakConfirmations>0&&!allowBrokenCandidates)continue;
...
function buildTrendlines({candles,swings,bias=null,interval='1h',source='canonical_swing_regression_v2_3',asOf}){
  if(!Array.isArray(candles)||candles.length<20)return{source,support:null,resistance:null,secondary:{support:[],resistance:[]},pair:null,pattern:{type:null,confirmed:false,confidence:0},dedup:{support:{raw:0,unique:0},resistance:{raw:0,unique:0}}};
  ...
  const eventSc=candidateLines({candles,swings,atrByIndex,side:'support',asOf:end,...cfg,allowBrokenCandidates:true}),eventRc=candidateLines({candles,swings,atrByIndex,side:'resistance',asOf:end,...cfg,allowBrokenCandidates:true});
  const sd=dedupe(sc,atrNow,end),rd=dedupe(rc,atrNow,end),eventSd=dedupe(eventSc,atrNow,end),eventRd=dedupe(eventRc,atrNow,end),maxDist=maxDistanceByTf[interval]||4.5;
  const decorate=x=>x?{...x,source,scale:'linear-price',confirmedBarIndex:Number(x.availableAt),anchorA:{...x.anchorA,time:candleTime(candles[x.anchorA.barIndex])},anchorB:{...x.anchorB,time:candleTime(candles[x.anchorB.barIndex])},touchPoints:x.touchBars.map((barIndex,i)=>({barIndex,time:candleTime(candles[barIndex]),price:x.touchPrices?.[i]??at(x,barIndex)})).filter(p=>p.time!=null),lastTouchTime:candleTime(candles[x.touchBars.at(-1)]),htfAlignment:'unknown',breakStatus:evaluateTrendlineBreak({candles,line:x,side:x.side,start:x.touchBars.at(-1),end,strongCloseAtr:.5,confirmBars:2})}:null;
  ...
  const sRaw=choosePrimary(sd.kept),rRaw=choosePrimary(rd.kept),support=decorate(sRaw),resistance=decorate(rRaw),pair=pairDiagnostics(support,resistance,atrNow,end),pattern=classifyTrendlinePattern({support,resistance,pair});
  const eventCandidates={
    support:eventSd.kept.slice(0,8).map(decorate),
    resistance:eventRd.kept.slice(0,8).map(decorate)
  };
  return{source,support,resistance,secondary:{support:sd.kept.filter(x=>x!==sRaw&&Math.abs(x.currentDistanceAtr)<=maxDist).slice(0,2).map(decorate),resistance:rd.kept.filter(x=>x!==rRaw&&Math.abs(x.currentDistanceAtr)<=maxDist).slice(0,2).map(decorate)},eventCandidates,pair,pattern,dedup:{support:{raw:sd.rawCount,unique:sd.uniqueCount},resistance:{raw:rd.rawCount,unique:rd.uniqueCount},eventSupport:{raw:eventSd.rawCount,unique:eventSd.uniqueCount},eventResistance:{raw:eventRd.rawCount,unique:eventRd.uniqueCount}},parameters:{recentConfirmedPoints:18,minAnchorGapBars:8,minSpanBars:cfg.minSpanBars,minTouches:3,minTouchSpacingBars:3,touchToleranceAtr:.30,wickBreachAtr:.30,strongCloseBreachAtr:.50,breakConfirmBars:2,maxBarsSinceLastTouch:cfg.maxBarsSinceTouch,maxCurrentDistanceAtr:maxDist,regression:true,atrNormalizedSlope:true,timeAnchors:true,barIndexAnchors:true,eventCandidatesIncludeBroken:true,scale:'linear-price',patternClassifier:'trendline_patterns_v2'}};
}

module.exports={fitLine,lineIntegrity,candidateLines,dedupe,pairDiagnostics,buildTrendlines,evaluateTrendlineBreak,classifyTrendlinePattern};
