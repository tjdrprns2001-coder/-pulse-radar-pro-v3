...
function candidateLines({candles,swings,atrByIndex,side,asOf,recentPoints=16,minAnchorGap=8,minSpanBars=20,minTouches=3,touchToleranceAtr=.30,minTouchSpacing=3,maxBarsSinceTouch=60}){
  ...
  for(let a=0;a<pts.length-1;a++)for(let b=a+1;b<pts.length;b++){
    ...
    if(integ.breakConfirmations>0)continue;
    ...
    const touchScore=Math.min(25,touches.length*7),fitScore=regressionScore*.20,integrityScore=integ.score*.25,recencyScore=Math.max(0,10-(asOf-lastTouch)*.2),spanScore=Math.min(20,spanBars*.35);
...
function pairDiagnostics(support,resistance,atrNow,asOf){
  ...
  const denom=resistance.slope-support.slope,slopeGapAtr=Math.abs(resistance.normalizedSlope-support.normalizedSlope),parallelScore=Math.round(clamp(100-slopeGapAtr*180));
...
function buildTrendlines({candles,swings,bias=null,interval='1h',source='canonical_swing_regression_v2_1',asOf}){
  ...
  const minSpanByTf={'15m':32,'1h':28,'4h':24,'1d':16},maxAgeByTf={'15m':48,'1h':60,'4h':72,'1d':90};
  const cfg={minSpanBars:minSpanByTf[interval]||24,maxBarsSinceTouch:maxAgeByTf[interval]||60};
  ...
  const decorate=x=>x?{...x,source,anchorA:{...x.anchorA,time:candles[x.anchorA.barIndex]?.time},anchorB:{...x.anchorB,time:candles[x.anchorB.barIndex]?.time},touchPoints:x.touchBars.map((barIndex,i)=>({barIndex,time:candles[barIndex]?.time,price:x.touchPrices?.[i]??at(x,barIndex)})),lastTouchTime:candles[x.touchBars.at(-1)]?.time,htfAlignment:'unknown'}:null;
  const choosePrimary=arr=>arr.find(x=>Math.abs(x.currentDistanceAtr)<=6&&x.totalScore>=60)||arr.find(x=>Math.abs(x.currentDistanceAtr)<=8)||arr[0]||null;
  const support=decorate(choosePrimary(sd.kept)),resistance=decorate(choosePrimary(rd.kept)),pair=pairDiagnostics(support,resistance,atrNow,end);
  return{source,support,resistance,secondary:{support:sd.kept.filter(x=>x!==support).slice(0,2).map(decorate),resistance:rd.kept.filter(x=>x!==resistance).slice(0,2).map(decorate)},pair,dedup:{support:{raw:sd.rawCount,unique:sd.uniqueCount},resistance:{raw:rd.rawCount,unique:rd.uniqueCount}},parameters:{recentConfirmedPoints:16,minAnchorGapBars:8,minSpanBars:cfg.minSpanBars,minTouches:3,minTouchSpacingBars:3,touchToleranceAtr:.30,wickBreachAtr:.30,strongCloseBreachAtr:.50,breakConfirmBars:2,maxBarsSinceLastTouch:cfg.maxBarsSinceTouch,regression:true,atrNormalizedSlope:true,timeAnchors:true}};
}
