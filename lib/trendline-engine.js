...
function buildTrendlines({candles,swings,bias=null,interval='1h',source='canonical_swing_regression_v2_3',asOf}){
  ...
  return{source,support,resistance,secondary:{support:sd.kept.filter(x=>x!==sRaw&&Math.abs(x.currentDistanceAtr)<=maxDist).slice(0,2).map(decorate),resistance:rd.kept.filter(x=>x!==rRaw&&Math.abs(x.currentDistanceAtr)<=maxDist).slice(0,2).map(decorate)},pair,pattern,dedup:{support:{raw:sd.rawCount,unique:sd.uniqueCount},resistance:{raw:rd.rawCount,unique:rd.uniqueCount}},parameters:{recentConfirmedPoints:18,minAnchorGapBars:8,minSpanBars:cfg.minSpanBars,minTouches:3,minTouchSpacingBars:3,touchToleranceAtr:.30,wickBreachAtr:.30,strongCloseBreachAtr:.50,breakConfirmBars:2,maxBarsSinceLastTouch:cfg.maxBarsSinceTouch,maxCurrentDistanceAtr:maxDist,regression:true,atrNormalizedSlope:true,timeAnchors:true,patternClassifier:'trendline_patterns_v2'}};
}
