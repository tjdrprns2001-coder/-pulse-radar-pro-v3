...
const SOURCES=new Set(['live','backfill']);
...
function snapshotId({symbol,scanClassKey,capturedAt,source='live'}={}){const base=`${String(symbol||'').toUpperCase()}:${String(scanClassKey||'')}:${bucketStart(capturedAt)}`;return String(source||'live')==='backfill'?`backfill:${base}`:base}
...
function buildSnapshot(input={}){
  ...
  const capturedAt=finite(input.capturedAt);
  const entryPrice=finite(input.entryPrice??input.lastPrice);
  const source=SOURCES.has(String(input.source||'live'))?String(input.source||'live'):'live';
  if(capturedAt==null)throw new Error('capturedAt must be a finite UTC millisecond timestamp');
  if(entryPrice==null||entryPrice<=0)throw new Error('entryPrice must be a finite positive number');
  const metaEvidence=input.metaEvidence&&typeof input.metaEvidence==='object'?JSON.parse(JSON.stringify(input.metaEvidence)):null;
  const out={id:snapshotId({symbol:input.symbol,scanClassKey,capturedAt,source}),source,symbol:String(input.symbol||'').toUpperCase(),scanClassKey,scanClassLabel:String(input.scanClassLabel||input.scanClass?.label||scanClassKey),capturedAt,entryPrice,tradeSignal:input.tradeSignal?{level:String(input.tradeSignal.level||''),confidence:finite(input.tradeSignal.confidence)}:null,candidateScore:finite(input.candidateScore),sector:String(input.sector||'기타'),priceChange24h:finite(input.priceChange24h),quoteVolume24h:finite(input.quoteVolume24h),volumeAcceleration:finite(input.volumeAcceleration),volumeAcceleration4h:finite(input.volumeAcceleration4h),volumeAcceleration1h:finite(input.volumeAcceleration1h),volumeAcceleration15m:finite(input.volumeAcceleration15m),takerRatio:finite(input.takerRatio),momentumSignals:input.momentumSignals?JSON.parse(JSON.stringify(input.momentumSignals)):null,metaEvidence,reasons:Array.isArray(input.reasons)?input.reasons.slice(0,8).map(String):[],targets:targetTimestamps(capturedAt),version:String(input.version||'signal-performance-v2')};
...
module.exports={ELIGIBLE,HORIZONS,SOURCES,isEligibleClass,bucketStart,snapshotId,targetTimestamps,returnPct,buildSnapshot,aggregate};
