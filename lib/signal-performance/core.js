...
function targetTimestamps(ts){const n=finite(ts);return Object.fromEntries(Object.entries(HORIZONS).map(([k,ms])=>[k,n+ms]))}
function returnPct(entry,future){const a=finite(entry),b=finite(future);return a&&b!=null?((b/a)-1)*100:null}
...
function aggregate(records=[],minSamples=30){
  ...
  const pendingCount=arr.filter(x=>!x||x.status!=='evaluated').length;const sorted=[...evals].sort((a,b)=>a-b);const n=evals.length;
  const mean=n?evals.reduce((a,b)=>a+b,0)/n:null;const median=n?(n%2?sorted[(n-1)/2]:(sorted[n/2-1]+sorted[n/2])/2):null;
  return{sampleCount:arr.length,evaluatedCount:n,pendingCount,positiveRatio:n?evals.filter(x=>x>0).length/n:null,meanReturnPct:mean,medianReturnPct:median,bestReturnPct:n?sorted[n-1]:null,worstReturnPct:n?sorted[0]:null,sampleState:n>=minSamples?'통계 사용 가능':'표본 부족'};
}
