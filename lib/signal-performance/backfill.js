...
function rowTs(row){return finite(Array.isArray(row)?row[0]:row?.openTime??row?.time??row?.timestamp)}
function trimFrames(frames,simulatedTs){const out={};for(const [tf,rows] of Object.entries(frames||{}))out[tf]=(Array.isArray(rows)?rows:[]).filter(row=>{const t=rowTs(row);return t!=null&&t<=simulatedTs});return out}
