...
function createVerifiedNewsProvider({fetchImpl=globalThis.fetch,env=process.env,now=()=>Date.now()}={}){
  async function binance(){
    try{const r=await fetchImpl('https://www.binance.com/en/support/announcement',{headers:{'user-agent':'PulseRadar-selector-r0.4'}});if(!r.ok)throw new Error('HTTP '+r.status);const items=parseBinanceHtml(await r.text(),now());return{available:items.length>0,provider:'Binance Official',items,status:items.length?'FRESH':'DEGRADED',reason:items.length?null:'NO_PARSEABLE_ITEMS'}}catch(e){return{available:false,provider:'Binance Official',items:[],status:'DEGRADED',reason:String(e?.message||e)}}}
