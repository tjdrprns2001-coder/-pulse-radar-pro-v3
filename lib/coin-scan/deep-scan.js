...
function analyzeDeep(input={}){
  ...
  const preSurge=PreSurge.evaluate({
    ...
    fundingPct,
    volumeAcceleration,
    alertState:String(input.alertState||'OBSERVE').toUpperCase()
  });
  const reasons=[];
  ...
  return{
    symbol:String(input.symbol||'').toUpperCase(),dataState:String(input.dataState||'unknown').toLowerCase(),structure,momentum,
