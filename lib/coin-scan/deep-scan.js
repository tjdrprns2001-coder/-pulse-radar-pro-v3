...
const SamplingV3=require('./sampling-v3.js');
const SamplingIntegrity=require('./sampling-v3-integrity.js');
const SamplingAltBars=require('./sampling-v3-alt-bars.js');
const V2=require('./v2-flow-scanner.js');
const V3=require('./v3-scan-engine.js');
const Bottom=require('./bottom-detector.js');
const SetupFeatures=require('./setup-feature-engine.js');
const Strategy=require('./strategy-pattern-engine.js');
const BookEvidence=require('./book-evidence.js');
...
function analyzeDeep(input={}){
  ...
  let bookEvidence=null;try{bookEvidence=BookEvidence.analyze(frames)}catch(_e){bookEvidence=null}
  const v2Input={symbol:String(input.symbol||'').toUpperCase(),timestamp:Date.now(),priceClose:close(lastCompleted(frames['1h'])),price1hPct:priceChange1h,price24hPct:finite(input.priceChange24hPct),atr1h:atrLast(frames['1h']),
    ...
    cleanRebuild:String(samplePattern?.flowType?.key||'')==='CLEAN_REBUILD',absorptionCandidate:String(samplePattern?.flowType?.key||'')==='ABSORPTION',overheated:Boolean(momentumSignals.overheated),distributionRisk:Boolean(s1.bslSweepFail),sampleDnaScore:finite(samplePattern?.score),
    bookBias:bookEvidence?.book?.bias||'중립',bookScore:finite(bookEvidence?.book?.score),bookState:bookEvidence?.book?.state||'관찰',bookHtfAligned:bookEvidence?.book?.htfAligned||'혼조',bookZoneCount:finite(bookEvidence?.book?.zoneCount),bookTrendlineState:bookEvidence?.book?.trendlineState||null,
    smcBias:bookEvidence?.smc?.bias||'중립',smcScore:finite(bookEvidence?.smc?.score),ictBias:bookEvidence?.ict?.bias||'중립',ictScore:finite(bookEvidence?.ict?.score),ictState:bookEvidence?.ict?.state||'none',smartMoneyBias:bookEvidence?.smartMoney?.bias||'중립',smartMoneyScore:finite(bookEvidence?.smartMoney?.score),smartMoneyAligned:Boolean(bookEvidence?.smartMoney?.aligned)};
  const v2RawFlow=V2.evaluate(v2Input);
  const effectiveType=V3.effectiveType(v2RawFlow,v3);
  const v3Final={...v3,effectiveType,stage:V3.stageFor(effectiveType,v3),outputLine:V3.formatOutput({symbol:String(input.symbol||'').toUpperCase(),type:effectiveType,v3})};
  const v2Flow={...v2RawFlow,type:effectiveType,v3Gate:v3.longTerm.filter.tier,v3RawType:v2RawFlow.rawType};
  const sampleSimilarityV2=SampleSimilarityV2.evaluate({
    ...v2Input,
    sampleSubtype:v2Flow.sampleSubtype,
    dnaPrimary:v2Flow.dnaPrimary,
    dnaTags:v2Flow.dnaTags,
    dnaStage:v2Flow.dnaStage,
    stageLabel:v2Flow.stageLabel,
    probeRvol:finite(samplePattern?.volumeShockMemory?.maxRvol),
    probeAgeH:finite(samplePattern?.volumeShockMemory?.ageHours),
    spotConfirmed:input.spotConfirmed,
    basisPct:finite(input.basisPct)
  });
  v2Flow.sampleSimilarityV2=sampleSimilarityV2;
  const v2Csv=V2.csvRow(v2Input,v2Flow,null);
  const strategyCycle=Strategy.analyze({frames,priceChange24h:finite(input.priceChange24hPct),priceChange1h,priceChange15m,structure,momentumSignals,takerRatio,oiChangePct});
  const bottomFinder=Bottom.evaluate({frames,derivativesProfile:input.derivativesProfile||null,v2Profile:v2p});
  const setupFeatures=SetupFeatures.build({frames,derivativesProfile:input.derivativesProfile||null,v2Profile:v2p,execution:input.setupExecutionContext||null,data:{stale:String(input.dataState||'').toLowerCase()!=='live'},eventRisk:input.eventRisk||null});
  const samplingAsOf=finite(input.asOf)??Date.now();
  const samplingV3=SamplingV3.buildSamplingV3({
    rows5m:frames['5m']||[],rows15m:frames['15m']||[],asOf:samplingAsOf,
    derivatives:{oi1hPct:v2Input.oi1hPct,oi4hPct:v2Input.oi4hPct,taker15m:v2Input.taker15m?.at?.(-1)??null,taker5m:finite(v3?.taker?.latest)}
  });
  samplingV3.integrity=SamplingIntegrity.auditFrameSet(frames,samplingAsOf);
  const research=input.samplingResearch&&typeof input.samplingResearch==='object'?input.samplingResearch:null;
  if(research){
    samplingV3.nativeSyntheticAudit=SamplingIntegrity.auditNativeVsSynthetic({
      rows1m:research.rows1m||[],native5m:frames['5m']||[],native15m:frames['15m']||[],asOf:samplingAsOf
    });
    samplingV3.alternativeBars=SamplingAltBars.summarize(research.aggTrades||[]);
    samplingV3.microstructure={status:research.status||'UNAVAILABLE',oneMinuteRows:(research.rows1m||[]).length,aggTradeCount:(research.aggTrades||[]).length,source:research.source||null};
  }else{
    samplingV3.nativeSyntheticAudit={status:'NOT_REQUESTED'};
    samplingV3.alternativeBars={version:SamplingAltBars.VERSION,revision:'3.2',status:'NOT_REQUESTED',tradeCount:0,evaluationTradeCount:0,policy:{thresholdMode:'FROZEN_CALIBRATION',overshootPolicy:'INCLUDE_FULL_TRADE',partialBarPolicy:'DROP_INCOMPLETE_TAIL'},bars:{tickCount:0,volumeCount:0,dollarCount:0,imbalanceCount:0,runCount:0}};
    samplingV3.microstructure={status:'NOT_REQUESTED',oneMinuteRows:0,aggTradeCount:0};
  }
  const reasons=[];
  ...
  if(bookEvidence?.book?.available)reasons.push(`책 합성 ${bookEvidence.book.bias} · ${bookEvidence.book.score}/100 · HTF ${bookEvidence.book.htfAligned}`);
  if(bookEvidence?.smartMoney?.bias&&bookEvidence.smartMoney.bias!=='중립')reasons.push(`SMC/ICT ${bookEvidence.smartMoney.bias} · ${bookEvidence.smartMoney.score}/100`);
  if(v2Flow?.fusion?.eligible)reasons.push(`통합 PRE-SURGE 근거 ${v2Flow.fusion.score}/100 · 책+DNA+수급+SMC/ICT 동시확인`);
  if(sampleSimilarityV2?.successTop5?.[0])reasons.push(`성공샘플 ${sampleSimilarityV2.successTop5[0].symbol} ${sampleSimilarityV2.successTop5[0].score}점 · ${sampleSimilarityV2.ignitionPath}`);
  if(sampleSimilarityV2?.negativeTop3?.[0]?.score>=65)reasons.push(`대조군 ${sampleSimilarityV2.negativeTop3[0].symbol} ${sampleSimilarityV2.negativeTop3[0].score}점 · 충돌검증 필요`);
  if(strategyCycle?.reentryEligible)reasons.push(`재상승 준비 ${strategyCycle.reentryScore}점 · ${strategyCycle.best?.label||'다중 패턴'}`);
  else if(strategyCycle?.patternEligible)reasons.push(`패턴 셋업 ${strategyCycle.reentryScore}점 · ${strategyCycle.best?.label||'다중 패턴'}`);
  if(bottomFinder?.watch)reasons.push(`바닥탐지 ${bottomFinder.label} · ${bottomFinder.score}점`);
  return{
    ...
    priceChange1h,priceChange15m,oiChangePct,fundingPct,preSurge,samplePattern,volumeShockMemory:samplePattern.volumeShockMemory,tfState,pullback,reaccumulating,momentumSignals,bookEvidence,
    v2Flow,v2Csv,v2Input,v3:v3Final,sampleSimilarityV2,samplingV3,strategyCycle,bottomFinder,setupFeatures,reasons:reasons.slice(0,10),updatedAt:Date.now()
  };
}
