...
function lastCompleted(rows){const a=Array.isArray(rows)?rows.filter(Array.isArray):[];return a.length>=2?a[a.length-2]:a[a.length-1]||null}
function rvolLast(rows,lookback=20){
  const a=Array.isArray(rows)?rows.filter(Array.isArray):[];if(a.length<lookback+2)return null;
  const complete=a.slice(0,-1),last=volume(complete[complete.length-1]),base=complete.slice(-lookback-1,-1).map(volume).filter(Number.isFinite);
  const m=avg(base);return last!=null&&m>0?last/m:null;
}
function atrLast(rows,period=14){
  const a=(Array.isArray(rows)?rows.filter(Array.isArray):[]).slice(0,-1);if(a.length<period+1)return null;const tr=[];
...
function rangePosition(rows,lookback=60){
  const a=(Array.isArray(rows)?rows.filter(Array.isArray):[]).slice(0,-1).slice(-lookback);if(a.length<5)return null;
...
function sweepState(rows){
  const a=(Array.isArray(rows)?rows.filter(Array.isArray):[]).slice(0,-1);if(a.length<12)return{sslSweepReclaim:false,bslSweepFail:false,bosUp:false,bosDown:false};
...
function maAlignedBullish(rows){
  const a=(Array.isArray(rows)?rows.filter(Array.isArray):[]).slice(0,-1),closes=a.map(close).filter(Number.isFinite);
...
function obvRising(rows,lookback=20){
  const a=(Array.isArray(rows)?rows.filter(Array.isArray):[]).slice(0,-1).slice(-Math.max(3,lookback));
...
function analyzeDeep(input={}){
  ...
  const bookEvidence=BookEvidence.analyze(frames);
  ...
  const v2Input={symbol:String(input.symbol||'').toUpperCase(),timestamp:Date.now(),priceClose:close(lastCompleted(frames['1h'])),price1hPct:priceChange1h,price24hPct:finite(input.priceChange24hPct),atr1h:atrLast(frames['1h']),
    ...
    range1dPct:rangePosition(frames['1d'],60),htfRangePct:rangePosition(frames['1w'],60),rvol4h:rvolLast(frames['4h']),rvol1h:rvolLast(frames['1h']),rvol15m:rvolLast(frames['15m']),rvol5m:rvolLast(frames['5m']),
    rsi1h:finite(momentumSignals.rsi1h),rsi15m:finite(momentumSignals.rsi15m),rsi5m:finite(momentum5m.rsi),macd1hPositive:finite(momentumSignals.macd1h)!=null&&momentumSignals.macd1h>0,maAligned1h,obv1hUp,breakout4h,lowerTfReset:Boolean((finite(momentum5m.rsi)!=null&&momentum5m.rsi<=35)||(finite(momentumSignals.rsi15m)!=null&&momentumSignals.rsi15m<=55)),
    ...
    displacement:Boolean((rvolLast(frames['15m'])||0)>=1.5&&Math.abs(priceChange15m||0)>=0.5),rvolReaccel:Boolean((rvolLast(frames['5m'])||0)>=3),volumeShockSeen:Boolean(samplePattern?.volumeShockMemory?.found),priceHoldAfterShock:Boolean(samplePattern?.volumeShockMemory?.found&&samplePattern?.volumeShockMemory?.status!=='BROKEN'),
    ...
    sampleDnaScore:finite(samplePattern?.score),sampleArchetype:String(samplePattern?.archetype||'NEUTRAL'),
  ...
  const v2Flow=V2.evaluate(v2Input);
  ...
  return{
    ...
    v2Flow,v2Csv,v2Input,reasons:reasons.slice(0,10),updatedAt:Date.now()
  };
}
