...
function analyzeDeep(input={}){
  ...
  const v2Input={symbol:String(input.symbol||'').toUpperCase(),timestamp:Date.now(),priceClose:close(lastCompleted(frames['1h'])),price1hPct:priceChange1h,price24hPct:finite(input.priceChange24hPct),atr1h:atrLast(frames['1h']),
    ...
    displacement:Boolean((v3Rvol15m||0)>=1.5&&Math.abs(priceChange15m||0)>=0.5),rvolReaccel:Boolean((v3Rvol5m||0)>=3),volumeShockSeen:Boolean(samplePattern?.volumeShockMemory?.found),priceHoldAfterShock:Boolean(samplePattern?.volumeShockMemory?.found&&samplePattern?.volumeShockMemory?.status!=='BROKEN'),
