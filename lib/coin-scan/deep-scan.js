...
function analyzeDeep(input={}){
  ...
  const v2Input={symbol:String(input.symbol||'').toUpperCase(),timestamp:Date.now(),priceClose:close(lastCompleted(frames['1h'])),price1hPct,price24hPct:finite(input.priceChange24hPct),atr1h:atrLast(frames['1h']),
    oi1hPct:finite(v2p.oi1hPct),oi4hPct:finite(v2p.oi4hPct),oi8hPct:finite(v2p.oi8hPct),oi12hPct:finite(v2p.oi12hPct),oi24hPct:finite(v2p.oi24hPct),oiDrawdownPct:finite(v2p.oiDrawdownPct),
    ...
    range1dPct:rangePosition(frames['1d'],60),htfRangePct:rangePosition(frames['1w'],60),rvol1h:rvolLast(frames['1h']),rvol15m:rvolLast(frames['15m']),rvol5m:rvolLast(frames['5m']),
    structureImproving:structure==='bullish'||structureShift4h,structureShift4h,htfStructureHeld:structure!=='bearish',sslSweepReclaim:Boolean(s1.sslSweepReclaim||s15.sslSweepReclaim),bosUp:Boolean(s1.bosUp||s15.bosUp),breakout,
    displacement:Boolean((rvolLast(frames['15m'])||0)>=1.5&&Math.abs(priceChange15m||0)>=0.5),rvolReaccel:Boolean((rvolLast(frames['5m'])||0)>=3),priceHoldAfterShock:Boolean(samplePattern?.volumeShockMemory?.found&&samplePattern?.volumeShockMemory?.status!=='BROKEN'),
