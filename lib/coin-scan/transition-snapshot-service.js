...
function createTransitionSnapshotService({store,now=()=>Date.now()}={}){
  ...
  async function observe({items=[],framesBySymbol={}}={}){
    ...
    for(const item of Array.isArray(items)?items:[]){
      ...
      const typeChanged=Boolean(prev?.type&&prev.type!==type),stageChanged=Boolean(prev?.stageLabel&&prev.stageLabel!==stageLabel),isTransition=typeChanged||stageChanged;
      ...
      item.isTransitionEvent=isTransition;
