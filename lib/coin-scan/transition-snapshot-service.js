...
const Trendlines=require('../trendline-engine.js');
...
function analyzeFrame(tf,rows){
  ...
  let trendlines={support:null,resistance:null};try{trendlines=Trendlines.buildTrendlines({candles,swings:smc.internalStructure||[],bias:ict?.bias||null,interval:tf,source:'scanner-transition'})}catch(_e){}
  return{candles,smc,liquidity,ict,analysis:{trendlines}};
}
...
function buildBundle({item,frames,previous,detectedAt=Date.now()}={}){
  ...
  for(const tf of TF_ORDER){
    ...
    records.push(Snapshot.buildRecord({
      symbol,tf,candles:analysis.candles,analysis:analysis.analysis,smc:analysis.smc,liquidity:analysis.liquidity,ict:analysis.ict,
...
function createTransitionSnapshotService({store,now=()=>Date.now()}={}){
  ...
  async function observe({items=[],framesBySymbol={}}={}){
    ...
    for(const item of Array.isArray(items)?items:[]){
      ...
      if(item.v2Csv&&typeof item.v2Csv==='object'){item.v2Csv.type_transition=item.typeTransition;item.v2Csv.stage_transition=item.stageTransition;item.v2Csv.is_transition_event=isTransition;item.v2Csv.is_stage_transition_event=stageChanged;item.v2Csv.transition_detected_at=isTransition?detectedAt:null}
      result.observed++;
      if(isTransition){
        result.transitions++;
