...
function buildDirection({taker1h=[],oi4hPct,price1hPct,type}={}){
  ...
  if((type==='A+B'||type==='NFB-SQ')&&improve>=2)return'long';
...
function sampleSubtype(input={}){
  const oi4=finite(input.oi4hPct),rv15=finite(input.rvol15m),rv5=finite(input.rvol5m),p1=finite(input.price1hPct);
  ...
  if(oi4!=null&&oi4>=3&&neutral&&rv15!=null&&rv15>=3&&Boolean(input.priceHoldAfterShock))return'PTB_OI_LED_REIGNITION';
...
function evaluate(input={}){
  ...
  return{version:'v2',paramSet:PARAM_SET,type:incomplete?'미완성':rawType,rawType,stage,stageLabel:stageLabel(stage),direction:incomplete?'none':direction,
    ...
    oiBuild:finite(input.oi4hPct)!=null&&finite(input.oi4hPct)>PARAMS.OI_BUILD_4H_PCT,takerLatest:finite(last(input.taker1h||[])),
