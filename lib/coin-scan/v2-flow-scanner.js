...
function sampleSubtype(input={}){
  ...
  const reaccel=Boolean(input.rvolReaccel)||(rv15!=null&&rv15>=3);
  ...
  if(rv15!=null&&rv15>=3&&Boolean(input.rvolReaccel))return'REIGNITION';
  return null;
}
function avg(values=[]){
...
function detectDnaTags(input={}){
  ...
  const lowerReset=Boolean(input.lowerTfReset)||(rsi15!=null&&rsi15<=55);
  ...
  const volumeReaccel=(rv15!=null&&rv15>=1.5)||(rv1!=null&&rv1>=1.5);
  ...
  const oversold=(rsi1!=null&&rsi1<=32)||(rsi15!=null&&rsi15<=18);
  const capitulationVolume=(rv15!=null&&rv15>=5)||(rv1!=null&&rv1>=3);
...
function evaluate(input={}){
  ...
  const dnaTags=detectDnaTags(input),primaryDna=dnaPrimary(dnaTags),parallelStage=dnaStage(dnaTags);
  return{version:'v2',paramSet:PARAM_SET,type:incomplete?'미완성':rawType,rawType,stage,stageLabel:stageLabel(stage),direction:incomplete?'none':direction,
    deadBand:trueTakerDirection(last(input.taker1h||[]))==='none',sampleSubtype:subtype,dnaTags,dnaPrimary:primaryDna,dnaStage:parallelStage,absorptionPresurge:dnaTags.includes('ABSORPTION_LIQUIDITY_SWEEP'),dataComplete:!incomplete,missing,
...
module.exports={PARAM_SET,PARAMS,trueTakerDirection,buildDirection,classifyType,classifyStage,sampleSubtype,detectDnaTags,dnaPrimary,dnaStage,evaluate,csvRow};
