...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,intervalConcurrency=2,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null,env=process.env,disableSpotRest=false}={}){
  ...
  let futuresBlockedUntil=0,lastFuturesError=null,marketFallbackUsed=false,futuresFallbackUsed=false;const inflight=new Map();
  ...
  async function futuresRequest(path,ttlMs,key){const cached=cache.get(key);if(cached!=null)return cached;if(now()<futuresBlockedUntil)throw lastFuturesError||new Error('Binance Futures temporarily blocked');const flightKey='futures:'+key;if(inflight.has(flightKey))return inflight.get(flightKey);const task=(async()=>{let lastError=null,lastStatus=null;for(const base of fapiBases){try{const res=await fetchImpl(base+path);if(!res||!res.ok){lastStatus=Number(res&&res.status)||null;lastError=new Error(`Futures HTTP ${res&&res.status||'ERR'}`);if([403,418,429,451].includes(lastStatus)){const retryAfter=Number(res?.headers?.get?.('retry-after'));lastFuturesError=lastError;futuresBlockedUntil=now()+(Number.isFinite(retryAfter)&&retryAfter>0?Math.min(3600,retryAfter)*1000:900000);break}continue}const data=await res.json();cache.set(key,data,ttlMs);lastFuturesError=null;futuresBlockedUntil=0;return data}catch(e){lastError=e}}throw lastError||new Error('Binance Futures request failed')})().finally(()=>{if(inflight.get(flightKey)===task)inflight.delete(flightKey)});inflight.set(flightKey,task);return task}
  ...
  async function getFuturesUniverse(){try{return await futuresRequest('/fapi/v1/exchangeInfo',300000,'futures:exchangeInfo')}catch(e){return getBybitUniverse()}}
  ...
  async function getFuturesTickers(){try{return await futuresRequest('/fapi/v1/ticker/24hr',30000,'futures:ticker24hr')}catch(e){return getBybitTickers()}}
  ...
  async function getV2OiProfile(symbol){
    ...
    }catch{try{return await getBybitOiProfile(s)}catch{return{rows:[],oi1hPct:null,oi4hPct:null,oi8hPct:null,oi12hPct:null,oi24hPct:null,oiDrawdownPct:null}}}
  }
  ...
  function resetSourceState(){marketFallbackUsed=false;futuresFallbackUsed=false}
  function getSourceState(){return{marketSource:futuresFallbackUsed?'bybit-futures-fallback':(marketFallbackUsed?'spot-fallback':'binance-futures'),futuresBlockedUntil:futuresBlockedUntil||null,lastFuturesError:lastFuturesError?String(lastFuturesError.message||lastFuturesError):null,futuresFallbackUsed}}
