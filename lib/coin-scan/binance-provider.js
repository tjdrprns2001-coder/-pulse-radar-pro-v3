...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null,env=process.env,disableSpotRest=false}={}){
  ...
  let futuresBlockedUntil=0,lastFuturesError=null,marketFallbackUsed=false;const inflight=new Map();
  ...
  async function scanDeepCandidates(symbols,intervals=['1w','1d','4h','1h','15m','5m'],marketBySymbol={},tickerBySymbol={}){const results={},errors=[],contexts={};let fundingMap=new Map();try{fundingMap=await getFundingMap()}catch{}await mapLimit(symbols,async symbol=>{const frames={},scope=String(marketBySymbol?.[symbol]||'futures');for(const interval of intervals){try{frames[interval]=scope==='spot'?await getSpotKlines(symbol,interval,deepRowsForInterval(interval)):await getKlines(symbol,interval,deepRowsForInterval(interval))}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}}if(Object.keys(frames).length){results[symbol]=frames;const tick=tickerBySymbol?.[symbol]||{},marketPromise=getFastMarketIntelligence(symbol,{spotTicker:tick.spotTicker||null,futuresTicker:tick.futuresTicker||null});if(scope==='spot'){const [marketIntelligence,xoiProfile]=await Promise.all([marketPromise,getCrossOiProfile(symbol)]),v2Profile={rows:[],oi1hPct:null,oi4hPct:null,oi8hPct:null,oi12hPct:null,oi24hPct:null,oiDrawdownPct:null,taker1h:[],taker15m:[],fundingRate:null};contexts[symbol]={oiChangePct:null,fundingPct:null,derivativesProfile:{oiProfile:Sample.deriveOiProfile([]),takerProfile:Sample.deriveTakerProfile([]),xoiProfile,v2Profile},v2Profile,marketScope:'spot',marketIntelligence}}else{const [marketIntelligence,derivatives]=await Promise.all([marketPromise,getDerivativesContext(symbol,fundingMap)]);contexts[symbol]={...derivatives,marketScope:scope,marketIntelligence}}}});return{results,errors,contexts}}
