...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null}={}){
  ...
  async function scanDeepCandidates(symbols,intervals=['1w','1d','4h','1h','15m','5m'],marketBySymbol={}){const results={},errors=[],contexts={};let fundingMap=new Map();try{fundingMap=await getFundingMap()}catch{}await mapLimit(symbols,async symbol=>{const frames={},scope=String(marketBySymbol?.[symbol]||'futures');for(const interval of intervals){try{frames[interval]=scope==='spot'?await getSpotKlines(symbol,interval,deepRowsForInterval(interval)):await getKlines(symbol,interval,deepRowsForInterval(interval))}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}}if(Object.keys(frames).length){results[symbol]=frames;contexts[symbol]=scope==='spot'?{oiChangePct:null,fundingPct:null,derivativesProfile:null,v2Profile:null,marketScope:'spot'}:{...(await getDerivativesContext(symbol,fundingMap)),marketScope:scope}}});return{results,errors,contexts}}
