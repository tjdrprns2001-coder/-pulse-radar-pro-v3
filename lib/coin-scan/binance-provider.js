...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase='https://fapi.binance.com',crossOiProvider=null}={}){
  ...
  async function getDerivativesContext(symbol,fundingMap){
    ...
    const [oiChangePct,oiProfile,takerProfile,xoiProfile]=await Promise.all([getOiChange(s),getOiProfile(s),getTakerProfile(s),getCrossOiProfile(s)]);
    return{oiChangePct,fundingPct,derivativesProfile:{oiProfile,takerProfile,xoiProfile}};
  }
  ...
  return{getUniverse,getTickers,getKlines,getKlinesAt,getHistoricalFrames,getFundingMap,getOiChange,getOiProfile,getTakerProfile,getCrossOiProfile,getDerivativesContext,mapLimit,scanDeepCandidates,cache,concurrency:limit};
}
