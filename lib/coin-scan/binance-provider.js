...
const DEFAULT_BASES=['https://api.binance.com','https://api1.binance.com','https://api2.binance.com','https://api3.binance.com'];
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES}={}){
  ...
  async function scanDeepCandidates(symbols,intervals=['1w','1d','4h','1h','15m','5m']){const results={},errors=[];await mapLimit(symbols,async symbol=>{const frames={};for(const interval of intervals){try{frames[interval]=await getKlines(symbol,interval,120)}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}}if(Object.keys(frames).length)results[symbol]=frames});return{results,errors}}
  return{getUniverse,getTickers,getKlines,mapLimit,scanDeepCandidates,cache,concurrency:limit};
}
