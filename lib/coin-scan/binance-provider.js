...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null}={}){
  ...
  async function getDetailedMarketIntelligence(symbol,{spotTicker=null,futuresTicker=null}={}){
    ...
    const [cex,indicators,dex,news,events,wallets,execution,macro]=await Promise.all([
    ...
    return{symbol:String(symbol||'').toUpperCase(),updatedAt:now(),cex,indicators,dex,news,events:{...events,newsDerived:newsEvents},macroCalendar:macro,wallets,walletVerification,execution,coverage:{cex:Boolean(cex?.available),indicators:Boolean(indicators?.available),dex:Boolean(dex?.available),news:Boolean(news?.available),scheduledEvents:Boolean(events?.available),macroCalendar:Boolean(macro?.available),wallets:Boolean(wallets?.available),execution:Boolean(execution?.spot?.available||execution?.futures?.available)}};
  }
