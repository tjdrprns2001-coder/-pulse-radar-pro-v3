...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null,env=process.env,disableSpotRest=false}={}){
  ...
  async function scanLightCandidates(symbols,intervals=['4h','1h'],rowsPerTf=150){
    const results={},errors=[];const src=Array.from(new Set((symbols||[]).map(s=>String(s||'').toUpperCase()).filter(Boolean)));
    await mapLimit(src,async symbol=>{const frames={};const jobs=(intervals||[]).map(async interval=>{try{frames[interval]=await getFuturesKlines(symbol,interval,rowsPerTf)}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}});await Promise.all(jobs);if(Object.keys(frames).length)results[symbol]=frames});
    return{results,errors,scanned:src.length,intervals:[...(intervals||[])]};
  }
