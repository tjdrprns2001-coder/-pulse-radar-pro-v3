...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,intervalConcurrency=2,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null,env=process.env,disableSpotRest=false,disableFuturesFallback=false}={}){
  ...
  let futuresBlockedUntil=0,lastFuturesError=null,marketFallbackUsed=false,futuresFallbackUsed=false,futuresFallbackSource=null,bybitBlockedUntil=0;const futuresBaseBlockedUntil=new Map(),inflight=new Map();
  const BYBIT_BASE='https://api.bybit.com';
  async function bybitRequest(path,ttlMs,key){
    if(now()<bybitBlockedUntil)throw new Error('Bybit temporarily blocked');
    ...
    const task=(async()=>{const signal=typeof AbortSignal!=='undefined'&&AbortSignal.timeout?AbortSignal.timeout(8000):undefined;const res=await fetchImpl(BYBIT_BASE+path,{headers:{accept:'application/json','user-agent':'PulseRadar-Pro-v5'},signal});if(!res?.ok){if([403,418,429,451].includes(Number(res?.status)))bybitBlockedUntil=now()+15*60*1000;throw new Error('Bybit HTTP '+(res?.status||'ERR'));}const body=await res.json();if(Number(body?.retCode)!==0)throw new Error('Bybit '+String(body?.retMsg||body?.retCode||'error'));cache.set(cacheKey,body,ttlMs);futuresFallbackUsed=true;futuresFallbackSource='bybit';return body})().finally(()=>{if(inflight.get(flightKey)===task)inflight.delete(flightKey)});inflight.set(flightKey,task);return task;
  }
  ...
  function klineTtl(tf){return({'1m':10000,'3m':15000,'5m':20000,'15m':30000,'30m':45000,'1h':60000,'2h':90000,'4h':180000,'6h':240000,'12h':300000,'1d':600000,'3d':900000,'1w':900000})[String(tf||'').toLowerCase()]||60000}
  ...
  async function externalJson(url,ttlMs,key){
    const cacheKey='external:'+key,cached=cache.get(cacheKey);if(cached!=null)return cached;
    const flightKey='external:'+key;if(inflight.has(flightKey))return inflight.get(flightKey);
    const task=(async()=>{const signal=typeof AbortSignal!=='undefined'&&AbortSignal.timeout?AbortSignal.timeout(8000):undefined;const res=await fetchImpl(url,{headers:{accept:'application/json','user-agent':'PulseRadar-Pro-v5'},signal});if(!res?.ok)throw new Error('External HTTP '+(res?.status||'ERR'));const body=await res.json();cache.set(cacheKey,body,ttlMs);return body})().finally(()=>{if(inflight.get(flightKey)===task)inflight.delete(flightKey)});
    inflight.set(flightKey,task);return task;
  }
  function baseAssetOf(symbol){const s=String(symbol||'').toUpperCase();return s.endsWith('USDT')?s.slice(0,-4):s}
  function markRows(rows,source){try{Object.defineProperty(rows,'_source',{value:source,enumerable:false,configurable:true})}catch{}return rows}
  async function getOkxKlines(symbol,interval,rows=120){
    const s=String(symbol||'').toUpperCase(),base=baseAssetOf(s),tf=String(interval||'1h').toLowerCase(),barMap={'1m':'1m','3m':'3m','5m':'5m','15m':'15m','30m':'30m','1h':'1H','2h':'2H','4h':'4H','6h':'6H','12h':'12H','1d':'1D','3d':'3D','1w':'1W'},bar=barMap[tf];
    if(!bar)throw new Error('OKX unsupported interval '+tf);
    const target=Math.max(2,Math.min(tf==='1d'?500:420,Number(rows)||120)),inst=base+'-USDT-SWAP',all=[],seen=new Set();let after=null,pages=0;
    while(all.length<target&&pages<3){
      const limit=Math.min(300,target-all.length),url='https://www.okx.com/api/v5/market/history-candles?instId='+encodeURIComponent(inst)+'&bar='+encodeURIComponent(bar)+'&limit='+limit+(after?'&after='+encodeURIComponent(after):'');
      const body=await externalJson(url,45000,'okx:kline:'+s+':'+tf+':'+limit+':'+(after||'latest'));
      if(String(body?.code??'0')!=='0')throw new Error('OKX '+String(body?.msg||body?.code||'error'));
      const data=Array.isArray(body?.data)?body.data:[];if(!data.length)break;
      for(const x of data){const ot=finite(x?.[0]);if(ot==null||seen.has(ot))continue;seen.add(ot);all.push([ot,x[1],x[2],x[3],x[4],x[6]??x[5],ot+tfMs(tf)-1,x[7]??null,null,null,null])}
      const oldest=Math.min(...data.map(x=>finite(x?.[0])).filter(Number.isFinite));if(!Number.isFinite(oldest))break;after=String(oldest);pages++;if(data.length<limit)break;
    }
    const out=all.sort((a,b)=>Number(a[0])-Number(b[0])).slice(-target);if(!out.length)throw new Error('OKX no kline data');futuresFallbackUsed=true;futuresFallbackSource='okx';return markRows(out,'OKX_SWAP');
  }
  async function getGateKlines(symbol,interval,rows=120){
    const s=String(symbol||'').toUpperCase(),base=baseAssetOf(s),tf=String(interval||'1h').toLowerCase(),map={'5m':'5m','15m':'15m','30m':'30m','1h':'1h','2h':'2h','4h':'4h','1d':'1d'},iv=map[tf];
    if(!iv)throw new Error('Gate unsupported interval '+tf);
    const n=Math.max(2,Math.min(1000,Number(rows)||120)),url='https://api.gateio.ws/api/v4/futures/usdt/candlesticks?contract='+encodeURIComponent(base+'_USDT')+'&interval='+encodeURIComponent(iv)+'&limit='+n;
    const data=await externalJson(url,45000,'gate:kline:'+s+':'+tf+':'+n);const out=(Array.isArray(data)?data:[]).map(x=>{const ot=(finite(x?.t)||0)*1000;return[ot,x?.o,x?.h,x?.l,x?.c,x?.v,ot+tfMs(tf)-1,x?.sum??null,null,null,null]}).sort((a,b)=>Number(a[0])-Number(b[0]));
    if(!out.length)throw new Error('Gate no kline data');futuresFallbackUsed=true;futuresFallbackSource='gate';return markRows(out,'GATE_SWAP');
  }
  async function getFallbackFuturesKlines(symbol,interval,rows=120){
    const errs=[];
    for(const fn of [getBybitKlines,getOkxKlines,getGateKlines]){try{return await fn(symbol,interval,rows)}catch(e){errs.push(String(e?.message||e))}}
    throw new Error('Fallback kline unavailable: '+errs.join(' | '));
  }
  ...
  async function getSpotKlines(symbol,interval,rows=120){if(disableSpotRest)throw new Error('Spot REST disabled');const s=String(symbol||'').toUpperCase(),tf=String(interval||'1h'),n=Math.max(2,Math.min(1500,Number(rows)||120));const ttl=klineTtl(tf);const qs=`?symbol=${encodeURIComponent(s)}&interval=${encodeURIComponent(tf)}&limit=${n}`;return request('/api/v3/klines'+qs,ttl,`spot:klines:${s}:${tf}:${n}`)}
  async function getFuturesKlines(symbol,interval,rows=120){const s=String(symbol||'').toUpperCase(),tf=String(interval||'1h'),n=Math.max(2,Math.min(1500,Number(rows)||120));const ttl=klineTtl(tf);const qs=`?symbol=${encodeURIComponent(s)}&interval=${encodeURIComponent(tf)}&limit=${n}`;try{const out=await futuresRequest('/fapi/v1/klines'+qs,ttl,`futures:klines:${s}:${tf}:${n}`);return markRows(out,'BINANCE_FUTURES')}catch(e){if(disableFuturesFallback)throw e;return getFallbackFuturesKlines(s,tf,n)}}
  async function getFuturesAggTrades(symbol,{limit=1000,startTime=null,endTime=null}={}){
    const s=String(symbol||'').toUpperCase(),n=Math.max(1,Math.min(1000,Number(limit)||1000)),parts=['symbol='+encodeURIComponent(s),'limit='+n];
    const st=finite(startTime),et=finite(endTime);if(st!=null)parts.push('startTime='+Math.floor(st));if(et!=null)parts.push('endTime='+Math.floor(et));
    try{return await futuresRequest('/fapi/v1/aggTrades?'+parts.join('&'),5000,'futures:aggTrades:'+s+':'+n+':'+(st??'')+':'+(et??''))}
    catch{return[]}
  }
  async function getSamplingResearchContext(symbol,{asOf=null,oneMinuteRows=360,aggTradeLimit=1000}={}){
    const s=String(symbol||'').toUpperCase(),end=finite(asOf)??now();
    const [rows1m,aggTrades]=await Promise.all([
      getFuturesKlines(s,'1m',Math.max(60,Math.min(720,Number(oneMinuteRows)||360))).catch(()=>[]),
      getFuturesAggTrades(s,{limit:Math.max(100,Math.min(1000,Number(aggTradeLimit)||1000)),endTime:end}).catch(()=>[])
    ]);
    return{
      version:'SAMPLING_RESEARCH_CONTEXT_v1',symbol:s,asOf:end,
      rows1m:Array.isArray(rows1m)?rows1m:[],
      aggTrades:Array.isArray(aggTrades)?aggTrades:[],
      status:(Array.isArray(rows1m)&&rows1m.length&&Array.isArray(aggTrades)&&aggTrades.length)?'READY':(Array.isArray(rows1m)&&rows1m.length)||(Array.isArray(aggTrades)&&aggTrades.length)?'PARTIAL':'UNAVAILABLE',
      source:{oneMinute:'BINANCE_FUTURES_KLINES',aggTrades:'BINANCE_FUTURES_AGGTRADES'}
    };
  }
  ...
  function deepRowsForInterval(tf){return tf==='1d'?1260:tf==='5m'?300:420}
  ...
  async function getV2OiProfile(symbol){
    ...
    try{
      // One 15m series now serves both legacy OI-profile sampling and v2 1H/4H/8H/12H/24H deltas.
      // This removes the former extra 5m + 15m OI requests without reducing the time-horizon coverage.
      const rows=await futuresRequest('/futures/data/openInterestHist?symbol='+encodeURIComponent(s)+'&period=15m&limit=97',45000,'futures:v2:oi15:'+s);
      const a=Array.isArray(rows)?rows:[];const val=x=>finite(x&&x.sumOpenInterest);const pctBars=n=>a.length>=n+1&&val(a[a.length-1-n])?((val(a[a.length-1])/val(a[a.length-1-n]))-1)*100:null;
      ...
      // Keep v2Profile.rows on the historical 1H cadence because V3 oiPath() interprets four rows as 4H.
      // raw15mRows is retained only for the legacy 8H OI sample profile.
      const hourly=[];for(let i=a.length-1;i>=0;i-=4)hourly.push(a[i]);hourly.reverse();
      return{rows:hourly,raw15mRows:a,oi1hPct:pctBars(4),oi4hPct:pctBars(16),oi8hPct:pctBars(32),oi12hPct:pctBars(48),oi24hPct:pctBars(96),oiDrawdownPct:dd,sourcePeriod:'15m'};
  ...
  async function getDerivativesContext(symbol,fundingMap){
    ...
    // v2 already owns a 15m OI path plus 1h/15m taker series. Reuse the same upstream rows
    // for the legacy sample profiles, keeping the old ~8H OI-profile window without duplicate REST calls.
    const [xoiProfile,v2Profile]=await Promise.all([getCrossOiProfile(s),getV2DerivativesProfile(s,map)]);
    const oiChangePct=finite(v2Profile?.oi1hPct);
    const oiProfile=Sample.deriveOiProfile((v2Profile?.raw15mRows||v2Profile?.rows||[]).slice(-32));
    const takerProfile=Sample.deriveTakerProfile(v2Profile?.taker15m||[]);
