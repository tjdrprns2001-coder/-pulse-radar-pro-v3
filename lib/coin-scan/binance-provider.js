...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null}={}){
  ...
  async function futuresRequest(path,ttlMs,key){const cached=cache.get(key);if(cached!=null)return cached;let lastError=null;for(const base of fapiBases){try{const res=await fetchImpl(base+path);if(!res||!res.ok){lastError=new Error(`Futures HTTP ${res&&res.status||'ERR'}`);continue}const data=await res.json();cache.set(key,data,ttlMs);return data}catch(e){lastError=e}}throw lastError||new Error('Binance Futures request failed')}
