...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase='https://fapi.binance.com'}={}){
  ...
  async function getDerivativesContext(symbol,fundingMap){
    ...
    const oiChangePct=await getOiChange(s);\n    const oiProfile=await getOiProfile(s);\n    const takerProfile=await getTakerProfile(s);\n    return{oiChangePct,fundingPct,derivativesProfile:{oiProfile,takerProfile}};
  }
  ...
  return{getUniverse,getTickers,getKlines,getKlinesAt,getHistoricalFrames,getFundingMap,getOiChange,getOiProfile,getTakerProfile,getDerivativesContext,mapLimit,scanDeepCandidates,cache,concurrency:limit};
}
