...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null}={}){
  ...
  async function getDetailedMarketIntelligence(symbol,{spotTicker=null,futuresTicker=null}={}){
    const [cex,indicators,dex,news,events,wallets]=await Promise.all([
    ...
    const newsEvents=marketIntel.deriveNewsEvents(news),walletVerification=marketIntel.walletVerification(wallets,dex);
    return{symbol:String(symbol||'').toUpperCase(),updatedAt:now(),cex,indicators,dex,news,events:{...events,newsDerived:newsEvents},wallets,walletVerification,coverage:{cex:Boolean(cex?.available),indicators:Boolean(indicators?.available),dex:Boolean(dex?.available),news:Boolean(news?.available),scheduledEvents:Boolean(events?.available),wallets:Boolean(wallets?.available)}};
  }
