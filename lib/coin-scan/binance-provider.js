...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase='https://fapi.binance.com',crossOiProvider=null}={}){
  ...
  async function getHistoricalFrames(symbol,{simulatedTs}={}){const end=finite(simulatedTs);if(end==null)throw new Error('simulatedTs required');const intervals=['1w','3d','1d','12h','4h','1h','15m','5m'];const frames={};for(const tf of intervals)frames[tf]=await getKlinesAt(symbol,tf,{endTime:end+1,rows:tf==='5m'?300:120});return frames}
  ...
  async function getV2TakerSeries(symbol,period='1h',limitRows=12){
    const s=String(symbol||'').toUpperCase();
    try{const rows=await futuresRequest('/futures/data/takerlongshortRatio?symbol='+encodeURIComponent(s)+'&period='+encodeURIComponent(period)+'&limit='+Math.max(3,Math.min(100,Number(limitRows)||12)),45000,'futures:v2:taker:'+s+':'+period+':'+limitRows);return(Array.isArray(rows)?rows:[]).map(x=>({timestamp:finite(x.timestamp),ratio:finite(x.buySellRatio),buyVol:finite(x.buyVol),sellVol:finite(x.sellVol)}))}catch{return[]}
  }
  async function getV2DerivativesProfile(symbol,fundingMap){
    ...
    return{...oi,taker1h,taker15m,fundingRate:map.get(s)??null,trueTakerSource:'Binance futures/data/takerlongshortRatio'};
  }
  ...
  async function scanDeepCandidates(symbols,intervals=['1w','1d','4h','1h','15m','5m']){const results={},errors=[],contexts={};let fundingMap=new Map();try{fundingMap=await getFundingMap()}catch{}await mapLimit(symbols,async symbol=>{const frames={};for(const interval of intervals){try{frames[interval]=await getKlines(symbol,interval,interval==='5m'?300:120)}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}}if(Object.keys(frames).length){results[symbol]=frames;contexts[symbol]=await getDerivativesContext(symbol,fundingMap)}});return{results,errors,contexts}}
  return{getUniverse,getTickers,getKlines,getKlinesAt,getHistoricalFrames,getFundingMap,getFundingInfoMap,getOiChange,getOiProfile,getTakerProfile,getCrossOiProfile,getV2OiProfile,getV2TakerSeries,getV2DerivativesProfile,getDerivativesContext,mapLimit,scanDeepCandidates,cache,concurrency:limit};
}
