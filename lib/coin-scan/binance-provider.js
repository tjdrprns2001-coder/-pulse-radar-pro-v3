...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,intervalConcurrency=2,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null,env=process.env,disableSpotRest=false,disableFuturesFallback=false}={}){
  ...
  let futuresBlockedUntil=0,lastFuturesError=null,marketFallbackUsed=false,futuresFallbackUsed=false,bybitBlockedUntil=0;const futuresBaseBlockedUntil=new Map(),inflight=new Map();
  ...
  async function getOkxKlines(symbol,interval,rows=120){
    ...
    const out=all.sort((a,b)=>Number(a[0])-Number(b[0])).slice(-target);if(!out.length)throw new Error('OKX no kline data');futuresFallbackUsed=true;return markRows(out,'OKX_SWAP');
  }
  async function getGateKlines(symbol,interval,rows=120){
    ...
    if(!out.length)throw new Error('Gate no kline data');futuresFallbackUsed=true;return markRows(out,'GATE_SWAP');
  }
