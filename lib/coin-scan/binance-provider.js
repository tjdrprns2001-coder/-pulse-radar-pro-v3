...
function createBinanceProvider({fetchImpl=globalThis.fetch,now=()=>Date.now(),concurrency=4,cache=createTtlCache({now}),bases=DEFAULT_BASES,futuresBase=DEFAULT_FUTURES_BASES[0],futuresBases=null,crossOiProvider=null}={}){
  ...
  async function getSpotUniverse(){return request('/api/v3/exchangeInfo',300000,'spot:exchangeInfo')}
  async function getFuturesUniverse(){return futuresRequest('/fapi/v1/exchangeInfo',300000,'futures:exchangeInfo')}
  async function getSpotTickers(){return request('/api/v3/ticker/24hr',30000,'spot:ticker24hr')}
  ...
  async function scanDeepCandidates(symbols,intervals=['1w','1d','4h','1h','15m','5m'],marketBySymbol={}){const results={},errors=[],contexts={};let fundingMap=new Map();try{fundingMap=await getFundingMap()}catch{}await mapLimit(symbols,async symbol=>{const frames={},scope=String(marketBySymbol?.[symbol]||'futures');for(const interval of intervals){try{frames[interval]=scope==='spot'?await getSpotKlines(symbol,interval,deepRowsForInterval(interval)):await getKlines(symbol,interval,deepRowsForInterval(interval))}catch(e){errors.push({symbol,interval,error:String(e&&e.message||e)})}}if(Object.keys(frames).length){results[symbol]=frames;if(scope==='spot'){const xoiProfile=await getCrossOiProfile(symbol);const v2Profile={rows:[],oi1hPct:null,oi4hPct:null,oi8hPct:null,oi12hPct:null,oi24hPct:null,oiDrawdownPct:null,taker1h:[],taker15m:[],fundingRate:null};contexts[symbol]={oiChangePct:null,fundingPct:null,derivativesProfile:{oiProfile:Sample.deriveOiProfile([]),takerProfile:Sample.deriveTakerProfile([]),xoiProfile,v2Profile},v2Profile,marketScope:'spot'}}else contexts[symbol]={...(await getDerivativesContext(symbol,fundingMap)),marketScope:scope}}});return{results,errors,contexts}}
