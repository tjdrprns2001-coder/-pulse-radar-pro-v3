...
function buildTickBars(rows=[],opts={}){
  ...
  for(let i=0;i+n<=trades.length;i+=n){
    ...
    bars.push(finalizeBar(bucket,{barType:'TICK_AGGTRADE',logicalType:'TICK',thresholdTrades:n,tickDefinition:'BINANCE_AGGTRADE_EVENT_COUNT',overshootPolicy:'NOT_APPLICABLE'}));
  }
  return bars;
}
...
function summarize(rows=[],opts={}){
  ...
  let evaluation=trades,calibration=null,bars,status;
  if(mode==='FROZEN_CALIBRATION'){
    ...
    if(calibration.status==='READY'){
      ...
      evaluation=[];
      bars={tick:[],volume:[],dollar:[],imbalance:[],run:[]};
      status=calibration.status||'INSUFFICIENT_CALIBRATION';
    }
  }else{
    bars={
    ...
    status=trades.length>=CONFIG.warmupTrades?'READY':'INSUFFICIENT_TRADES';
  }
  ...
  return{
    version:VERSION,revision:'3.2',status,tradeCount:trades.length,evaluationTradeCount:evaluation.length,
    source:'BINANCE_FUTURES_AGGTRADES',sourceSchema:{venue:'BINANCE',market:'USD_M_LINEAR_PERPETUAL',eventType:'AGG_TRADE',eventTimeField:'T',receiveTimeAvailable:false,priceField:'p',quantityField:'q',aggressorRule:'m=true => SELL aggressor; m=false => BUY aggressor'},
    policy:{thresholdMode:mode,thresholdScope:'PER_SNAPSHOT_CAUSAL_SPLIT',overshootPolicy:cfg.overshootPolicy,partialBarPolicy:'DROP_INCOMPLETE_TAIL',tickDefinition:'BINANCE_AGGTRADE_EVENT_COUNT',volumeDefinition:'SUM_EXCHANGE_REPORTED_QTY',notionalDefinition:'PRICE_X_QTY_USDT_LINEAR'},
    calibration,
    bars:c,
