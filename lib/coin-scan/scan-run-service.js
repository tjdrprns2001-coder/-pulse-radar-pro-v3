...
function mergeItems(base=[],next=[]){
  ...
  for(const x of next||[])if(x?.symbol)m.set(x.symbol,x);
  return [...m.values()];
}
...
function createScanRunService({scanService,store,now=()=>Date.now()}={}){
  ...
  async function execute(id){
    ...
    try{
      ...
      for(let i=0;i<candidates.length;i+=DEEP_CHUNK){
        const chunk=candidates.slice(i,i+DEEP_CHUNK);
        const deep=await scanService.run({mode:'deep',limit:chunk.length,symbols:chunk,precision:run.precision,persistObservations:false});
        run.items=mergeItems(run.items,deep.items||[]);
        run.autoScreening=mergeScreening(run.autoScreening,deep.autoScreening||{});
        run.deepDone=Math.min(run.deepTotal,i+chunk.length);
  ...
  return{VERSION,DEEP_CHUNK,start,get,execute};
}
module.exports={VERSION,DEEP_CHUNK,mergeItems,mergeScreening,createScanRunService};
