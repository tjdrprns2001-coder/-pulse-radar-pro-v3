...
function createScanRunService({scanService,store,now=()=>Date.now()}={}){
  ...
  async function start({precision=false}={}){
    const id=idNow(now()),run={version:VERSION,id,status:'QUEUED',stage:'queued',createdAt:now(),updatedAt:now(),precision:Boolean(precision),deepDone:0,deepTotal:0,candidateSymbols:[],items:[],autoScreening:{all:[]},samplingResearch:{requested:0,ready:0,partial:0,unavailable:0},meta:null,error:null};
    return save(run);
  }
    ...
    try{
      ...
      async function applyChunk(entry,deep){
        commit=commit.then(async()=>{
          ...
          run.meta={...(run.meta||{}),...slimMeta(deep),prescan:run.meta?.prescan||null,samplingResearch:run.samplingResearch||null};
