...
function analyze({frames={},strategyCycle=null,bookManual=null,structure='neutral'}={}){
  ...
  const late=timing?.stage==='LATE_ENTRY_WARNING',failed=timing?.stage==='FAILED_RETEST_RESISTANCE'||timing?.stage==='RETEST_RESISTANCE_CHECK';
  ...
  if(late){score-=22;warnings.push('돌파 후 과이격 · 늦은 추격')}if(failed){score-=30;warnings.push('돌파선 지지 실패/저항 전환 위험')}if(middleRisk)score-=6;
  score=Math.round(clamp(score));
  let grade=score>=80?'A':score>=65?'B':score>=45?'C':'D';
  if(late&&grade==='A')grade='B';if(failed)grade='D';if(htf.bias==='bearish'&&grade==='A')grade='B';
  const action=grade==='A'?'확인 진입 후보':grade==='B'?'소액/추가 확인':grade==='C'?'관찰 우선 · 신규 진입 보류':'진입 회피';
  const setup=setupType(strategy),trigger=triggerText(strategy,vol);
  let cancel='핵심 구조 또는 무효화 가격 이탈 시 취소';
  ...
  return{version:VERSION,available:true,grade,score,action,setup,higherTimeframe:htf,entry,trigger,invalidation,target,nextResistance,spaceR,volume:vol,lateEntry:late,failedRetest:failed,positionGood,triggerConfirmed,reasons:reasons.slice(0,6),warnings:warnings.slice(0,6),cancelCondition:cancel};
}
