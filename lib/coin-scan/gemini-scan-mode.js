...
function priceOiDelta(row={}){const oi=finite(row.oi4hPct),p=finite(row.priceChange24h);if(oi==null||p==null)return{ratio:null,label:'MISSING'};const ratio=oi/Math.max(.5,Math.abs(p));let label='BALANCED';if(p<0&&oi>1)label='SHORT_BUILD_RISK';else if(p>3&&oi<0)label='SHORT_COVER_RISK';else if(Math.abs(p)<=3&&p>=0&&oi>=2)label='POSITION_BUILD';return{ratio,label}}
