...
function metrics(rows=[]){
  const selected=rows.filter(x=>x.selected),rets=selected.map(x=>n(x.net_return??x.return)).filter(v=>v!=null),wins=rets.filter(x=>x>0).length;
  let streak=0,worst=0;for(const r of rets){if(r<0){streak++;worst=Math.max(worst,streak)}else streak=0}
  const rejected=rows.filter(x=>!x.selected),falseRejected=rejected.filter(x=>(n(x.net_return??x.return)??0)>0);
  const byRegime={};for(const regime of [...new Set(selected.map(x=>x.regime).filter(Boolean))]){const v=selected.filter(x=>x.regime===regime).map(x=>n(x.net_return??x.return)).filter(x=>x!=null);byRegime[regime]={sampleCount:v.length,expectancy:mean(v),hitRate:v.length?v.filter(x=>x>0).length/v.length:null}}
  return{sampleCount:rets.length,selectedCount:selected.length,expectancy:mean(rets),hitRate:rets.length?wins/rets.length:null,maxDrawdown:maxDrawdown(rets),worstConsecutiveLosses:worst,coverage:rows.length?selected.length/rows.length:null,falseRejectionRate:rejected.length?falseRejected.length/rejected.length:null,byRegime};
}
