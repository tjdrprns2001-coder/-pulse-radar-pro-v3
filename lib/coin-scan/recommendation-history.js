...
function createRecommendationHistoryService({store,resolver=null,now=()=>Date.now(),cooldownMs=1800000,scoreDelta=8,maxEvaluationsPerRun=6}={}){
  ...
  async function stats({state='RECOMMEND'}={}){
    const rows=await list({state,limit:500,includeOutcomes:true});const horizons={};
    for(const key of Object.keys(HORIZONS)){const vals=rows.map(x=>finite(x.outcome?.horizons?.[key]?.returnPct)).filter(v=>v!=null);horizons[key]={evaluatedCount:vals.length,positiveRatio:vals.length?vals.filter(v=>v>0).length/vals.length:null,meanReturnPct:vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null}}
