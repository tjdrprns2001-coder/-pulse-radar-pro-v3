...
const SampleBlind=require('./sample-blind-validation.js');
...
function dataQuality(row={},requiredTfs=MANUS_TIMEFRAMES){
  const hardMissing=[],softMissing=[];
  for(const tf of requiredTfs)if(!row.tf?.[tf]?.available)hardMissing.push(`tf:${tf}`);
  if(finite(row.oi4hPct)==null)hardMissing.push('oi4h');
  if(finite(row.priceChange24h)==null)hardMissing.push('futuresTicker');
  if(!row.takerCross?.['15m']||row.takerCross['15m'].status==='MISSING')softMissing.push('taker15m');
  if(!row.takerCross?.['5m']||row.takerCross['5m'].status==='MISSING')softMissing.push('taker5m');
  if(finite(row.spotPriceChange24h)==null)softMissing.push('spotTicker');
  if(finite(row.fundingRatePct)==null)softMissing.push('funding');
  const conflicts=['15m','5m'].filter(tf=>['CONFLICT','TIME_MISMATCH'].includes(row.takerCross?.[tf]?.status));
  const status=hardMissing.length?'FAIL':conflicts.length?'CONFLICTED':softMissing.length?'PARTIAL':'PASS';
  return{status,missingFields:[...hardMissing,...softMissing],hardMissing,softMissing,conflicts,score:status==='PASS'?5:status==='PARTIAL'?3:status==='CONFLICTED'?2:0};
}
...
function createAstraAutoScanner({provider,now=()=>Date.now()}={}){
  ...
  async function deep(symbols=[],{method=METHODS.ASTRA,market={},asOf=null}={}){
    const sampleApproval=methodOf(method)===METHODS.ASTRA?SampleBlind.productionApproval():null;
    ...
    const items=await safeMapLimit(provider,list,2,async symbol=>{
      const frames={},errors=[];await safeMapLimit(provider,tfs,2,async tf=>{try{const rows=method===METHODS.CLAUDE?420:(method===METHODS.GROK||method===METHODS.GEMINI)?(tf==='5m'?1500:tf==='15m'?520:tf==='1d'?180:150):(tf==='1d'?180:150);frames[tf]=await provider.getFuturesKlines(symbol,tf,rows)}catch(e){errors.push(`${tf}:${String(e?.message||e)}`)}});
      let oiProfile={},t1=[],t15=[],t5=[],okx=null,micro1m=[],aggTrades=[];try{[oiProfile,t1,t15,t5]=await Promise.all([provider.getV2OiProfile(symbol),provider.getV2TakerSeries(symbol,'1h',8),provider.getV2TakerSeries(symbol,'15m',8),provider.getV2TakerSeries(symbol,'5m',8)])}catch(e){errors.push(`derivatives:${String(e?.message||e)}`)}
      if(method===METHODS.ASTRA){
        const micro=await Promise.all([
          typeof provider.getFuturesKlines==='function'?provider.getFuturesKlines(symbol,'1m',360).catch(e=>{errors.push(`1m:${String(e?.message||e)}`);return[]}):Promise.resolve([]),
          typeof provider.getFuturesAggTrades==='function'?provider.getFuturesAggTrades(symbol,{limit:1000,endTime:snapshotAsOf}).catch(e=>{errors.push(`aggTrades:${String(e?.message||e)}`);return[]}):Promise.resolve([])
        ]);
        micro1m=micro[0]||[];aggTrades=micro[1]||[];
      }
      ...
      if(method===METHODS.ASTRA){
        row.samplingV3=SamplingV3.buildSamplingV3({
          rows5m:frames['5m']||[],rows15m:frames['15m']||[],asOf:snapshotAsOf,
          derivatives:{oi1hPct:row.oi1hPct,oi4hPct:row.oi4hPct,taker15m:row.taker15m,taker5m:row.taker5m}
        });
        row.samplingV3.integrity=SamplingIntegrity.auditFrameSet({...frames,'1m':micro1m},snapshotAsOf);
        row.samplingV3.nativeSyntheticAudit=SamplingIntegrity.auditNativeVsSynthetic({
          rows1m:micro1m,native5m:frames['5m']||[],native15m:frames['15m']||[],asOf:snapshotAsOf
        });
        row.samplingV3.alternativeBars=SamplingAltBars.summarize(aggTrades);
        row.samplingV3.microstructure={
          oneMinuteRows:Array.isArray(micro1m)?micro1m.length:0,
          aggTradeCount:Array.isArray(aggTrades)?aggTrades.length:0,
          status:(Array.isArray(micro1m)&&micro1m.length&&Array.isArray(aggTrades)&&aggTrades.length)?'READY':'PARTIAL'
        };
      }
      ...
      if(method===METHODS.ASTRA){
        row.verdict.samplingV3=row.samplingV3||null;
        row.verdict.samplingV3EvidenceScore=row.samplingV3?.evidenceScore??null;
        row.verdict.samplingV3Stage=row.samplingV3?.sequence?.stage||null;
      }
      if(method===METHODS.ASTRA){
        const adj=sampleApproval?.approved?SampleBlind.rankingAdjustment(row.sampleSimilarityV2):{points:0,eligible:false,reason:'VALIDATION_GATE_BLOCKED',policy:SampleBlind.POLICY.version};
        row.sampleRankingAdjustment=adj;
        row.verdict.baseScore=finite(row.verdict.score);
        row.verdict.sampleAdjustment=adj.points;
        row.verdict.samplePolicy=adj.policy;
        row.verdict.sampleValidationPassed=Boolean(sampleApproval?.approved);
        row.verdict.score=Math.round(clamp((finite(row.verdict.baseScore)||0)+(finite(adj.points)||0)));
      }
    ...
    return{status:'ok',stage:'deep',method,version:VERSION,updatedAt:stamp,asOf:snapshotAsOf,timeframes:tfs,requested:list.length,marketState:market,sampleValidation:method===METHODS.ASTRA?sampleApproval:null,items};
  }
