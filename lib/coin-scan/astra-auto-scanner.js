...
const VERSION='ASTRA_AUTO_SCAN_v3';
const METHODS=Object.freeze({ASTRA:'astra',MANUS:'manus',PERPLEXITY:'perplexity'});
...
function createAstraAutoScanner({provider,now=()=>Date.now()}={}){
  ...
  async function oi(symbols=[],{method=METHODS.ASTRA,asOf=null}={}){
    ...
    return{status:'ok',stage:'oi',method,version:VERSION,updatedAt:stamp,asOf:snapshotAsOf,requested:list.length,success,failed:failed.length,missing:missing.length,mismatchedWindow:mismatched.length,successRate,terminalRate,degraded,failureSymbols:failed.map(x=>x.symbol),missingSymbols:missing.map(x=>x.symbol),mismatchedSymbols:mismatched.map(x=>x.symbol),passCount:items.filter(x=>x.pass).length,items};
  }
  async function deep(symbols=[],{method=METHODS.ASTRA,market={},asOf=null}={}){
    method=methodOf(method);const tfs=method===METHODS.MANUS?MANUS_TIMEFRAMES:method===METHODS.PERPLEXITY?PERPLEXITY_TIMEFRAMES:ASTRA_TIMEFRAMES,list=normalizeSymbols(symbols,CONFIG.maxDeepSymbols),stamp=now(),snapshotAsOf=finite(asOf)||stamp;let fundingMap=new Map(),spotMap=new Map(),futuresMap=new Map();
    ...
    const items=await safeMapLimit(provider,list,2,async symbol=>{
      ...
      row.verdict=method===METHODS.MANUS?classifyManus(row,market):method===METHODS.PERPLEXITY?Perplexity.classify(row,{market,frames,okxStatus}):classifyAstra(row);return row;
    });
    const order=method===METHODS.MANUS?{A_FIRE:0,B_PREPARE:1,C_WATCH:2,HOLD:3,EXCLUDE:4}:method===METHODS.PERPLEXITY?{IGNITION_CONFIRMED:0,TRANSITION_WATCH:1,ACCUMULATION_CANDIDATE:2,OI_BUILD:3,INCOMPLETE:4,SHORT_BUILD_RISK:5,OVERHEATED:6,DATA_DEGRADED:7}:{IGNITION_CONFIRMED:0,WATCH_PRIORITY:1,WAIT:2,HOLD:3,PROGRESSED:4};items.sort((a,b)=>(order[a.verdict?.key]??9)-(order[b.verdict?.key]??9)||((b.verdict?.priority??b.verdict?.score)||0)-((a.verdict?.priority??a.verdict?.score)||0));
...
module.exports={VERSION,METHODS,ASTRA_TIMEFRAMES,MANUS_TIMEFRAMES,PERPLEXITY_TIMEFRAMES,CONFIG,MANUS_CONFIG,PERPLEXITY_CONFIG:Perplexity.CONFIG,finite,confirmed,candleMetrics,lastConfirmedTaker,takerCross,buildUniverse,initialFilter,marketState,oiFromProfile,scoreCandidate,classifyAstra,classifyManus,classifyPerplexity:Perplexity.classify,normalizeSymbols,methodOf,createAstraAutoScanner};
