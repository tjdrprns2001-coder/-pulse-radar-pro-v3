...
const VERSION='ASTRA_AUTO_SCAN_v2';
const METHODS=Object.freeze({ASTRA:'astra',MANUS:'manus'});
...
function marketState(rows=[],config=MANUS_CONFIG){
  ...
  return{total,up,down,flat,breadthRatio,median24hChange:median(valid.map(x=>x.priceChange24h)),positiveVolumeRatio,topMoverRatio:total?valid.filter(x=>Math.abs(x.priceChange24h)>=10).length/total:null,btc24hChange:btcPct,eth24hChange:ethPct,regime};
}
...
function methodOf(v){return String(v||'').toLowerCase()===METHODS.MANUS?METHODS.MANUS:METHODS.ASTRA}
function createAstraAutoScanner({provider,now=()=>Date.now()}={}){
  if(!provider)throw new Error('provider required');
  async function universe({method=METHODS.ASTRA}={}){
    method=methodOf(method);const requestedAt=now(),[ex,tickers]=await Promise.all([provider.getFuturesUniverse(),provider.getFuturesTickers()]),receivedAt=now(),rows=buildUniverse(ex,tickers),filtered=initialFilter(rows),asOf=Math.max(...rows.map(x=>finite(x.closeTime)||0),requestedAt),sorted=filtered.slice().sort((a,b)=>(b.quoteVolume24h||0)-(a.quoteVolume24h||0)),mkt=marketState(rows);
    return{status:'ok',stage:'universe',method,version:VERSION,updatedAt:receivedAt,asOf,closedBarPolicy:'close_time < as_of',config:method===METHODS.MANUS?{...CONFIG,...MANUS_CONFIG}:CONFIG,requestMeta:{requestedAt,receivedAt},universeCount:rows.length,filteredCount:sorted.length,pipelineCounts:{universe:rows.length,tickerPass:sorted.length},breadth:mkt,marketState:mkt,majors:Object.fromEntries(['BTCUSDT','ETHUSDT'].map(s=>[s,rows.find(x=>x.symbol===s)||null])),items:sorted.map(({ticker,...x})=>x)};
  }
  async function oi(symbols=[],{method=METHODS.ASTRA}={}){
    method=methodOf(method);const list=normalizeSymbols(symbols,CONFIG.maxOiSymbols),stamp=now(),items=await safeMapLimit(provider,list,6,async symbol=>{try{const p=await provider.getV2OiProfile(symbol),o=oiFromProfile(p),missing=o.oi4hPct==null;return{symbol,...o,pass:!missing&&o.oi4hPct>=CONFIG.minOi4hPct,status:missing?'MISSING':'SUCCESS',error:null}}catch(e){return{symbol,oi1hPct:null,oi4hPct:null,oi8hPct:null,oi12hPct:null,oi24hPct:null,pass:false,status:'FAILED',error:String(e?.message||e)}}});
    const failed=items.filter(x=>x.status==='FAILED'),missing=items.filter(x=>x.status==='MISSING');return{status:'ok',stage:'oi',method,version:VERSION,updatedAt:stamp,requested:list.length,success:items.length-failed.length-missing.length,failed:failed.length,missing:missing.length,failureSymbols:failed.map(x=>x.symbol),missingSymbols:missing.map(x=>x.symbol),passCount:items.filter(x=>x.pass).length,items};
  }
  async function deep(symbols=[],{method=METHODS.ASTRA,market={},asOf=null}={}){
    method=methodOf(method);const tfs=method===METHODS.MANUS?MANUS_TIMEFRAMES:ASTRA_TIMEFRAMES,list=normalizeSymbols(symbols,CONFIG.maxDeepSymbols),stamp=now(),snapshotAsOf=finite(asOf)||stamp;let fundingMap=new Map(),spotMap=new Map(),futuresMap=new Map();
    ...
    const items=await safeMapLimit(provider,list,2,async symbol=>{
      const frames={},errors=[];await safeMapLimit(provider,tfs,3,async tf=>{try{frames[tf]=await provider.getFuturesKlines(symbol,tf,tf==='1d'?180:150)}catch(e){errors.push(`${tf}:${String(e?.message||e)}`)}});
      let oiProfile={},t15=[],t5=[];try{[oiProfile,t15,t5]=await Promise.all([provider.getV2OiProfile(symbol),provider.getV2TakerSeries(symbol,'15m',8),provider.getV2TakerSeries(symbol,'5m',8)])}catch(e){errors.push(`derivatives:${String(e?.message||e)}`)}
      const tf=Object.fromEntries(tfs.map(x=>[x,candleMetrics(frames[x]||[],snapshotAsOf)])),tak15=lastConfirmedTaker(t15,'15m',snapshotAsOf),tak5=lastConfirmedTaker(t5,'5m',snapshotAsOf),oiPack=oiFromProfile(oiProfile),ft=futuresMap.get(symbol)||{},st=spotMap.get(symbol)||{};
      const row={symbol,method,asOf:snapshotAsOf,lastPrice:finite(ft.lastPrice)??finite(tf['5m']?.close),priceChange24h:finite(ft.priceChangePercent),quoteVolume24h:finite(ft.quoteVolume),spotPriceChange24h:finite(st.priceChangePercent),fundingRatePct:finite(fundingMap.get(symbol)),...oiPack,taker15m:finite(tak15?.ratio),taker5m:finite(tak5?.ratio),taker15mAt:finite(tak15?.timestamp),taker5mAt:finite(tak5?.timestamp),tf,errors};
      row.takerCross={'15m':takerCross(tf['15m'],tak15,'15m'),'5m':takerCross(tf['5m'],tak5,'5m')};row.verdict=method===METHODS.MANUS?classifyManus(row,market):classifyAstra(row);return row;
    });
    const order=method===METHODS.MANUS?{A_FIRE:0,B_PREPARE:1,C_WATCH:2,HOLD:3,EXCLUDE:4}:{IGNITION_CONFIRMED:0,WATCH_PRIORITY:1,WAIT:2,HOLD:3,PROGRESSED:4};items.sort((a,b)=>(order[a.verdict?.key]??9)-(order[b.verdict?.key]??9)||(b.verdict?.score||0)-(a.verdict?.score||0));
...
module.exports={VERSION,METHODS,ASTRA_TIMEFRAMES,MANUS_TIMEFRAMES,CONFIG,MANUS_CONFIG,finite,confirmed,candleMetrics,lastConfirmedTaker,takerCross,buildUniverse,initialFilter,marketState,oiFromProfile,scoreCandidate,classifyAstra,classifyManus,normalizeSymbols,methodOf,createAstraAutoScanner};
