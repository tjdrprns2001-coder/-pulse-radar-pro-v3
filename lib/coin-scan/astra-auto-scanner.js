...
function createAstraAutoScanner({provider,now=()=>Date.now()}={}){
  ...
  async function deep(symbols=[],{method=METHODS.ASTRA,market={},asOf=null}={}){
    const sampleValidation=methodOf(method)===METHODS.ASTRA?SampleBlind.blindValidation():null;
    ...
    const items=await safeMapLimit(provider,list,2,async symbol=>{
      ...
      if(method===METHODS.ASTRA){
        const adj=sampleValidation?.gate?.passed?SampleBlind.rankingAdjustment(row.sampleSimilarityV2):{points:0,eligible:false,reason:'VALIDATION_GATE_BLOCKED',policy:SampleBlind.POLICY.version};
        ...
        row.verdict.sampleValidationPassed=Boolean(sampleValidation?.gate?.passed);
    ...
    return{status:'ok',stage:'deep',method,version:VERSION,updatedAt:stamp,asOf:snapshotAsOf,timeframes:tfs,requested:list.length,marketState:market,sampleValidation:method===METHODS.ASTRA?sampleValidation:null,items};
  }
