...
const VERSION='ASTRA_AUTO_SCAN_v4';
const METHODS=Object.freeze({ASTRA:'astra',MANUS:'manus',PERPLEXITY:'perplexity',GROK:'grok',GEMINI:'gemini'});
...
function createAstraAutoScanner({provider,now=()=>Date.now()}={}){
  ...
  async function deep(symbols=[],{method=METHODS.ASTRA,market={},asOf=null}={}){
    method=methodOf(method);const tfs=method===METHODS.MANUS?MANUS_TIMEFRAMES:method===METHODS.PERPLEXITY?PERPLEXITY_TIMEFRAMES:method===METHODS.GROK?GROK_TIMEFRAMES:method===METHODS.GEMINI?GEMINI_TIMEFRAMES:ASTRA_TIMEFRAMES,list=normalizeSymbols(symbols,CONFIG.maxDeepSymbols),stamp=now(),snapshotAsOf=finite(asOf)||stamp;let fundingMap=new Map(),spotMap=new Map(),futuresMap=new Map();
    ...
    const items=await safeMapLimit(provider,list,2,async symbol=>{
      const frames={},errors=[];await safeMapLimit(provider,tfs,3,async tf=>{try{const rows=(method===METHODS.GROK||method===METHODS.GEMINI)?(tf==='5m'?1500:tf==='15m'?520:tf==='1d'?180:150):(tf==='1d'?180:150);frames[tf]=await provider.getFuturesKlines(symbol,tf,rows)}catch(e){errors.push(`${tf}:${String(e?.message||e)}`)}});
...
module.exports={VERSION,METHODS,ASTRA_TIMEFRAMES,MANUS_TIMEFRAMES,PERPLEXITY_TIMEFRAMES,GROK_TIMEFRAMES,GEMINI_TIMEFRAMES,CONFIG,MANUS_CONFIG,PERPLEXITY_CONFIG:Perplexity.CONFIG,GROK_CONFIG:Grok.CONFIG,GEMINI_CONFIG:Gemini.CONFIG,finite,confirmed,candleMetrics,lastConfirmedTaker,takerCross,buildUniverse,initialFilter,marketState,oiFromProfile,scoreCandidate,classifyAstra,classifyManus,classifyPerplexity:Perplexity.classify,classifyGrok:Grok.classify,classifyGemini:Gemini.classify,normalizeSymbols,methodOf,createAstraAutoScanner};
