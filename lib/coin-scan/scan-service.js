...
function buildCandidateSymbols(staged=[],limit=DEEP_LIMIT,seed=Date.now()){
  const scoreSort=(a,b)=>b.fast.candidateScore-a.fast.candidateScore||a.row.symbol.localeCompare(b.row.symbol);
  const ranked=staged.slice().sort(scoreSort);
  ...
  const continuation=staged.filter(x=>{const ch=Number(x.evidence.priceChange24h)||0,q=Number(x.evidence.quoteVolume24h)||0;return ch>=5&&ch<=22&&q>=3_000_000}).sort(scoreSort);
  const recovery=staged.filter(x=>{const ch=Number(x.evidence.priceChange24h)||0,q=Number(x.evidence.quoteVolume24h)||0;return ch>=-12&&ch<=3&&q>=3_000_000}).sort(scoreSort);
  const isFutures=x=>Boolean(x?.row?.futuresListed||x?.row?.marketScope==='futures'||x?.row?.marketScope==='spot+futures');
  const futuresUniverse=staged.filter(isFutures),base=futuresUniverse.length?futuresUniverse:staged;
  const h=s=>{let v=(Math.floor(Number(seed)/3600000)||0)>>>0;for(const ch of String(s)){v=((v*33)^ch.charCodeAt(0))>>>0}return v};
  const exploration=base.filter(x=>(Number(x.evidence.quoteVolume24h)||0)>=2_000_000&&Math.abs(Number(x.evidence.priceChange24h)||0)<=25).sort((a,b)=>h(a.row.symbol)-h(b.row.symbol));
  const quotas=[[quiet,10],[laggards,8],[continuation,8],[recovery,6],[ranked,6],[exploration,8]];
  const out=[],seen=new Set(),take=(bucket,max)=>{let n=0;for(const x of bucket){if(n>=max||out.length>=limit)break;if(!isFutures(x)&&futuresUniverse.length)continue;if(seen.has(x.row.symbol))continue;seen.add(x.row.symbol);out.push(x.row.symbol);n++}};
  for(const [bucket,max] of quotas)take(bucket,max);
  if(out.length<limit)take(ranked,limit-out.length);
  return out.slice(0,limit);
}
function buildPrescanPool(staged=[],limit=PRESCAN_POOL_LIMIT,seed=Date.now()){
  const isFutures=x=>Boolean(x?.row?.futuresListed||x?.row?.marketScope==='futures'||x?.row?.marketScope==='spot+futures');
  const futures=staged.filter(isFutures),base=futures.length?futures:staged;
  const scoreSort=(a,b)=>(Number(b.fast?.candidateScore)||0)-(Number(a.fast?.candidateScore)||0)||String(a.row?.symbol||'').localeCompare(String(b.row?.symbol||''));
  ...
  const movers=liquid.filter(x=>{const ch=Math.abs(Number(x.evidence?.priceChange24h)||0);return ch>=3&&ch<=18}).sort(scoreSort);
  const ranked=liquid.slice().sort(scoreSort);
  const h=s=>{let v=(Math.floor(Number(seed)/1800000)||0)>>>0;for(const ch of String(s)){v=((v*33)^ch.charCodeAt(0))>>>0}return v};
  const exploration=liquid.slice().sort((a,b)=>h(a.row.symbol)-h(b.row.symbol));
  const quotas=[[quiet,18],[continuation,18],[recovery,18],[movers,12],[ranked,12],[exploration,30]],out=[],seen=new Set();
  const take=(bucket,n)=>{let used=0;for(const x of bucket){if(used>=n||out.length>=limit)break;const s=x?.row?.symbol;if(!s||seen.has(s))continue;seen.add(s);out.push(s);used++}};
  for(const [bucket,n] of quotas)take(bucket,n);
  if(out.length<limit)take(base.slice().sort(scoreSort),limit-out.length);
  return out.slice(0,limit);
}
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false,persistObservations=true}={}){
    ...
    try{
      ...
      if(requestedMode==='prescan'){
        ...
        const prescanRows=[];
        for(const symbol of poolSymbols){const x=stagedBySymbol.get(symbol);if(!x)continue;prescanRows.push(LightPrescan.analyze({symbol,frames:lightBatch.results?.[symbol]||{},fast:x.fast,evidence:x.evidence}))}
        ...
        const sourceState=typeof provider.getSourceState==='function'?provider.getSourceState():{marketSource:'futures'};
      ...
      const candidateSymbols=buildCandidateSymbols(staged,Math.min(DEEP_LIMIT,staged.length),ts);
...
module.exports={DEEP_INTERVALS,DEEP_LIMIT,DEEP_REQUEST_LIMIT,PRESCAN_POOL_LIMIT,PRESCAN_OUTPUT_LIMIT,sectorFor,normalizeMetaEvidence,buildMarketBreadth,buildCandidateSymbols,buildPrescanPool,mergeDualUniverse,applySectorRotation,createScanService};
