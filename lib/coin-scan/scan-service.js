...
function buildCandidateSymbols(staged=[],limit=DEEP_LIMIT){
  const eligible=staged.filter(x=>x.candidatePriority?.eligible!==false);
...
function candidateSelectionMeta(staged=[],candidateSymbols=[]){
  const extended=staged.filter(x=>x.candidatePriority?.reason==='PRICE_EXTENDED').length,eligible=staged.filter(x=>x.candidatePriority?.eligible!==false).length;
  return{version:'PREIGNITION_SELECTION_v1',max24hAutoDeepPct:AUTO_DEEP_MAX_24H_PCT,eligibleCount:eligible,extendedExcludedCount:extended,selectedCount:candidateSymbols.length,futuresSelectedCount:candidateSymbols.filter(s=>staged.find(x=>x.row.symbol===s)?.row?.futuresListed).length};
}
