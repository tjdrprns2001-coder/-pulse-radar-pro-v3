...
function baseItem(staged,ts){
  ...
  const tradeSignal=Core.buildTradeSignal({...merged,category:c.category,dataState:c.dataState,structure:'neutral',preSurge:null});
  const item={symbol:row.symbol,baseAsset:row.baseAsset,category:c.category,scanClass,sector,priority:c.priority,dataState:c.dataState,reasons:c.reasons,structure:'neutral',momentum:'neutral',momentumSignals:null,tfState:{},preSurge:null,candidateScore:fast.candidateScore,tradeSignal,priceChange24h:evidence.priceChange24h,quoteVolume24h:evidence.quoteVolume24h,priceChange1h:null,priceChange15m:null,volumeAcceleration:null,takerRatio:null,updatedAt:ts};
  item.summary=Core.beginnerSummary(item);return item;
}
function deepItem(staged,deepBatch,ts){
  const {row,evidence,fast,sector}=staged;let deep=null,dataState='live';const frames=deepBatch.results?.[row.symbol];const symbolErrors=(deepBatch.errors||[]).filter(e=>e.symbol===row.symbol);const complete=frames&&DEEP_INTERVALS.every(tf=>Array.isArray(frames[tf])&&frames[tf].length>0);const ctx=deepBatch.contexts?.[row.symbol]||{};
  if(symbolErrors.length||!complete)dataState='failed';
  else deep=Deep.analyzeDeep({symbol:row.symbol,frames,dataState:'live',oiChangePct:ctx.oiChangePct??null,fundingPct:ctx.fundingPct??null,alertState:ctx.alertState||null});
  const merged={...evidence,...fast,...(deep||{}),metaEvidence:ctx.metaEvidence||null,dataState,reasons:deep?.reasons||fast.fastReasons||[]};if(dataState==='failed')merged.reasons=[`정밀 스캔 실패: ${symbolErrors[0]?.error||'필수 시간봉 데이터 누락'}`];
  const c=Core.classify(merged);const scanClass=Core.classifyV2(merged);const tradeSignal=Core.buildTradeSignal({...merged,category:c.category,dataState:c.dataState});
  const item={symbol:row.symbol,baseAsset:row.baseAsset,category:c.category,scanClass,sector,priority:c.priority,dataState:c.dataState,reasons:scanClass.reasons.length?scanClass.reasons:c.reasons,structure:deep?.structure||'neutral',momentum:deep?.momentum||'neutral',momentumSignals:deep?.momentumSignals||null,tfState:deep?.tfState||{},preSurge:deep?.preSurge||null,candidateScore:fast.candidateScore,tradeSignal,
...
function createScanService({provider,now=()=>Date.now()}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[]}={}){
    ...
    try{
      ...
      const response={status:'ok',mode:requestedMode,updatedAt:ts,scanCount:universe.length,deepScanCount,partial,dataHealth:health(items),categories:countBy(items,'category'),scanClasses:countScanClasses(items),candidateSymbols,items};
...
module.exports={DEEP_INTERVALS,DEEP_LIMIT,DEEP_REQUEST_LIMIT,sectorFor,applySectorRotation,createScanService};
