...
function baseItem(staged,ts){
  const {row,evidence,fast,sector}=staged;const merged={...evidence,...fast,dataState:'live',reasons:fast.fastReasons||[]};if(marketIntelligence?.available&&Number(marketIntelligence.exchangeCount)>=3)merged.reasons=[...(merged.reasons||[]),`다중거래소 ${marketIntelligence.exchangeCount}곳 교차확인`];
...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,recommendationHistory=null}={}){
  ...
  return{run,getMarketIntelligence,getTransitionSnapshot,listTransitionSnapshots,listRecommendationHistory,getRecommendationStats,evaluateRecommendationHistory,recordRecommendationPromotion};
}
