...
function cloneDelayed(last,now,error){
  const items=last.items.map(x=>{const category=x.category==='급등 전조 강함'?'급등 전조 관찰':x.category;const next={...x,category,priority:category==='급등 전조 관찰'&&x.category==='급등 전조 강함'?Math.min(Number(x.priority)||0,72):x.priority,dataState:'delayed',updatedAt:now};next.tradeSignal=Core.buildTradeSignal({...next,dataState:'delayed'});next.summary=Core.beginnerSummary(next);return next});
...
function baseItem(staged,ts){
  ...
  const item={symbol:row.symbol,baseAsset:row.baseAsset,category:c.category,sector,priority:c.priority,dataState:c.dataState,reasons:c.reasons,structure:'neutral',momentum:'neutral',momentumSignals:null,tfState:{},preSurge:null,candidateScore:fast.candidateScore,tradeSignal,updatedAt:ts};
...
function deepItem(staged,deepBatch,ts){
  ...
  const c=Core.classify(merged);const tradeSignal=Core.buildTradeSignal({...merged,category:c.category,dataState:c.dataState});const item={symbol:row.symbol,baseAsset:row.baseAsset,category:c.category,sector,priority:c.priority,dataState:c.dataState,reasons:c.reasons,structure:deep?.structure||'neutral',momentum:deep?.momentum||'neutral',momentumSignals:deep?.momentumSignals||null,tfState:deep?.tfState||{},preSurge:deep?.preSurge||null,candidateScore:fast.candidateScore,tradeSignal,updatedAt:ts};item.summary=Core.beginnerSummary(item);return item;
}
