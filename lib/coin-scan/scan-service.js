...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,recommendationHistory=null,marketValidationStore=null,marketValidationPerformance=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false}={}){
    ...
    try{
      ...
      let autoRecommendations=null,autoRecommendationValidation=null,autoScreening=null,autoScreeningMeta=null;
      if(requestedMode==='deep'){
        ...
        const screeningRows=gated.map(row=>CandidateScreening.screen(row,{execution:checked.get(row.symbol)||null,intelligence:intelChecked.get(row.symbol)||{},asOf:ts,now:ts}));
        autoScreening=CandidateScreening.bundle(screeningRows);
        autoScreeningMeta={version:'CANDIDATE_SCREENING_v2',screenedCount:screeningRows.length,detailedIntelCount:intelChecked.size,aCandidateCount:autoScreening.aCandidates.length,bWatchCount:autoScreening.bWatch.length,eventRiskCount:autoScreening.eventRisk.length,dataInsufficientCount:autoScreening.dataInsufficient.length,excludedCount:autoScreening.excluded.length,readOnly:true,note:'시장 후보 선별 결과이며 자동 주문 또는 수익 보장을 의미하지 않습니다.'};
      }
