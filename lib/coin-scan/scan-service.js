...
function createScanService({provider,now=()=>Date.now()}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100}={}){
    ...
    try{
      ...
      const deepBatch=candidates.length?await provider.scanDeepCandidates(candidates.map(x=>x.row.symbol),DEEP_INTERVALS):{results:{},errors:[],contexts:{}};
      const errorSymbols=new Set((deepBatch.errors||[]).map(e=>e.symbol));
      ...
      const items=staged.map(({row,evidence,fast,sector})=>{
        ...
        if(candidateSet.has(row.symbol)){
          const frames=deepBatch.results?.[row.symbol];
          const complete=frames&&DEEP_INTERVALS.every(tf=>Array.isArray(frames[tf])&&frames[tf].length>0);
          if(errorSymbols.has(row.symbol)||!complete)dataState='failed';
        ...
        if(dataState==='failed')merged.reasons=[`정밀 스캔 실패: ${errorBySymbol.get(row.symbol)||'필수 시간봉 데이터 누락'}`];
      ...
      if(lastGood){let fallback=cloneDelayed(lastGood,now(),e);let filtered=fallback.items;if(category)filtered=filtered.filter(x=>x.category===category);if(sector)filtered=filtered.filter(x=>x.sector===sector);return{...fallback,items:filtered.slice(0,max)}}
