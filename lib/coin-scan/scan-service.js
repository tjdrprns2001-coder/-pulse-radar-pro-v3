  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false}={}){
    ...
    try{
      ...
      if(requestedMode==='deep'){
        ...
        const screeningRows=gated.map(row=>CandidateScreening.screen(row,{execution:checked.get(row.symbol)||null,intelligence:intelChecked.get(row.symbol)||{},decisionTime:ts,dataCutoff:ts,timeframe:'5m',asOf:ts,now:ts}));
