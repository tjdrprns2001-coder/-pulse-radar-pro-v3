...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,recommendationHistory=null,marketValidationStore=null}={}){
  ...
  async function getMarketIntelligence(symbol){
    ...
    return{status:'ok',mode:'intelligence',symbol:s,binanceListed:true,marketScope:spotTicker&&futuresTicker?'spot+futures':spotTicker?'spot':'futures',updatedAt:now(),intelligence};
  }
  async function getMarketValidation(symbol,{decisionTimestamp=null,persist=true}={}){
    ...
    const decision=Number(decisionTimestamp)||now();
    const [deep,intel]=await Promise.all([run({mode:'deep',limit:1,symbols:[s],precision:true}),getMarketIntelligence(s)]);
    ...
    const out=MarketValidation.validate({symbol:s,item,intelligence:{...(intel.intelligence||{}),updatedAt:intel.updatedAt},decisionTimestamp:decision,capturedAt:now()});
    let persisted=false;if(persist&&marketValidationStore&&typeof marketValidationStore.putMarketValidationSnapshot==='function'){try{persisted=await marketValidationStore.putMarketValidationSnapshot(out.snapshotId,out.snapshot)}catch(_e){persisted=false}}
    return{status:'ok',mode:'validation',symbol:s,marketScope:intel.marketScope,updatedAt:now(),persisted,scanner:{updatedAt:deep.updatedAt,item},intelligence:intel.intelligence,validation:{...out,snapshot:undefined}};
  }
  ...
  return{run,getMarketIntelligence,getMarketValidation,getMarketValidationSnapshot,listMarketValidationSnapshots,replayMarketValidation,getTransitionSnapshot,listTransitionSnapshots,listRecommendationHistory,getRecommendationStats,evaluateRecommendationHistory,recordRecommendationPromotion};
}
