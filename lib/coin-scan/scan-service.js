...
function buildCandidateSymbols(staged=[],limit=DEEP_LIMIT){
  ...
  const futures=pool.filter(x=>x.row.futuresListed||x.row.marketScope==='futures'||x.row.marketScope==='spot+futures'),futuresSymbols=new Set(futures.map(x=>x.row.symbol)),spotOnly=pool.filter(x=>!futuresSymbols.has(x.row.symbol));
