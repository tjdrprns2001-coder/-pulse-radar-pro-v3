...
function buildCandidateSymbols(staged=[],limit=DEEP_LIMIT){
  ...
  const ranked=eligible.slice().sort((a,b)=>(Number(b.candidatePriority?.score)||0)-(Number(a.candidatePriority?.score)||0)||(Number(b.fast?.candidateScore)||0)-(Number(a.fast?.candidateScore)||0)||a.row.symbol.localeCompare(b.row.symbol));
  const laggards=eligible.filter(x=>Math.abs(Number(x.evidence.priceChange24h)||0)<=6&&(Number(x.evidence.quoteVolume24h)||0)>=5_000_000).sort((a,b)=>(Number(b.candidatePriority?.score)||0)-(Number(a.candidatePriority?.score)||0)||(Number(b.evidence.quoteVolume24h)||0)-(Number(a.evidence.quoteVolume24h)||0));
  const quiet=eligible.filter(x=>Math.abs(Number(x.evidence.priceChange24h)||0)<=3&&(Number(x.evidence.quoteVolume24h)||0)>=5_000_000).sort((a,b)=>(Number(b.candidatePriority?.score)||0)-(Number(a.candidatePriority?.score)||0)||Math.abs(Number(a.evidence.priceChange24h)||0)-Math.abs(Number(b.evidence.priceChange24h)||0));
  const pool=[],poolSeen=new Set();for(const bucket of [ranked.slice(0,24),quiet.slice(0,18),laggards.slice(0,18)])for(const x of bucket){if(poolSeen.has(x.row.symbol))continue;poolSeen.add(x.row.symbol);pool.push(x)}
  const futures=pool.filter(x=>x.row.futuresListed||x.row.marketScope==='futures'||x.row.marketScope==='spot+futures'),futureSet=new Set(futures.map(x=>x.row.symbol)),spotOnly=pool.filter(x=>!futureSet.has(x.row.symbol));
...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,setupStateTracker=null,recommendationHistory=null,preIgnitionHistory=null,marketValidationStore=null,marketValidationPerformance=null,selectorLedger=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false}={}){
    ...
    try{
      ...
      const candidateSymbols=buildCandidateSymbols(staged,Math.min(DEEP_LIMIT,staged.length)),candidateSelection=candidateSelectionMeta(staged,candidateSymbols);
      ...
      if(requestedMode==='deep'&&setupStateTracker&&typeof setupStateTracker.observe==='function'){
      ...
      if(requestedMode==='deep'&&preIgnitionHistory&&typeof preIgnitionHistory.observe==='function'){
...
module.exports={DEEP_INTERVALS,DEEP_LIMIT,DEEP_REQUEST_LIMIT,AUTO_DEEP_MAX_24H_PCT,sectorFor,normalizeMetaEvidence,buildMarketBreadth,fastCandidatePriority,buildCandidateSymbols,candidateSelectionMeta,deepPreIgnitionScore,mergeDualUniverse,applySectorRotation,createScanService};
