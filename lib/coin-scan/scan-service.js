...
function deepItem(staged,deepBatch,ts){
  ...
  const item={symbol:row.symbol,baseAsset:row.baseAsset,category:c.category,scanClass,sector,priority:c.priority,dataState:c.dataState,reasons:scanClass.reasons.length?scanClass.reasons:c.reasons,structure:deep?.structure||'neutral',momentum:deep?.momentum||'neutral',momentumSignals:deep?.momentumSignals||null,tfState:deep?.tfState||{},preSurge:deep?.preSurge||null,candidateScore:fast.candidateScore,tradeSignal,lastPrice:evidence.lastPrice,
    ...
    volumeAcceleration4h:deep?.volumeAcceleration4h??null,volumeAcceleration1h:deep?.volumeAcceleration1h??null,volumeAcceleration15m:deep?.volumeAcceleration15m??null,volumeIncreasing5m:deep?.volumeIncreasing5m??0,structureShift4h:Boolean(deep?.structureShift4h),lowTrend:deep?.lowTrend||'unknown',breakout:Boolean(deep?.breakout),samplePattern:deep?.samplePattern||null,sampleArchetype:deep?.samplePattern?.archetype||null,samplePhase:deep?.samplePattern?.phase||null,sampleScore:deep?.samplePattern?.score??null,xoiSequence:deep?.samplePattern?.sequenceTag||null,xoiProfile:deep?.samplePattern?.xoiProfile||null,liquidityPattern:deep?.samplePattern?.liquidity?.key||null,bookEvidence:deep?.bookEvidence||null,
    bookScore:deep?.bookEvidence?.book?.score??null,bookBias:deep?.bookEvidence?.book?.bias||'중립',bookHtfAligned:deep?.bookEvidence?.book?.htfAligned||'혼조',smartMoneyScore:deep?.bookEvidence?.smartMoney?.score??null,smartMoneyBias:deep?.bookEvidence?.smartMoney?.bias||'중립',ictState:deep?.bookEvidence?.ict?.state||'none',
    v2Flow:deep?.v2Flow||null,v2Csv:deep?.v2Csv||null,v2Type:deep?.v2Flow?.type||'미완성',v2Stage:deep?.v2Flow?.stageLabel||'⚪ 관찰',direction:deep?.v2Flow?.direction||'none',deadBand:Boolean(deep?.v2Flow?.deadBand),sampleSubtype:deep?.v2Flow?.sampleSubtype||null,fusion:deep?.v2Flow?.fusion||null,fusionScore:deep?.v2Flow?.fusion?.score??null,fusionEligible:Boolean(deep?.v2Flow?.fusion?.eligible),oi4hChangePct:deep?.v2Input?.oi4hPct??null,oi8hChangePct:deep?.v2Input?.oi8hPct??null,trueTakerRatio:deep?.v2Flow?.takerLatest??null,fundingRate:deep?.v2Input?.fundingRate??null,paramSet:deep?.v2Flow?.paramSet||null,metaEvidence,updatedAt:ts};
  item.summary=Core.beginnerSummary(item);return item;
}
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[]}={}){
    ...
    try{
      ...
      if(requestedMode==='deep'){
        ...
        deepBatch=selected.length?await provider.scanDeepCandidates(targets,DEEP_INTERVALS):{results:{},errors:[],contexts:{}};deepScanCount=targets.length;partial=(deepBatch.errors||[]).length>0;items=selected.map(x=>deepItem(x,deepBatch,ts));
      ...
      items.sort((a,b)=>(Number(b.fusionEligible)||0)-(Number(a.fusionEligible)||0)||(Number(b.fusionScore)||0)-(Number(a.fusionScore)||0)||(v2Rank.get(a.v2Type)??99)-(v2Rank.get(b.v2Type)??99)||(Number(b.isTransitionEvent)||0)-(Number(a.isTransitionEvent)||0)||(classRank.get(a.scanClass?.key)??99)-(classRank.get(b.scanClass?.key)??99)||(Number(b.tradeSignal?.confidence)||0)-(Number(a.tradeSignal?.confidence)||0)||b.priority-a.priority||b.candidateScore-a.candidateScore||a.symbol.localeCompare(b.symbol));
      const response={status:'ok',mode:requestedMode,scannerVersion:'v2',paramSet:'v2_taker_0.8_1.2',universe:'Binance USDT perpetual',universeMeta:{key:'binance-usdt-perpetual',label:'코어 유니버스 · Binance USDT 무기한',count:universe.length},timeframes:DEEP_INTERVALS,updatedAt:ts,scanCount:universe.length,deepScanCount,partial,dataHealth:health(items),marketBreadth:buildMarketBreadth(universe),categories:countBy(items,'category'),scanClasses:countScanClasses(items),candidateSymbols,performanceRecording,alertRecording,transitionSnapshotRecording,auxiliary,items};
