...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[]}={}){
    ...
    try{
      ...
      let performanceRecording=null;
      if(requestedMode==='deep'&&performanceRecorder&&typeof performanceRecorder.recordItems==='function'){
        try{performanceRecording=await performanceRecorder.recordItems(items)}catch(e){performanceRecording={recorded:0,duplicate:0,skipped:0,errors:[String(e?.message||e)]}}
      }
      ...
      const response={status:'ok',mode:requestedMode,updatedAt:ts,scanCount:universe.length,deepScanCount,partial,dataHealth:health(items),categories:countBy(items,'category'),scanClasses:countScanClasses(items),candidateSymbols,performanceRecording,items};
