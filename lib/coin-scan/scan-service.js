...
function buildCandidateSymbols(staged=[],limit=DEEP_LIMIT){
  ...
  const pool=[],poolSeen=new Set();for(const bucket of [quiet.slice(0,20),laggards.slice(0,20),ranked.slice(0,24)])for(const x of bucket){if(poolSeen.has(x.row.symbol))continue;poolSeen.add(x.row.symbol);pool.push(x)}
  const futures=pool.filter(isFutures),spotOnly=pool.filter(x=>!isFutures(x));
  const source=futures.length?futures:spotOnly,out=[];for(const x of source){out.push(x.row.symbol);if(out.length>=limit)return out}return out;
}
