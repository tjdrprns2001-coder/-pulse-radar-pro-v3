...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,setupStateTracker=null,recommendationHistory=null,marketValidationStore=null,marketValidationPerformance=null,selectorLedger=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false}={}){
    ...
    try{
      ...
      let setupValidationRecording={checked:0,errors:[]};
      if(requestedMode==='deep'&&typeof provider.getSetupValidationContext==='function'){
        ...
        const worker=async item=>{
          try{
            const ctx=await provider.getSetupValidationContext(item.symbol,{spotListed:Boolean(item.spotListed),futuresListed:Boolean(item.futuresListed)});
            SetupExecution.apply({item,frames:deepBatch?.results?.[item.symbol]||{},execution:ctx.execution||{},spot15m:ctx.spot15m||[],eventRisk:null,sequenceGap:false,feeBps:4});
            return null;
          }catch(e){return item.symbol+':'+String(e?.message||e)}
        };
        const errs=typeof provider.mapLimit==='function'?await provider.mapLimit(setupItems,worker):await Promise.all(setupItems.map(worker));
        setupValidationRecording={checked:setupItems.length,errors:(errs||[]).filter(Boolean)};
      }
