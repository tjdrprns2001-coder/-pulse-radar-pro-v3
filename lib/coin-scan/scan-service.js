...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[]}={}){
    ...
    try{
      ...
      const response={status:'ok',mode:requestedMode,updatedAt:ts,scanCount:universe.length,deepScanCount,partial,dataHealth:health(items),categories:countBy(items,'category'),scanClasses:countScanClasses(items),candidateSymbols,performanceRecording,alertRecording,auxiliary,items};
...
module.exports={DEEP_INTERVALS,DEEP_LIMIT,DEEP_REQUEST_LIMIT,sectorFor,normalizeMetaEvidence,applySectorRotation,createScanService};
