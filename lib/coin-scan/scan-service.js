...
function createScanService({provider,now=()=>Date.now(),performanceRecorder=null,alertRecorder=null,transitionSnapshotRecorder=null,setupStateTracker=null,recommendationHistory=null,preIgnitionHistory=null,marketValidationStore=null,marketValidationPerformance=null,selectorLedger=null}={}){
  ...
  async function run({mode='summary',category=null,sector=null,limit=100,symbols=[],precision=false,validation='auto',persistObservations=true}={}){
    const max=Math.max(1,Math.min(500,Number(limit)||100)),requestedMode=String(mode||'summary').toLowerCase(),precisionMode=Boolean(precision),validationMode=String(validation||'auto').toLowerCase();
    try{
