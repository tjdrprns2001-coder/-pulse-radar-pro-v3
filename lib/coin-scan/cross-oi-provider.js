...
function createCrossOiProvider({fetchImpl=globalThis.fetch,cache=null,bybitBase='https://api.bybit.com',okxBase='https://www.okx.com',gateBase='https://api.gateio.ws',adapters=[],requestTimeoutMs=3000}={}){
  ...
  async function getJson(url,ttlMs,key){
    ...
    const timeout=Math.max(500,Math.min(10000,Number(requestTimeoutMs)||3000));
