...
function createMemoryRawEventJournal({maxEvents=5000,maxQueue=1000,maxDlq=1000}={}){
  const events=new Map(),idem=new Map(),queue=[],dlq=[];
  function trimEvents(){while(events.size>maxEvents){const first=events.keys().next().value;if(first==null)break;const ev=events.get(first);events.delete(first);if(ev?.idempotency_key&&idem.get(ev.idempotency_key)===first)idem.delete(ev.idempotency_key)}}
  function trimArray(arr,max){if(arr.length>max)arr.splice(0,arr.length-max)}
  return{
    append(input){
      ...
      events.set(e.event_id,e);if(!prior)idem.set(e.idempotency_key,e.event_id);trimEvents();
    ...
    correction(originalId,input){
      ...
      events.set(e.event_id,e);trimEvents();return JSON.parse(JSON.stringify(e));
    },
    enqueue(kind,eventId,reason){queue.push({kind:String(kind),event_id:String(eventId),reason:String(reason||''),queued_at:Date.now()});trimArray(queue,maxQueue);return true},
    deadLetter(eventId,reason){dlq.push({event_id:String(eventId),reason:String(reason||''),created_at:Date.now()});trimArray(dlq,maxDlq);return true},
