...
function createEvmCollector({chainId='1',primary,secondary=null,confirmations=12,finalityBlocks=64,now=()=>Date.now()}={}){
  ...
  async function receipt(txHash){return primary.call('eth_getTransactionReceipt',[txHash])}
  async function verifyReceipt(txHash){
    const r=await receipt(txHash);if(!r)return{status:'PENDING',txHash};
    const bnum=num(r.blockNumber),cmp=await compareBlock(bnum),headNumber=await head();
    if(cmp.conflicted||String(r.blockHash||'').toLowerCase()!==String(cmp.primaryHash||'').toLowerCase())return{status:'REORG_CANDIDATE',txHash,blockNumber:bnum,blockHash:r.blockHash,canonicalHash:cmp.primaryHash};
    return{status:finalityStatus(bnum,headNumber),txHash,blockNumber:bnum,blockHash:r.blockHash,headNumber,receipt:r};
  }
  ...
  return{head,block,receipt,verifyReceipt,compareBlock,logs,transferEvents,reconcile,watermark,finalityStatus};
}
