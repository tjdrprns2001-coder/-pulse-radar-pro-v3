...
function canonicalize({symbol,item={},intelligence={},decisionTimestamp,capturedAt=Date.now()}={}){
 const d=ts(decisionTimestamp)??ts(item.updatedAt)??capturedAt,cex=intelligence.cex||{},ind=intelligence.indicators||{},dex=intelligence.dex||{},news=intelligence.news||{},events=intelligence.events||{},wallets=intelligence.wallets||{},verify=intelligence.walletVerification||{};
 return{
  engineVersion:VERSION,symbol:cleanSymbol(symbol||item.symbol),decisionTimestamp:d,capturedAt:ts(capturedAt)??Date.now(),
  signal:{updatedAt:ts(item.updatedAt),dataState:item.dataState||null,marketScope:item.marketScope||null,spotListed:Boolean(item.spotListed),futuresListed:Boolean(item.futuresListed),direction:item.direction||item.v2Flow?.direction||'none',tradeLevel:item.tradeSignal?.level||null,v3LongTier:item.v3LongTier||null,v3Invalidation:Boolean(item.v3Invalidation),priceChange24h:finite(item.priceChange24h),oi4hChangePct:finite(item.oi4hChangePct),oi8hChangePct:finite(item.oi8hChangePct),trueTakerRatio:finite(item.trueTakerRatio),fundingRate:finite(item.fundingRate),spotFuturesBasisPct:finite(item.spotFuturesBasisPct)},
...
function buildGates(c,evidence){
 ...
 const gates=[
  ...
  {id:'execution',label:'체결성',pass:null,failure:'NOT_EVALUATED',note:'spread/depth/slippage 실시간 원장이 아직 없어 v1에서는 판정 제외'},
...
function deriveStatus(c,evidence,gp){
 ...
 else if(fail.has('STALE'))status='STALE';else if(fail.has('CONFLICTED'))status='CONFLICTED';else if(fail.has('SINGLE_SOURCE')||gp.missingCritical.length)status='INSUFFICIENT_DATA';
