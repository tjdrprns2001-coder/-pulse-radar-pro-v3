...
function candidateUniqueness(selected=[],candidate){
  const bag=[...selected,candidate],bounds=[...new Set(bag.flatMap(x=>[x.startTime,x.endTime+1]))].sort((a,b)=>a-b);
  let weighted=0,total=0;
  for(let i=0;i<bounds.length-1;i++){
    const a=bounds[i],b=bounds[i+1],mid=(a+b)/2;if(mid<candidate.startTime||mid>candidate.endTime)continue;
    const active=bag.filter(x=>x.startTime<=mid&&x.endTime>=mid).length,span=b-a;
    if(active>0){weighted+=span/active;total+=span}
  }
...
function sequentialBootstrap(events=[],sampleSize=null,{seed=17}={}){
  ...
  const size=Math.max(1,Math.floor(sampleSize??e.length)),picked=[],rng=seeded(seed);
  for(let draw=0;draw<size;draw++){
    const candidates=e.map(c=>({c,w:Math.max(1e-6,candidateUniqueness(picked,c))}));
    ...
    picked.push({...candidates[choice].c,bootstrapDraw:draw});
  }
  return picked;
}
...
module.exports={VERSION,normalizeEvents,uniquenessWeights,candidateUniqueness,sequentialBootstrap,buildPurgedWalkForward,classificationMetrics,validate};
