...
function hardBlock(item){
  ...
  if(item?.strategyCycle?.fakeout?.longBlocked)push(reasons,'가짜 돌파 확정 · 롱 회피');
  if(String(item?.strategyCycle?.entryQuality?.grade||'')==='D')push(reasons,'D급 진입 자리');
...
function evaluate(item={}){
  ...
  const reentry=Boolean(item.strategyCycle?.reentryEligible),patternSetup=Boolean(item.strategyCycle?.patternEligible),entryGrade=String(item.strategyCycle?.entryQuality?.grade||'');if(entryGrade==='A'){score+=8;push(reasons,'A급 진입 자리')}else if(entryGrade==='B'){score+=3;push(reasons,'B급 진입 자리')}else if(entryGrade==='C'){score-=6;push(invalidations,'C급 진입 자리 · 관찰 우선')}const ch=n(item.priceChange24h);if(ch!=null){if(Math.abs(ch)<=3){score+=10;push(reasons,'24H 덜 진행 '+(ch>=0?'+':'')+ch.toFixed(1)+'%')}else if(Math.abs(ch)<=6){score+=6;push(reasons,'24H 아직 과진행 아님')}else if(ch>8&&!reentry){score-=8;push(invalidations,'24H 추격 위험 '+ch.toFixed(1)+'%')}else if(ch>8&&reentry){score+=4;push(reasons,'급등 후 재진입 사이클 확인')}}else push(missing,'24H 변동');if(reentry||patternSetup){const ss=n(item.strategyCycle?.reentryScore);if(ss!=null){score+=Math.min(14,ss*.14);push(reasons,(reentry?'재상승':'패턴')+' 점수 '+ss.toFixed(0))}}
