...
const SPEC_VERSION='selector-r0.2';
const LABELS={
  CANDIDATE:'검증 후보',
  WATCHLIST:'감시 목록',
  EVENT_RISK:'이벤트 위험',
...
function screen(row={},opts={}){
  ...
  let penalty=0;if(der.crowded)penalty+=20;if(evt.level==='WATCH')penalty+=10;if(dq.priceDispersionPct!=null&&dq.priceDispersionPct>.2)penalty+=10;penalty+=Number(on.risk_penalty)||0;penalty+=Number(cat.risk_penalty)||0;
  ...
  else if(cat.hard_event_risk||evt.level==='HIGH')classification='EVENT_RISK';
  else if(cat.high_impact_event||evt.level==='WATCH')classification='WATCHLIST';
  ...
  return{
    version:'CANDIDATE_SCREENING_v4',
