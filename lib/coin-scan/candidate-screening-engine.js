...
const SPEC_VERSION='selector-r0.1';
const LABELS={
  CANDIDATE:'검증 후보',
  WATCHLIST:'감시 목록',
  EVENT_RISK:'이벤트 위험',
...
const WEIGHTS={marketQuality:.25,ictSetup:.25,executionQuality:.20,derivativesContext:.10,onchainContext:.10,dataQuality:.10};
...
function iso(v){const x=n(v);if(x!=null)return new Date(x).toISOString();const d=new Date(v);return Number.isFinite(d.getTime())?d.toISOString():null}
...
function structureScore(item={}){
  ...
  const cls=String(item.scanClass?.key||'');w+=20;if(cls==='PRE-SURGE'){s+=20;push(ev,'liquidity/structure presurge condition')}else if(['ACCUMULATION-PRE','META-PRE'].includes(cls)){s+=12;push(ev,'accumulation/leading structure')}
...
function derivativesScore(item={}){
  ...
  const crowded=Boolean(item.validationGate?.crowding?.crowded);if(crowded){s-=30;push(contra,'derivatives crowding gate')}
...
function hardFilters(row={},dq={},evt={}){
  ...
  if(evt.level==='HIGH'&&evt.high.some(x=>/(hack|exploit|honeypot|transfer restrict)/i.test(x)))push(reasons,'SECURITY_EVENT');
...
function screen(row={},opts={}){
  ...
  const dq=dataQuality(item,intel,execution,decisionTime),st=structureScore(item),der=derivativesScore(item),on=onchainScore(intel),evt=eventRisk(intel,decisionTime),mq=marketQuality(item,dq);
  ...
  const weighted=[[scores.market_quality,WEIGHTS.marketQuality],[scores.ict_setup,WEIGHTS.ictSetup],[scores.execution_quality,WEIGHTS.executionQuality],[scores.derivatives_context,WEIGHTS.derivativesContext],[scores.onchain_context,WEIGHTS.onchainContext],[scores.data_quality,WEIGHTS.dataQuality]].filter(([v])=>v!=null);
  const wt=weighted.reduce((s,[,w])=>s+w,0),base=wt?weighted.reduce((s,[v,w])=>s+v*w,0)/wt:null;
  let penalty=0;if(der.crowded)penalty+=20;if(evt.level==='WATCH')penalty+=10;if(dq.priceDispersionPct!=null&&dq.priceDispersionPct>.2)penalty+=10;
  scores.final_score=base==null?null:Math.round(clamp(base-penalty)*10)/10;
  let classification='WATCHLIST';
  if(hardRejectReasons.includes('SECURITY_EVENT')||hardRejectReasons.includes('UPSTREAM_VALIDATION_REJECTED')||hardRejectReasons.includes('SPREAD_HARD_LIMIT')||hardRejectReasons.includes('SLIPPAGE_HARD_LIMIT')||hardRejectReasons.includes('DEPTH_HARD_LIMIT'))classification='REJECTED';
  ...
  else if(hardRejectReasons.includes('CROSS_VENUE_PRICE_CONFLICT'))classification='RISK_FILTERED';
  else if(evt.level==='HIGH'||evt.level==='WATCH')classification='EVENT_RISK';
  ...
  const input={symbol,instrumentId,decisionTime,dataCutoff,timeframe,scores,classification,evidence,contradictions,missing,hardRejectReasons,eventRisk:evt};
  ...
  return{
    version:'CANDIDATE_SCREENING_v3',
...
function bundle(rows=[]){
  const order={CANDIDATE:0,WATCHLIST:1,EVENT_RISK:2,RISK_FILTERED:3,INSUFFICIENT_DATA:4,REJECTED:5};
