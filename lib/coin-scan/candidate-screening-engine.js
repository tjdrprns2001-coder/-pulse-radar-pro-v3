...
function screen(row={},opts={}){
  ...
  const dq=dataQuality(item,intel,execution,decisionTime),st=structureScore(item),der=derivativesScore(item,row.validationGate),on=onchainScore(intel),evt=eventRisk(intel,decisionTime),mq=marketQuality(item,dq);
