...
function createTraderService({provider,now=()=>Date.now()}={}){
  ...
  async function market(){return memo('market',60000,async()=>{
    // Slow-changing metadata is reusable; prices/regime must not outlive the 2m freshness gate.
    const [info,tickers]=await Promise.all([memo('universe',900000,()=>provider.getFuturesUniverse()),provider.getFuturesTickers()]);
  ...
  function audit(item,m,ticker,stage){
    return{source:'Binance USDT perpetual futures',stage,evaluatedAt:item.asOf,marketSnapshotAt:m.asOf,
      tickerTimestamp:Engine.num(ticker.closeTime),tickerAgeMs:Engine.num(ticker.closeTime)==null?null:item.asOf-Number(ticker.closeTime),
      regimeObservedAt:m.regime?.observedAt??null,oiTimestamp:item.flow?.oiTimestamp??null,takerTimestamp:item.flow?.takerTimestamp??null,
      frames:Object.fromEntries(Object.entries(item.stats||{}).map(([tf,s])=>[tf,{available:s.available,bars:s.bars,lastClosedAt:s.lastClosedAt,fresh:s.fresh}]))};
  }
  async function validate(symbol,m){if(!/^[A-Z0-9]{1,30}USDT$/.test(symbol)||!m.universe.some(x=>x.symbol===symbol))throw bad('Symbol is not in Binance USDT perpetual universe')}
  ...
  async function deep(symbol){
    ...
    return memo('deep:'+symbol,20000,async()=>{
      ...
      if(prefilter){
        ...
        item.dataAudit=audit(item,m,ticker,'PREFILTER');
      ...
      item.dataAudit=audit(item,m,ticker,'DEEP');
      return{status:'ok',item,errors,source:'Binance futures',asOf:item.asOf};
    });
  }
