...
function rows(raw=[]){
  return (Array.isArray(raw)?raw:[]).filter(Array.isArray).filter(r=>n(r[4])!=null&&n(r[2])!=null&&n(r[3])!=null).map((r,i)=>({
...
function impulseReset(c){
  ...
  let lo=Infinity,hi=-Infinity,loIndex=0,hiIndex=0;
  for(let i=0;i<w.length;i++){if(w[i].low<lo){lo=w[i].low;loIndex=i}if(i>=loIndex&&w[i].high>hi){hi=w[i].high;hiIndex=i}}
  const last=w.at(-1).close,impulsePct=lo>0?((hi/lo)-1)*100:null,pullbackPct=hi>0?((last/hi)-1)*100:null;
...
function analyze({frames={},priceChange24h=null,priceChange1h=null,priceChange15m=null,structure='neutral',momentumSignals=null,takerRatio=null,oiChangePct=null}={}){
  ...
  const reentryEligible=Boolean(priorSurge&&cooled&&(reset||structural.some(x=>['LIQUIDITY_RECLAIM','MA_RETEST'].includes(x.key)))&&reentryScore>=68&&structure!=='bearish');
  const patternEligible=Boolean(!reentryEligible&&cooled&&structural.length&&reentryScore>=68&&structure!=='bearish');
  ...
  return{version:VERSION,stage,reentryEligible,patternEligible,reentryScore,priorSurge,cooled,reset,impulse,activeTracks:active.slice(0,8),best:active[0]||null,reasons:reasons.slice(0,8),timeframes:tfResults};
}
