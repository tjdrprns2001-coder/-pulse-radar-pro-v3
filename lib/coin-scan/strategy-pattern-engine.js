...
function analyze({frames={},priceChange24h=null,priceChange1h=null,priceChange15m=null,structure='neutral',momentumSignals=null,takerRatio=null,oiChangePct=null,trueCvd=null}={}){
  ...
  if(structural.length>=2)reentryScore+=6;if(structure==='bullish')reentryScore+=5;if(tk!=null&&tk>=1)reentryScore+=3;if(oi!=null&&oi>=0)reentryScore+=2;if((bookManual?.warnings||[]).length)reentryScore-=8;if(entryTiming)reentryScore+=Number(entryTiming.scoreDelta)||0;
  reentryScore=Math.round(clamp(reentryScore));
  const flowOk=tk==null||tk>=.90;
  const hardRetestFail=entryTiming?.stage==='FAILED_RETEST_RESISTANCE';
  ...
  const reasons=[];if(priorSurge)reasons.push('최근 상승 파동 이력');if(reset)reasons.push('고점 대비 눌림·리셋 확인');if(cooled)reasons.push('1H·15m 단기 과열 해소');if(entryTiming)reasons.push(entryTiming.label);for(const x of structural.slice(0,3))reasons.push(x.label+' '+x.stage);
  const strategyCore={version:VERSION,stage,reentryEligible,patternEligible,reentryScore,priorSurge,cooled,reset,flowOk,impulse,entryTiming,breakoutStates:breakoutStates.slice(0,6),activeTracks:active.slice(0,10),best:active[0]||null,reasons:reasons.slice(0,8),timeframes:tfResults,bookManual};
