...
function analyze({frames={},priceChange24h=null,priceChange1h=null,priceChange15m=null,structure='neutral',momentumSignals=null,takerRatio=null,oiChangePct=null,trueCvd=null}={}){
  ...
  if(bookManual){const top=[...(bookManual.rankable||[]),...(bookManual.evidence||[])].sort((a,b)=>b.score-a.score)[0];if(top)tracks.push(track('BOOK_MANUAL','책 종합기법',top.score>=58,top.score,top.tf,top.stage,[top.label,...(top.reasons||[])],{techniqueId:top.id,bookScore:bookManual.score,riskPlan:bookManual.riskPlan}))}
  ...
  if(structural.length>=2)reentryScore+=6;if(structure==='bullish')reentryScore+=5;if(tk!=null&&tk>=1)reentryScore+=3;if(oi!=null&&oi>=0)reentryScore+=2;if((bookManual?.warnings||[]).length)reentryScore-=8;
  ...
  return{version:VERSION,stage,reentryEligible,patternEligible,reentryScore,priorSurge,cooled,reset,flowOk,impulse,activeTracks:active.slice(0,10),best:active[0]||null,reasons:reasons.slice(0,8),timeframes:tfResults,bookManual};
}
