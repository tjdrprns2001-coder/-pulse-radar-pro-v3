...
const BookManual=require('./book-manual-engine.js');
const EntryConfirmation=require('./entry-confirmation-engine.js');
const Fakeout=require('./fakeout-state-engine.js');
...
function postBreakoutState(c,tf,forex,book){
  if(c.length<20)return null;
  const a=atr(c,14),av=a.at(-1),last=c.at(-1);if(!(av>0&&last?.close>0))return null;
  const candidates=[];
  const trend=book?.trendline?.best;
  if(trend?.side==='상승'&&n(trend.line)!=null)candidates.push({kind:'TRENDLINE',label:'추세선',level:Number(trend.line),sourceState:String(trend.state||'')});
  const chart=forex?.bestChartPattern;
  if(chart?.side==='상승'&&n(chart.trigger)!=null)candidates.push({kind:'CHART_PATTERN',label:String(chart.label||chart.id||'차트 패턴'),level:Number(chart.trigger),sourceState:'PATTERN_TRIGGER'});
  if(!candidates.length)return null;
  const out=[];
  for(const q of candidates){
    const level=q.level;if(!(level>0))continue;
    const recent=c.slice(-10),touchBand=av*.28,breakBuffer=av*.18,failBuffer=av*.22;
    let breakIndex=-1;for(let i=1;i<recent.length;i++)if(recent[i-1].close<=level+breakBuffer&&recent[i].close>level+breakBuffer)breakIndex=i;
    const broke=breakIndex>=0||['CLOSE_CONFIRMED','RETEST'].includes(q.sourceState)||last.close>level+breakBuffer;
    if(!broke)continue;
    const since=breakIndex>=0?recent.slice(breakIndex+1):recent.slice(-5);
    const touched=q.sourceState==='RETEST'||since.some(x=>x.low<=level+touchBand&&x.high>=level-touchBand);
    const held=last.close>=level-av*.05;
    const failed=last.close<level-failBuffer;
    const distanceAtr=(last.close-level)/av,distancePct=(last.close/level-1)*100;
    let stage='BREAKOUT_HOLD',severity='info',scoreDelta=0,label='돌파선 위 안착 확인 중',priority=50;
    if(failed){stage='FAILED_RETEST_RESISTANCE';severity='danger';scoreDelta=-15;label='돌파 후 재이탈 · 돌파선이 다시 저항으로 작동하는지 확인';priority=100}
    else if(touched&&held){stage='RETEST_SUPPORT_HOLD';severity='positive';scoreDelta=6;label='돌파 후 되돌림 · 리테스트 지지 확인 · 안착 중';priority=95}
    else if(distanceAtr>=1.5||distancePct>=7){stage='LATE_ENTRY_WARNING';severity='warning';scoreDelta=-5;label='돌파 후 이격 과다 · 늦은 진입 주의 · 추격보다 되돌림 대기';priority=90}
    else if(distanceAtr>=.70){stage='WAIT_PULLBACK_RETEST';severity='warning';scoreDelta=-2;label='돌파 진행 중 · 되돌림 리테스트 또는 안착 확인 대기';priority=75}
    else if(touched&&!held){stage='RETEST_RESISTANCE_CHECK';severity='danger';scoreDelta=-8;label='돌파선 재접근 · 지지 실패/저항 전환 여부 확인';priority=85}
    out.push({tf,kind:q.kind,source:q.label,stage,severity,label,scoreDelta,priority,level,distanceAtr,distancePct,touched,held,failed});
  }
  return out.sort((a,b)=>b.priority-a.priority)[0]||null;
}
...
function analyze({frames={},priceChange24h=null,priceChange1h=null,priceChange15m=null,structure='neutral',momentumSignals=null,takerRatio=null,oiChangePct=null,trueCvd=null}={}){
  const candles={};for(const tf of TF_ORDER)candles[tf]=rows(frames[tf]);
  const tfResults={};const tracks=[],breakoutStates=[];
  ...
  const bookManual=safe(()=>BookManual.analyze({frames,trueCvd}),null);
  if(bookManual){const top=(bookManual.rankable||[]).slice().sort((a,b)=>b.score-a.score)[0];if(top)tracks.push(track('BOOK_MANUAL','책 종합기법',top.score>=58,top.score,top.tf,top.stage,[top.label,...(top.reasons||[])],{techniqueId:top.id,bookScore:bookManual.score,riskPlan:bookManual.riskPlan}))}
  const tfWeight={'1d':12,'4h':8,'1h':3};const entryTiming=breakoutStates.slice().sort((a,b)=>(b.priority+(tfWeight[b.tf]||0))-(a.priority+(tfWeight[a.tf]||0)))[0]||null;
  const fakeout=safe(()=>Fakeout.analyze({frames,breakoutStates,closeBufferFraction:.001,minBarsBeforeFakeout:1,recoveryBufferFraction:.001}),null);
  const active=tracks.filter(x=>x.active).sort((a,b)=>b.score-a.score);
  const structural=active.filter(x=>['MA_RETEST','LIQUIDITY_RECLAIM','BOOK_PATTERN','BOOK_CONFLUENCE','DANTE','BOOK_MANUAL'].includes(x.key));
  ...
  let reentryScore=Math.max(structural[0]?.score||0,Number(bookManual?.score)||0);
  if(structural.length>=2)reentryScore+=6;if(structure==='bullish')reentryScore+=5;if(tk!=null&&tk>=1)reentryScore+=3;if(oi!=null&&oi>=0)reentryScore+=2;if((bookManual?.warnings||[]).length)reentryScore-=8;if(entryTiming)reentryScore+=Number(entryTiming.scoreDelta)||0;if(fakeout?.longBlocked)reentryScore-=25;else if(fakeout?.recovered)reentryScore+=4;
  reentryScore=Math.round(clamp(reentryScore));
  const flowOk=tk==null||tk>=.90;
  const hardRetestFail=entryTiming?.stage==='FAILED_RETEST_RESISTANCE'||Boolean(fakeout?.longBlocked);
  const reentryEligible=Boolean(!hardRetestFail&&priorSurge&&cooled&&flowOk&&(reset||structural.some(x=>['LIQUIDITY_RECLAIM','MA_RETEST'].includes(x.key)))&&reentryScore>=68&&structure!=='bearish');
  const patternEligible=Boolean(!hardRetestFail&&!reentryEligible&&cooled&&flowOk&&structural.length&&reentryScore>=68&&structure!=='bearish');
  const stage=fakeout?.longBlocked?'FAKEOUT_RISK':fakeout?.recovered?'FAKEOUT_RECOVERED':reentryEligible?'REENTRY_READY':patternEligible?'PATTERN_READY':priorSurge&&cooled?'REENTRY_WATCH':active.length?'PATTERN_WATCH':'NONE';
  const reasons=[];if(priorSurge)reasons.push('최근 상승 파동 이력');if(reset)reasons.push('고점 대비 눌림·리셋 확인');if(cooled)reasons.push('1H·15m 단기 과열 해소');if(fakeout?.primary)reasons.push(fakeout.primary.label);if(entryTiming)reasons.push(entryTiming.label);for(const x of structural.slice(0,3))reasons.push(x.label+' '+x.stage);
  const strategyCore={version:VERSION,stage,reentryEligible,patternEligible,reentryScore,priorSurge,cooled,reset,flowOk,impulse,entryTiming,breakoutStates:breakoutStates.slice(0,6),fakeout,activeTracks:active.slice(0,10),best:active[0]||null,reasons:reasons.slice(0,8),timeframes:tfResults,bookManual};
  const entryQuality=safe(()=>EntryConfirmation.analyze({frames,strategyCycle:strategyCore,bookManual,structure}),null);
  return{...strategyCore,entryQuality};
}
module.exports={VERSION,rows,maRetest,liquidityReclaim,impulseReset,postBreakoutState,analyze};
