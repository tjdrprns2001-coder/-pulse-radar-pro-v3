...
function classifyV2(input={}){
  ...
  if(sampleFresh)return scanClass('PRE-SURGE',96,[sample?.archetypeLabel,sample?.phaseLabel,`샘플 DNA ${sampleScore}점`,...(Array.isArray(sample?.reasons)?sample.reasons.slice(0,2):[])]);
  ...
  if(sampleAccum)return scanClass('ACCUMULATION-PRE',90,[sample?.archetypeLabel,`샘플 DNA ${sampleScore}점`,sample?.liquidity?.label]);
