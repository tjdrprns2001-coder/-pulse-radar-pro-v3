...
function buildTradeSignal(input={}){
  ...
  const strategy=input.strategyCycle&&typeof input.strategyCycle==='object'?input.strategyCycle:null,reentry=Boolean(strategy?.reentryEligible),patternSetup=Boolean(strategy?.patternEligible);
  const invalidations=[];
