...
const CATEGORY_ORDER=Object.freeze([
  '급등 전조 강함','재상승 준비','패턴 셋업','가짜 돌파·롱 회피','급등 전조 관찰','거래량 이상징후','매수세 유입',
  '눌림·재축적','이미 급등함','약세·이탈','데이터 부족·판정 보류'
]);
const SCAN_CLASS_ORDER=Object.freeze([
  'PRE-SURGE','RE-ENTRY','PATTERN-SETUP','FAKEOUT-RISK','ACCUMULATION-PRE','META-PRE','SECTOR-ROTATION','ANOMALY',
...
const TRADE_BLOCKED_SCAN_CLASSES=new Set(['FAKEOUT-RISK','POST-SURGE','DISTRIBUTION-RISK','PUMP-RISK','STALE']);
...
function classifyV2(input={}){
  ...
  const fusion=input.v2Flow?.fusion&&typeof input.v2Flow.fusion==='object'?input.v2Flow.fusion:null;
  ...
  const strategy=input.strategyCycle&&typeof input.strategyCycle==='object'?input.strategyCycle:null;
  if(strategy?.fakeout?.longBlocked)return scanClass('FAKEOUT-RISK',99,[strategy.fakeout?.primary?.label||'가짜 돌파 확정 · 신규 롱 회피','돌파선 재회복 전까지 롱 후보 제외']);
  if(strategy?.reentryEligible)return scanClass('RE-ENTRY',98,[`재상승 점수 ${Number(strategy.reentryScore||0).toFixed(0)}`,strategy.best?.label,strategy.best?.stage,...(strategy.reasons||[])]);
  if(strategy?.patternEligible)return scanClass('PATTERN-SETUP',92,[`패턴 점수 ${Number(strategy.reentryScore||0).toFixed(0)}`,strategy.best?.label,strategy.best?.stage,...(strategy.reasons||[])]);
  if(fusion?.eligible&&!extended)return scanClass('PRE-SURGE',99,[`책+DNA+수급+SMC/ICT 통합 ${fusion.score}/100`,...(fusion.reasons||[]).slice(0,3)]);
  if(fusion?.watch&&!extended&&fusion.bookStrong&&fusion.smartStrong)return scanClass('ACCUMULATION-PRE',92,[`통합 근거 ${fusion.score}/100 · 추가 수급 확인 대기`,...(fusion.counterEvidence||[]).slice(0,2)]);
...
function filterUniverse(exchangeInfo={},tickers=[]){
  ...
  return (Array.isArray(exchangeInfo.symbols)?exchangeInfo.symbols:[]).filter(s=>{
    ...
    return symbol&&base&&quote==='USDT'&&s.status==='TRADING'&&(!s.contractType||s.contractType==='PERPETUAL')&&s.isSpotTradingAllowed!==false&&!isLeveragedBase(base)&&tickerMap.has(symbol);
...
function classify(input={}){
  ...
  const strategy=input.strategyCycle&&typeof input.strategyCycle==='object'?input.strategyCycle:null;
  if(strategy?.fakeout?.longBlocked)return{category:'가짜 돌파·롱 회피',priority:99,reasons:reasons.length?reasons:[strategy.fakeout?.primary?.label||'가짜 돌파 확정 · 돌파선 재회복 대기'],dataState:state};
  if(strategy?.reentryEligible)return{category:'재상승 준비',priority:94,reasons:reasons.length?reasons:(strategy.reasons||['급등 후 재축적·재돌파 조건 확인']).slice(0,4),dataState:state};
  if(strategy?.patternEligible)return{category:'패턴 셋업',priority:88,reasons:reasons.length?reasons:(strategy.reasons||['다중 패턴 근거 확인']).slice(0,4),dataState:state};
...
function buildTradeSignal(input={}){
  ...
  const strategy=input.strategyCycle&&typeof input.strategyCycle==='object'?input.strategyCycle:null,reentry=Boolean(strategy?.reentryEligible),patternSetup=Boolean(strategy?.patternEligible),entryGrade=String(strategy?.entryQuality?.grade||'');
  const fusion=input.v2Flow?.fusion&&typeof input.v2Flow.fusion==='object'?input.v2Flow.fusion:null;
  const book=input.bookEvidence?.book&&typeof input.bookEvidence.book==='object'?input.bookEvidence.book:null;
  const smart=input.bookEvidence?.smartMoney&&typeof input.bookEvidence.smartMoney==='object'?input.bookEvidence.smartMoney:null;
  const invalidations=[];
  if(state!=='live')invalidations.push(`데이터 상태 ${state||'unknown'}는 실시간 신호에 사용할 수 없음`);
  if(TRADE_BLOCKED_SCAN_CLASSES.has(scanClassKey)&&!reentry){
    if(scanClassKey==='FAKEOUT-RISK')invalidations.push('가짜 돌파 확정 · 돌파선 재회복 전 신규 롱 제외');
    else if(scanClassKey==='POST-SURGE')invalidations.push('이미 급등 완료 구간이라 신규 매수 후보에서 제외');
  ...
  if((input.alreadySurged||category==='이미 급등함')&&!reentry)invalidations.push('이미 급등한 구간이라 신규 매수 후보에서 제외');
  ...
  if(strategy?.fakeout?.longBlocked)invalidations.push('가짜 돌파/저항 전환 위험 · 롱 회피');
  if(entryGrade==='D')invalidations.push('D급 진입 자리 · 추격/무효화/목표 공간 조건 불량');
  if(book?.bias==='하락'&&Number(book.score)>=72&&smart?.bias==='하락')invalidations.push(`책 합성·SMC/ICT 하락 반증이 겹침 (책 ${Math.round(Number(book.score))}/100)`);
  ...
  if(fusion?.eligible){confirmations++;confidence+=20;reasons.push(`책+DNA+OI/taker+거래량+SMC/ICT 통합 ${fusion.score}/100`)}
  else if(fusion?.watch){confirmations++;confidence+=8;reasons.push(`통합 근거 관찰 ${fusion.score}/100`)}
  if(reentry){confirmations++;confidence+=28;reasons.push(`재상승 패턴 ${Number(strategy.reentryScore||0).toFixed(0)}점 · ${strategy.best?.label||'다중 근거'}`)}
  else if(patternSetup){confirmations++;confidence+=22;reasons.push(`패턴 셋업 ${Number(strategy.reentryScore||0).toFixed(0)}점 · ${strategy.best?.label||'다중 근거'}`)}
  ...
  if(entryGrade==='A'){confidence+=10;reasons.push('A급 진입 자리 확인')}else if(entryGrade==='B'){confidence+=4;reasons.push('B급 진입 자리 · 추가 확인 허용')}
  ...
  const strongContext=preLabel==='가능성 높음'||category==='급등 전조 강함'||reentry||patternSetup||Boolean(fusion?.eligible);
  let level=strongContext&&confirmations>=4&&confidence>=80?'매수 후보':confidence>=35&&confirmations>=2?'관찰':'제외';
  const softObserveClasses=new Set(['SECTOR-ROTATION','ACCUMULATION-PRE','META-PRE','ANOMALY']);
  if(level==='제외'&&!invalidations.length&&softObserveClasses.has(scanClassKey)){
    level='관찰';
    invalidations.push('독립 확인 신호 대기 · 상태 분류는 유지하되 진입 확정은 보류');
  }
  ...
  if(level==='매수 후보'&&entryGrade==='C'){level='관찰';confidence=Math.min(confidence,68);invalidations.push('C급 진입 자리 · 목표 공간/위치 조건 부족으로 관찰 전환')}
  if(level==='매수 후보'&&strategy?.entryQuality?.lateEntry){level='관찰';confidence=Math.min(confidence,66);invalidations.push('돌파 후 늦은 진입 경고 · 추격보다 리테스트 대기')}
...
function beginnerSummary(item={}){
  ...
  const dataText=state==='live'?'가격·시간봉 데이터는 정상입니다.':state==='delayed'?'데이터가 지연되어 보수적으로 관찰합니다.':'데이터 상태가 불안정해 판정을 보류합니다.';
  const classification=item.scanClass?.label||item.category;
  const category=classification?`상태 분류는 ${classification}입니다.`:'';
  const signal=item.tradeSignal&&typeof item.tradeSignal==='object'?item.tradeSignal:null;
  const signalReason=Array.isArray(signal?.invalidations)&&signal.invalidations[0]?String(signal.invalidations[0]):'';
  const signalText=signal?.level==='매수 후보'
    ?`진입 판정은 매수 후보 · 신뢰도 ${Math.round(Number(signal.confidence)||0)}%입니다.`
    :signal?.level==='관찰'
      ?`진입 판정은 관찰입니다${signalReason?' · '+signalReason:''}.`
      :signal?.level==='제외'
        ?`진입 판정은 제외입니다${signalReason?' · '+signalReason:''}.`
        :'';
  return [structureText,category,signalText,reason.endsWith('.')?reason:`${reason}.`,dataText].filter(Boolean).join(' ');
}
