...
function classifyV2(input={}){
  ...
  const sampleFresh=sampleScore!=null&&sampleScore>=58&&sampleArchetype!=='NEUTRAL'&&['LATENT','IGNITION-WAIT','IGNITION-EARLY'].includes(samplePhase);
  if(sampleFresh){const best=Array.isArray(sample?.similarity)?sample.similarity[0]:null;return scanClass('PRE-SURGE',96,[sample?.archetypeLabel,sample?.phaseLabel,`샘플 DNA ${sampleScore}점`,best&&Number(best.score)>=55?`과거 샘플 ${best.name} ${best.score}% 유사`:null,...(Array.isArray(sample?.reasons)?sample.reasons.slice(0,2):[])])}
  const sampleAccum=sampleScore!=null&&sampleScore>=50&&['A','A+B','B'].includes(sampleArchetype)&&['LATENT','IGNITION-WAIT','OBSERVE'].includes(samplePhase);
