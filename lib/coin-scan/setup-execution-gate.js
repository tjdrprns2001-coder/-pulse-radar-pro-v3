...
function spotVolumeSignal(rows=[]){
  const q=(Array.isArray(rows)?rows:[]).map(kQuoteVolume).filter(Number.isFinite);
...
function nearestOpposingLiquidity(frames={},entry=null){
  const rows=Array.isArray(frames?.['1h'])?frames['1h']:[];
...
function atr(rows=[],period=14){
  const a=Array.isArray(rows)?rows:[];if(a.length<period+1)return null;const tr=[];
...
function bottomRisk({features,frames,execution,fundingRate,feeBps=4}={}){
  const s=features?.bottom?.evidence?.bottomStruct||{},entry=kClose((frames?.['5m']||[]).at(-1))??kClose((frames?.['15m']||[]).at(-1));
  const a=atr(frames?.['5m']||[],14)||atr(frames?.['15m']||[],14);
  const sweep=finite(s.sweepLow);const stop=sweep!=null&&a!=null?sweep-Math.max(.10*a,0):null;
  const target=nearestOpposingLiquidity(frames,entry);
...
function breakoutRisk({features,frames,execution,fundingRate,feeBps=4}={}){
  const e=features?.breakout?.evidence?.breakout||{},entry=kClose((frames?.['5m']||[]).at(-1))??kClose((frames?.['15m']||[]).at(-1));
  const a=atr(frames?.['5m']||[],14)||atr(frames?.['15m']||[],14);
...
function apply({item,frames={},execution={},spot15m=[],eventRisk=null,sequenceGap=false,feeBps=4}={}){
  ...
  const ex=executionMetrics(execution),spot=spotVolumeSignal(spot15m),funding=finite(item.fundingRate);
  const bottomR=bottomRisk({features:item.setupFeatures,frames,execution,fundingRate:funding,feeBps});
  const breakoutR=breakoutRisk({features:item.setupFeatures,frames,execution,fundingRate:funding,feeBps});
...
module.exports={executionMetrics,spotVolumeSignal,costAdjustedR,bottomRisk,breakoutRisk,apply};
