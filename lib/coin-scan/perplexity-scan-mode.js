...
function quality(row={},okxStatus='NOT_AVAILABLE'){
  const hardMissing=[],softMissing=[],warmup=[],core=['1w','1d','4h','1h','15m','5m'];
  for(const tf of core){const m=row.tf?.[tf];if(!m?.available)hardMissing.push(`tf:${tf}`);else if((finite(m.bars)||0)<120)warmup.push(`tf:${tf}:WARMUP_SHORT`)}
  if(finite(row.oi4hPct)==null)hardMissing.push('oi4h');
  if(finite(row.priceChange24h)==null)hardMissing.push('futuresTicker');
  const t15=crossStatus(row.takerCross?.['15m']);if(['MISSING','TIME_MISMATCH','CONFLICT'].includes(t15))softMissing.push(`taker15m:${t15}`);
  const t1=crossStatus(row.takerCross?.['1h']);if(['MISSING','TIME_MISMATCH','CONFLICT'].includes(t1))softMissing.push(`taker1h:${t1}`);
  const spot=spotStatus(row);if(spot!=='VERIFIED')softMissing.push('spotTicker');
  if(finite(row.fundingRatePct)==null)softMissing.push('funding');
  const status=hardMissing.length?'FAILED':(softMissing.length||warmup.length)?'PARTIAL':'VERIFIED';
  const crossValidationQuality=spot==='VERIFIED'&&okxStatus==='VERIFIED'?'VERIFIED':spot==='NOT_AVAILABLE'&&okxStatus==='NOT_AVAILABLE'?'NOT_AVAILABLE':'PARTIAL';
  return{status,missingFields:[...hardMissing,...softMissing],hardMissing,softMissing,warmup,spotStatus:spot,okxStatus,crossValidationQuality};
}
...
function classify(row={},context={},config=CONFIG){
  ...
  const ev=evidence(row,{pivot15m},config),spotAligned=spot!=null&&fut!=null&&((spot>=0&&fut>=0)||(spot<0&&fut<0)),takerOk=crossStatus(row.takerCross?.['15m'])==='PASS',oneHourTakerOk=crossStatus(row.takerCross?.['1h'])==='PASS',overheated=finite(h1.rsi14)>=70||(Math.abs(fut||0)>=9&&r15>=3),shortBuild=oi4>config.minOi4hPct&&(t15!=null&&t15<config.takerWeak||dir==='LONG_RISK'),ignition=q.status==='VERIFIED'&&!globalOiDegraded&&oi4>config.minOi4hPct&&dir==='LONG_BIAS'&&r15>=config.ignitionRvol15m&&takerOk&&t15>=config.takerImprove&&oneHourTakerOk&&t1>=config.takerWeak&&spotAligned&&!(fund>config.fundingCrowdedLongPct),transition=oi4>config.minOi4hPct&&dir!=='LONG_RISK'&&(h1.macdUp||m15.macdUp)&&r15>=config.transitionRvol15m&&takerOk&&t15>=config.takerImprove,accumulation=oi4>config.minOi4hPct&&Math.abs(fut||0)<=5&&h1.above20!==false&&t15>=config.takerWeak&&(spotAligned||spot==null),patternTags=[];
  if(oi4>config.minOi4hPct&&Math.abs(fut||0)<=5&&dir!=='LONG_RISK'&&(r15==null||r15<config.ignitionRvol15m))patternTags.push('PRE_SURGE');
  if(oi4>config.minOi4hPct&&t15>=config.takerImprove&&(r15==null||r15<config.ignitionRvol15m))patternTags.push('FLOW_LEAD');
  if(finite(row.oiDrawdownPct)<0&&finite(row.oi1hPct)>0&&oi4>0&&h1.macdUp)patternTags.push('CLEAN_REBUILD');
  if(flags.nfbHint&&Math.abs(fut||0)<=5)patternTags.push('NFB');
  if(globalOiDegraded)ev.counter.unshift('OI_SCAN_GLOBAL_DEGRADED');
  let key='INCOMPLETE',stage='INCOMPLETE',label='⚪ INCOMPLETE';
  if(q.status==='FAILED'||(globalOiDegraded&&oi4==null)){key='DATA_DEGRADED';stage='DATA_DEGRADED';label='⚠️ DATA_DEGRADED'}
  ...
  return{key,stage,label,direction:dir,priority:Math.max(0,Math.min(100,priority)),signalConfidence,crossValidationQuality,dataQuality:q,patternTags,positiveEvidence:ev.positive,counterEvidence:ev.counter,nextConfirmation,invalidationLevel:pivot15m.support,pivot15m,flags,takerZones:{'1h':takerZone(t1,config),'15m':takerZone(t15,config),'5m':takerZone(row.takerCross?.['5m']?.endpointRatio,config)},market};
}
