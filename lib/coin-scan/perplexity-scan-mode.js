...
function quality(row={},okxStatus='NOT_AVAILABLE'){
  ...
  const spot=spotStatus(row),status=missing.length?'FAILED':warmup.length?'PARTIAL':'VERIFIED',crossValidationQuality=spot==='VERIFIED'&&okxStatus==='VERIFIED'?'VERIFIED':spot==='NOT_AVAILABLE'&&okxStatus==='NOT_AVAILABLE'?'NOT_AVAILABLE':'PARTIAL';
  return{status,missingFields:missing,warmup,spotStatus:spot,okxStatus,crossValidationQuality};
}
...
function classify(row={},context={},config=CONFIG){
  ...
  if(q.status==='FAILED'||globalOiDegraded){key='DATA_DEGRADED';stage='DATA_DEGRADED';label='⚠️ DATA_DEGRADED'}
