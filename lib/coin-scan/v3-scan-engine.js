...
function evaluate({frames={},v2Profile={},range1wPct=null,range1dPct=null,precisionMode=false}={}){
  const long=analyzeLongTerm(frames),filter=longFilter(long.cards),rvols=rvolPack(frames),ma=movingAveragePack(frames),oi=oiPath(v2Profile),taker=takerPack(v2Profile),pd=nearestPd(frames['1h']||[]),flags=precisionFlags({range1wPct,range1dPct});
  const contradictions=[];if(filter.core.down>=2)contradictions.push('장기 핵심 3TF 중 2개 이상 하락');if(filter.mixed)contradictions.push(filter.lowCoverage?'장기추세 데이터 커버리지 부족':'6TF 정렬도 50% 미만');if(taker.state==='WEAK'&&oi.mainBuild)contradictions.push('OI 증가 중 taker 매도 우위');
  return{version:VERSION,paramSet:PARAM_SET,longTerm:{filter,cards:long.cards,summary:long.summary},oiPath:oi,taker,rvol:rvols,movingAverage:ma,nearestPd:pd,precision:{enabled:Boolean(precisionMode),...flags},contradictions,invalidation:contradictions.length>0,classificationAllowed:filter.typeClassificationAllowed};
}
