...
function causalSequencePack(frames={}){
  const out={version:Causal.VERSION,available:false,timeframes:{},reason:null};
  for(const tf of ['4h','1h']){
    const rows=toConfirmedCandles(frames[tf]||[]).slice(-560);
    if(rows.length<40){out.timeframes[tf]={bars:rows.length,available:false,reason:'INSUFFICIENT_BARS',minimumBars:40,contractPass:null,sequence:null};continue}
    ...
    out.timeframes[tf]={bars:r.bars,available:true,reason:null,sequence:r.sequence,contractPass:contract.pass,eventCounts:counts,latestEvents:(r.events||[]).slice(-12).map(e=>({type:e.event_type,bar:e.bar_index,payload:e.payload}))};
    out.available=true;
  }
  const primary=out.timeframes['4h']?.available?out.timeframes['4h']:out.timeframes['1h']?.available?out.timeframes['1h']:null;
  out.primaryTf=out.timeframes['4h']?.available?'4h':out.timeframes['1h']?.available?'1h':null;
  out.longStage=primary?.sequence?.long?.stage||'N/A';out.shortStage=primary?.sequence?.short?.stage||'N/A';
  out.contractPass=out.available?Object.values(out.timeframes).filter(x=>x.available).every(x=>x.contractPass!==false):null;
  out.reason=out.available?null:'INSUFFICIENT_BARS';
  return out;
}
...
function evaluate({frames={},v2Profile={},range1wPct=null,range1dPct=null,precisionMode=false}={}){
  const long=analyzeLongTerm(frames),filter=longFilter(long.cards),rvols=rvolPack(frames),ma=movingAveragePack(frames),oi=oiPath(v2Profile),taker=takerPack(v2Profile),pd=nearestPd(frames['1h']||[]),flags=precisionFlags({range1wPct,range1dPct}),causalIct=causalSequencePack(frames);
