...
function createMarketIntelligenceProvider({
  ...
  async function getCexCrossCheck(symbol,{binanceSpotTicker=null,binanceFuturesTicker=null}={}){
    const key=normalizeSymbol(symbol),settled=await Promise.all(sourceLoaders.map(fn=>fn())),sources=[];
  ...
  return{getCexCrossCheck,dex,news,deriveNewsEvents,scheduledEvents,walletMovements,walletVerification};
}
