...
function normalizeSymbol(s){let x=String(s||'').toUpperCase().replace(/[-_/:]/g,'');x=x.replace(/^T/,'').replace(/SWAP$/,'').replace(/PERP$/,'').replace(/USDTM$/,'USDT').replace(/^XBT/,'BTC');if(x.endsWith('USD')&&!x.endsWith('USDT'))x=x.slice(0,-3)+'USDT';return x}
...
function createMarketIntelligenceProvider({
  ...
  async function scheduledEvents(symbol){
    ...
    const base=baseOf(symbol);let mapping={};try{mapping=JSON.parse(env.COINMARKETCAL_COIN_IDS_JSON||'{}')}catch{}
    const coinId=mapping[base];if(!coinId)return{available:false,provider:'coinmarketcal',reason:'COIN_ID_NOT_CONFIGURED',items:[]};
    try{
      const d=await json('https://developers.coinmarketcal.com/v1/events?max=50&coins='+encodeURIComponent(String(coinId)),{ttlMs:300000,key:'intel:cmc:'+base,headers:{'x-api-key':token,Accept:'application/json'}});
