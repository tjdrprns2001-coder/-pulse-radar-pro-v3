...
function normalizeCalendar(items=[],decisionTime){
  ...
  for(const x of items||[]){
    ...
    out.push({
      ...
      revision:actualAt!=null&&actualAt<=t?n(x.revision):null,observed_at:observed,source_ref:String(x.source_ref??x.url??'')
    });
  }
  return out;
}
...
function onchainContext({items=[],unlockPct7d=null,walletLabelConfidence=0,flow={}}={}){
  const final=items.filter(x=>['CONFIRMED','FINAL'].includes(x.finality_status));
  const pending=items.filter(x=>!['CONFIRMED','FINAL'].includes(x.finality_status));
...
function buildEvidenceContext({intelligence={},decisionTime=Date.now(),onchainEvents=[],calendarEvents=[]}={}){
  ...
  return{
    version:'EVIDENCE_CONTEXT_r0.2',decision_time:new Date(ts(decisionTime)||Date.now()).toISOString(),
