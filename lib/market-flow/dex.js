...
function normalizeDexMarket(raw={},source='unknown'){
  ...
  const liquidityUsd=finite(raw.liquidityUsd??raw.liquidity?.usd),marketCapUsd=finite(raw.marketCapUsd??raw.marketCap??raw.fdv);
  const impulse=volume5mUsd!=null&&volume1hUsd>0?volume5mUsd/Math.max(1,volume1hUsd/12):null;
  const eventTimeMs=finite(raw.eventTimeMs??raw.updatedAtMs??raw.timestamp),fetchedAtMs=finite(raw.fetchedAtMs??raw.receiveTimeMs)??Date.now();
  ...
  return{
    ...
    liquidityUsd,marketCapUsd,liquidityMarketCapRatio:liquidityUsd!=null&&marketCapUsd!=null&&marketCapUsd>0?liquidityUsd/marketCapUsd:null,
    volume5mUsd,volume1hUsd,volume24hUsd,buys5m,sells5m,tx5m:finite(raw.tx5m)??(buys5m!=null&&sells5m!=null?buys5m+sells5m:null),
    volumeImpulse5m:impulse,volumeAcceleration:finite(raw.volumeAcceleration)??impulse,poolLiquidityChangePct:finite(raw.poolLiquidityChangePct??raw.poolLiquidityDeltaPct),lpNetFlowUsd:finite(raw.lpNetFlowUsd),priceImpactBps:finite(raw.priceImpactBps),uniqueTraderAcceleration:finite(raw.uniqueTraderAcceleration),whaleSwapShare:finite(raw.whaleSwapShare??raw.whaleSwapConcentration),
    activityScore,sourceConfidence:finite(raw.sourceConfidence)??(source==='geckoterminal'?84:80),eventTimeMs,fetchedAtMs,freshnessMs:eventTimeMs!=null?Math.max(0,fetchedAtMs-eventTimeMs):null,...side
  };
}
