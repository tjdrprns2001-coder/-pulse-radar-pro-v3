...
const handler=require('../../handlers/signal-alerts.js');
...
export default async function signalAlerts(req){const b=bridge();try{const rt=createSignalRuntime({getStore:name=>getStore(name)});await handler({method:req.method,query:queryFrom(req.url)},b.res,{alerts:rt.alerts})}catch(e){b.res.status(503).json({status:'error',error:String(e?.message||e),items:[]})}return b.response()}
